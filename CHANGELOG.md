...
- **Subcommand help is now read-only.** Every dispatched command accepts a
  sole `--help` or `-h` argument before parsing or handler dispatch, prints
  the top-level usage text, and exits 0 without initializing local state,
