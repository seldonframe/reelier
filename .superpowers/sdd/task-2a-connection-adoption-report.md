...
- `src/connections.ts`: adds reviewed inspection adapters, stable normalized MCP tool-schema digests, injected connection factories, reviewed read-only account probes, callable inspection, honest non-usable classifications, inventory orchestration, and the filesystem inventory loader. It never starts a real downstream itself and never returns provider bodies, route specifications, credential values, or caught error text.
- `src/observation/index.ts`: strengthens `ConnectionDescriptorV1`, adds `ConnectionAdoptionV1`, closed inventory contracts and normalizers, and exposes the connection APIs only through `reelier/observation`.
...
None. `src/mcp-client.ts` and package metadata did not require modification: the existing `DownstreamConnection` interface supports injected hermetic factories, the `reelier/observation` subpath already ships its declarations/runtime, the existing `./contract/*` export exposes the schemas, and the existing authority build copies the whole ABI directory. `.tmp-pack/` was pre-existing and remained untouched.
...
```text
✔ candidate pack matches come only from installed manifest identities (0.7405ms)
ℹ tests 15
ℹ suites 0
ℹ pass 15
ℹ fail 0
