...
- approved Gate 0 Linux fixed-slot correction: RED `906b68c` (`test(ledger): pin live slot released residue`) and GREEN/report `b15277f` (`fix(ledger): tolerate unrelated released slot residue`). The Task 4B plan is amended to authorize exactly this evidence-led exception; neither commit widens any other classifier branch or Windows host boundary.
...
Hosted runs `31690656890` / Ubuntu job `94417084543` and `31691716040` / Ubuntu job `94420370893` established the same narrow falsifier: reservation converged to one winner and 99 exact-existing outcomes, dispatch succeeded, and immediate acknowledge refused with `{ok:false,reason:"corruption"}` while the valid in-flight root carried one live fixed admission slot beside a byte-valid unrelated `.authority-ledger-lock-<same parent pid>-<different nonce>.released` marker plus normal ledger directories. The bare fixed-slot branch alone rejected `retired.size`; the preparation, W1, retired-preparation, withdrawal, and orphan-final families already use `blockingRetiredResidue`. The correction applies that same boundary to the bare fixed slot and changes no timeout, retry, concurrency, lock, or non-released corruption rule.
