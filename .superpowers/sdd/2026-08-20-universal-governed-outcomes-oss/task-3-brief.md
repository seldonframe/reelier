...
**Terminal review amendment:** Task 3 stopped after three fix loops because pure in-process JavaScript cannot attach a rejection reaction to a hostile native Promise without executing caller-controlled `constructor`/`Symbol.species`. Task 3B therefore makes executor code an explicit host-minted trusted, callback-only capability; it must synchronously return `undefined` and own its internal async rejection handling. Only serialized data crosses back. Promise-returning executors are unsupported in V1; arbitrary async executor containment requires a later child-process IPC task.
