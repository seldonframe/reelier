...
const bearerRegistryAuth: AuthFn<Request> = async (request) => {
  ...
  if (!entry || typeof entry.principalId !== "string" || typeof entry.taskId !== "string" || typeof entry.taskOwnerPrincipalId !== "string" || typeof entry.workloadId !== "string") {
    return null;
  }
  return {
    ...
    principalType: "user",
    attributes: { taskId: entry.taskId, taskOwnerPrincipalId: entry.taskOwnerPrincipalId, workloadId: entry.workloadId },
  };
};
