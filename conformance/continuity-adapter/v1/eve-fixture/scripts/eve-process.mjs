...
export async function runLiveAgentAdapterV0(resultPath, runtimeRoot, options = {}) {
  ...
  try {
    ...
    let cursor = 0;
    const messages = ["adapter catalog", "adapter load", `adapter delegation::${suffix}`, `adapter delegation status::${suffix}`, `adapter task status::${suffix}`, `adapter invoke v0::${suffix}`, `adapter status v0::${suffix}`];
