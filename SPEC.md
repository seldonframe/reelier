...
The authorization MUST link a signed closed `ReleaseOperationPlanV1`. `candidateTreeDigest` is the authority digest of the complete ordered path/mode/content-digest/blob-SHA list; it is distinct from the separately committed Git tree SHA. The candidate commit, commit identities/timestamps, PR and squash metadata, required checks, workflow bytes, package/version, npm absence, and tag MUST be fixed before dispatch. The squash commit SHA MUST be obtained from the merge result and reconciled against the exact PR, `main`, commit, and candidate tree; tag authority MUST derive its target only from that signed verified merge predecessor.

Each request MUST use a signed hash-chain journal with exclusive event creation, durable publication, atomic head, semantic request-ID binding, and authorization-wide cross-process serialization. The authenticated execution allocation MUST equal the alias's signed one-effect allocation, and a spent allocation MUST NOT be accepted under a different request ID. Content-addressed object calls MAY repeat only when the same signed expected SHA makes them convergent. Branch, PR, merge, and tag ambiguity MUST use authoritative readback. Merge and tag MUST NOT be resent after a persisted intent. Expiry MUST be checked immediately before every provider write; an already-ambiguous write MAY be reconciled read-only after expiry. Provider acknowledgement without exact readback MUST remain pending and MUST NOT produce verified evidence. The normal Authority coordinator, not the dedicated saga, owns durable budget consumption and receipt publication. A successor effect MUST require a signed confirmation of the predecessor's successfully published coordinator receipt, not merely the predecessor's provider evidence. The reference transition semantics and local-journal nonclaim are in `docs/specs/github-release-outcomes-v1.md`.
