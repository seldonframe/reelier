...
| `emit` | No (artifact attestation, 0.31.0+) | 0 or 1 | `"Duplicate 'emit' field in step"` |
...
`emit: {"projection":["args.<key>",...]}` (artifact attestation, 0.31.0+;
record shape §4.1's `emit` block, runtime §6.1e) declares WHICH parts of the
**filled** action args constitute the artifact this step emits. It exists for
the class of write that `attest:` cannot reach: a send, a settled refund,
anything a human has already read — there is no post-state to probe, so the
attestable object is the artifact that left rather than the world afterwards.
No probe tool is involved.

A closed shape — `projection` is the only key, and an unknown one is rejected
(`"Unknown 'emit' key ... — expected projection"`). In particular there is no
`constraints` slot: a reserved key with no defined types is an invitation to
mint some, and AP2 verifiers MUST treat unknown constraint types as failing.
`projection` MUST be a non-empty array of non-empty strings; **every entry MUST
carry the explicit `args.` prefix** addressing a top-level key of the filled
args, and there is deliberately no bare form (a bare entry already means a
top-level BODY key everywhere else in this spec, and re-using the spelling
request-side would invite exactly the confusion §3.2's `status` deferral
avoided). Entries are **ordered and duplicate-free** — the order is part of the
declaration and survives serialization verbatim; a repeat is rejected
(`"Duplicate 'emit.projection' entry ..."`), following RFC 9421 §2.3's rule
that a covered-components list names each identifier exactly once.
The list is capped at 32 entries and each full `args.<key>` name at 120
characters, reusing the same definitions as `attest`'s absent-field diagnosis.
An own `__proto__` key anywhere in the filled args is refused before a digest
is minted; canonicalization cannot represent that key faithfully, so dropping
it would create a false-MATCH class.

Selection semantics are inherited from the typed projection twin, not forked:
top-level scalars only, own properties only; absent and non-scalar values do
not project and are reported as `unresolved` (§4.1) rather than dropped.
`emit` on an `effect: read` step is a parse error (`"'emit' requires a
write-effect step ..."`) for the same reason `expect` is: the record block is
written only when a write-effect call dispatches, so it would be a declaration
nothing records, rendering as a clean pass. Unlike `expect`, `emit` does **not**
require `approve:` — an unapproved write has no authorization to name and still
has an artifact. Covered by the approval hash when present (§6.1c), and a step
without `emit:` hashes byte-identically to 0.29.0.
...
closed shape (`tool`/`args`/`projection`/`defer` — an unknown key is rejected:
`"Unknown 'attest' key ... — expected tool/args/projection/defer"`); `tool` MUST
...
| `write` (0.19.0+, §6.1c) | Present **iff** this step's tool call actually dispatched a write-effect (`idempotent-write`/`destructive`) call — never for a refused, skipped, or mocked step. `approved` is `true` iff dispatched via a matching `Step.approve` hash, `false` via the legacy `--allow-writes`/`--yes` flags. `approvalHash` (0.28.0+) names WHICH approval authorized it — the step's stamped hash, verified equal to the recomputed hash before dispatch (the same equality `approved` is read off) — and is absent exactly when `approved` is `false`, because a flag dispatch has no authorization to point at. `dispatchId` is an opaque UUID present only on deferred writes, distinguishing repeated identical dispatches; it is not content or authorization evidence. The two are derived from one value, so a record can never claim an approval it cannot name or name one it does not claim. **Unlike `attest`'s salted commitments below, this value is an UNSALTED `sha256` over the operation shape and is deliberately a stable correlator across runs and across tenants — that is its purpose (it is the join key of an expectation↔outcome pair). It is therefore recomputable by any third party holding a candidate skill file, i.e. a confirmation oracle for "did this receipt execute THIS operation". No new exposure class: `idempotencyKey` in the same block is already an unsalted hash over the FILLED args, which is strictly more revealing. Do not read §4.1's "cross-run hash joins are deliberately impossible" as covering this field; that sentence governs `attest` only.** `resource` is a best-effort, honestly-labeled extraction from the response body; absent, never fabricated, when nothing was found. Its two keys — `id` (from the response body's `id`, else `_id`) and `version` (from `version`, else `etag`, else `revision`, else `sha`) — are read only when the raw value is a string or number, stringified verbatim, and are **each independently optional**: `resource` may carry one, the other, or both, and is omitted entirely (never `{}`) when the body is not a JSON object or carries neither. A missing key means "the response did not state it" — never a default, never a fabricated value. Neither key is verified against the provider: they are copied out of the response the write itself returned, so a consumer MUST NOT read them as independent confirmation that the resource exists or is at that version. `duplicateOf` is the step number of an earlier step in the SAME run that wrote the identical `idempotencyKey`. `dispatchedAt` (state-conditioned approval) is the instant the dispatch was issued — present **only** on `expect:`-bearing steps (a skill with no `expect:` produces byte-identical records); with `stateCheck.observedAt` it carries the measured observation→dispatch window, which renderers omit (never clamp, never fabricate) when negative or absurd. |
| `emit` (artifact attestation, §6.1e) | Present **iff** the step declared `emit:` (§3.2) AND its tool call actually dispatched a write-effect call — never for a refused, skipped, or mocked step, the same rule `write` and `attest` follow. A commitment on a call that never went out would assert an emission that did not happen. It is the attestation for the class of write with NO post-state a probe could read back: a send, a settled refund, anything a human has already read. `artifactDigest` is an UNSALTED `sha256` over the type-tagged projection of the **filled** action args, computed after the fill and **before** dispatch. Like `write.approvalHash` and unlike `attest`'s salted commitments it is deliberately recomputable by a third party holding the artifact — that is its purpose, as the join key of an emission↔authorization pair — and it adds no new exposure class only because `idempotencyKey` in the `write` block beside it already hashes ALL the filled args, which is strictly more revealing. A consumer MUST NOT read that argument as generalizing to any other field. `projection` is the DECLARED coverage list, verbatim and in declared order; `resolved` and `unresolved` always partition it exactly. `unresolved` names declared entries the filled args did not carry, or carried non-scalar — names only, omitted when empty and never `[]`. A non-empty `unresolved` is a first-class finding, not a degraded digest: it is what keeps a server-side-rendered payload, a reference-valued field, or a two-call draft→send composition visible instead of publishing a commitment that covers less than the step declares. `approvalHash` names WHICH approval authorized the emission and is absent exactly when `write.approved` is `false`. `at` is when the commitment was computed, never an input to any comparison. **A consumer MUST NOT read this block as evidence that the artifact was delivered, read, or acted on, nor that its content was correct** — it attests what was emitted and nothing downstream of that (never-list #8). |
| `resolutionOf` (deferred resolution, §6.1e) | Present only on a `RunRecord.deferredResolution` step. It is the immutable cross-record join back to the dispatch being answered: `approvalHash` is the authorization that was current at dispatch, `artifactDigest` is the emitted artifact commitment, `deferredUntil` is the exact deadline that this resolution closes, and `dispatchId` identifies the individual dispatch even when identical payloads repeat. All four are required; a pending record missing any join key is unresolvable and MUST be skipped rather than guessed. This block asserts no new write and is never a substitute for `write` or `emit`. |
...
| `passed` | On an ordinary execution record, `passed` is `true` iff zero steps have `outcome === "failed"` — an `"unchecked"` step does **not** make the run fail, but it also does not count toward `totals.passed`. Always present. The sole exception is a record carrying `deferredResolution: true`, where this field is conservatively `false` so a long-lived `pending` or `absent` attestation can never serialize as a pass; that record is neither PASS nor FAIL, as the next row defines. On ordinary runs, because an `"unchecked"` step is by definition one whose final outcome carried no failure and which asserted nothing — which includes a step that diverged and was then **healed** by L1/L2 into a zero-assertion state (`level: 1`/`2`, `failures` reset to `[]`), and a synthetic `--fail` step that never really dispatched — a run of nothing but `"unchecked"` steps is `passed: true` while proving nothing (a step with zero assertions is not immune to failing — a tool error, an unbound template var, a bind failure or a refusal still makes it `"failed"`, per the `outcome` row in §4.1; it simply cannot fail an *assertion* it never made): `passed` is the absence of a detected failure, never evidence that anything was verified (§4.3). |
| `deferredResolution` (artifact attestation, 0.31.0+) | `true` only on the second record appended by `reelier resolve`; absent on ordinary runs and all older records. This record observes no action and is neither a passing nor a failing run: its result lives in each step's `attest.confidence`. To keep `pending`/`absent` from ever serializing as a pass, its required `passed` field is conservatively `false` even though `totals.failed` remains `0`; consumers MUST detect this marker and render a neutral deferred-resolution state, never PASS or FAIL. |
...
### 6.1e Pre-dispatch artifact attestation (`emit:`, 0.31.0+)

Source of truth: `src/artifact.ts`, `src/runner.ts` (`executeStep`'s
commitment block and the coverage gate), `test/artifact.test.ts`,
`test/emit-grammar.test.ts`, `test/emit-record.test.ts`. Design:
`docs/specs/artifact-attestation-v1.md`.

**Where the commitment is computed [Normative].** After the action args are
filled and **before** the tool call is dispatched. It MUST NOT be attached to
the approval-hash gate: that gate recomputes over the args TEMPLATE
(`{{placeholders}}` intact, §6.1c) and returns before any fill, so there is no
rendered artifact in hand there. This is the same window the `probeArgs`
commitment and the state gate already occupy.

Consequence worth stating, because it bounds the whole feature: for a
**fully-static** write the approval hash already binds the exact args, so
`emit:` adds a named, projected commitment but no new binding. The gap it
closes is the **templated** write, where the approval covers the shape and the
run-time fill supplies the bytes.

**What is recorded.** §4.1's `emit` block: `artifactDigest` over the type-tagged
projection, the declared `projection`, and the `resolved`/`unresolved`
partition. Values never appear — hashes, counts and field names only.

**The coverage gate.** With `state_gate: refuse` (§6.1c) the repo opts in, and a
write step whose declared projection did not **fully** resolve is refused
before dispatch: `outcome: "failed"`, the refusal string in `failures[]`
naming the unresolved fields, and the record carries **no `write` block, no
`attest`, and no `emit`** — dispatch provably never issued, and there is no
emission to commit to. No flag overrides it (`--allow-writes`/`--yes` are not
consulted). In **recorder mode** the finding is stamped into `emit.unresolved`
and the write still dispatches: fail open at the recorder, fail closed at the
gate. A repo without the opt-in, and any step without `emit:`, is
byte-identical to 0.29.0.

The refusal claims only that declared coverage did not resolve. It does **not**
claim the write would have been wrong, and no surface may imply it. The
comparison is membership over declared field NAMES — never content inspection.

**What it does not prove [Normative].** That the artifact was delivered, read,
or acted upon, and that its content was correct. It attests what was emitted
and nothing downstream of that. A consumer MUST NOT render an `emit` block as
evidence of delivery or of semantic correctness (never-list #8). An L2
re-dispatch sends a different artifact and the escalation path computes no
commitment of its own, so the main-path `emit` block is **dropped** rather than
carried onto a healed step — absent is honest, stale is not.
...
| Policy record (four states, raw-byte digest, run-path narrowness) | `test/policy-record.test.ts`, `test/policy-run-record.test.ts`, `test/policy-trace-meta.test.ts` |
...
The HTTP invocation form MUST be `POST /v1/jobs/<percent-encoded-jobRef>/invoke`, routed to the
same authenticated `handler.invoke` as MCP. The path MUST bind the opaque job reference; an omitted
body `jobRef` is filled from the path, an identical body value is accepted, and a different value
MUST refuse before dispatch. The request MUST retain the closed `jobRef`, `requestId`, `sourceRefs`,
and `choices` shape and MUST NOT accept caller-supplied identity, grant, allocation, repository, or
credential fields. The raw alias HTTP path MUST remain unable to bypass multi-definition signed job
authority. A missing invoke handler or malformed request MUST refuse and MUST NOT fall through to a
generic provider adapter.

The authorization MUST link a signed closed `ReleaseOperationPlanV1`. `candidateTreeDigest` is the authority digest of the complete ordered path/mode/content-digest/blob-SHA list; it is distinct from the separately committed Git tree SHA. The base commit and its read-back base-tree SHA are separate signed fields, and Git tree creation MUST use the base-tree SHA rather than a commit SHA. The candidate commit, commit identities/timestamps, exact draft PR metadata, the signed `readyForReview: true` internal PR subtransition, squash metadata, required checks, workflow bytes, package/version, npm absence, and tag MUST be fixed before dispatch. Draft creation and ready-for-review share the one PR Outcome/allocation and MUST NOT be modeled as a fifth provider effect. The squash commit SHA MUST be obtained from the merge result and reconciled against the exact PR, `main`, commit, candidate tree, and authorized base commit as its parent; tag authority MUST derive its target only from that signed verified merge predecessor. Every required quality check MUST carry the exact signed `.github/workflows/ci.yml` path and digest; another release workflow's digest is not CI evidence.

Each request MUST use a signed hash-chain journal with exclusive event creation, durable publication, atomic head, semantic request-ID binding, and authorization-wide cross-process serialization. Lock ownership uses monotonic persistent owner tickets: complete PID/nonce bytes MUST be fsynced privately and atomically hard-linked into the numbered queue, so a paused creator cannot expose a partial published ticket and contenders cannot overwrite a winner. No contender may check and unlink another live owner's path. Abandoned locks MUST recover without allowing concurrent entry. The authenticated execution allocation MUST equal the alias's signed one-effect allocation, and a spent allocation MUST NOT be accepted under a different request ID. Content-addressed object calls MAY repeat only when the same signed expected SHA makes them convergent. Branch, PR, ready, merge, and tag ambiguity MUST use authoritative readback. A persisted `*-dispatching` boundary forbids resend and permits read-only reconciliation; an intent without that boundary MUST NOT adopt externally created exact state. Explicit definitive refusal at any consequential write MUST be terminal for that request; only explicit transport uncertainty or a malformed acknowledgement from a write that may have applied may remain pending. Earlier confirmed internal provider writes MUST keep the logical Outcome's effect status partial/conflicting rather than `not-applied`. Expiry and base/ref state MUST be checked after durable intent and immediately before every provider write. Tag dispatch MUST also recheck package/version and npm absence after durable intent and immediately before write. Recovery of an already-dispatched tag MUST read the exact tag first and MAY reconcile an authorized applied tag after downstream npm publication, but MUST NOT resend an absent tag. Provider acknowledgement without exact readback MUST remain pending and MUST NOT produce verified evidence. The normal Authority coordinator, not the dedicated saga, owns durable budget consumption and receipt publication; release provider execution stays inside coordinator-minted prepared and ambiguous-reconcile capabilities bound to the durable request/allocation/semantics. The host composer and receipt wrapper MUST remain absent from the public package barrel; the supported public release runtime factory MUST accept only the exact four reviewed definitions and a branded host-owned runner, exposing only the normal authenticated opaque-job surface. Receipt confirmation requires a coordinator-minted publication call whose result reconciles to the authoritative durable head. If restart adopts an immutable acknowledged/reconciled head committed before that confirmation, the private wrapper MUST revalidate its request, semantics, authorization, alias, allocation, and provider evidence and idempotently confirm it without provider or publication resend; conflicts MUST refuse. A successor effect MUST require that signed confirmation of the predecessor's successfully published coordinator receipt, not merely the predecessor's provider evidence. The PR stage MUST first ensure the exact draft and then journal and reconcile the signed exact-PR ready-for-review subtransition before merge. Receipt completeness remains `unchecked`; logical-Outcome reconciliation does not prove every possible provider write was governed. The reference transition semantics and local-journal nonclaim are in `docs/specs/github-release-outcomes-v1.md`.
