...
export type OutcomeSemanticClass = "communication_commit_v1" | "record_state_set_v1" | "artifact_deliver_v1" | "schedule_commit_v1" | "money_refund_v1" | "commerce_purchase_commit_v1" | "deployment_release_v1";
...
export function normalizeSignedJobCard(value: unknown): SignedJobCardV1 {
  ...
  if (!Array.isArray(classes) || classes.length === 0 || classes.some(item => !["communication_commit_v1", "record_state_set_v1", "artifact_deliver_v1", "schedule_commit_v1", "money_refund_v1", "commerce_purchase_commit_v1", "deployment_release_v1"].includes(String(item)))) throw new TypeError("invalid semantic classes");
