...
export interface AuthorityMcpHandler { outcome(alias: string, input: unknown, context: { readonly tenant: string; readonly requester: string }): Promise<AuthorityIngressOutcome>; status(input: unknown, context: { readonly tenant: string; readonly requester: string }): Promise<AuthorityIngressOutcome>; jobsSearch?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>; jobLoad?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>; invoke?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<AuthorityIngressOutcome>; delegationRequest?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>; delegationStatus?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>; taskStatus?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>; }
...
export function buildAuthorityMcpServer(definitions: readonly AuthorityMcpDefinition[], handler: AuthorityMcpHandler, context: { readonly tenant: string; readonly requester: string }, artifactStage?: (input: unknown, context: { readonly tenant: string; readonly requester: string }) => Promise<unknown>): Server {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: [
    ...
    { name: "reelier_delegation_request", description: "Request a narrower child authority from an explicitly delegable parent.", inputSchema: { type: "object", additionalProperties: false, required: ["taskId", "parentAllocationId", "child", "effects", "activeChildCount"], properties: { taskId: { type: "string" }, parentAllocationId: { type: "string" }, child: { type: "object", additionalProperties: false }, effects: { type: "integer", minimum: 1 }, activeChildCount: { type: "integer", minimum: 0 } } } },
