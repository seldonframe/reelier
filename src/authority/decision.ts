...
export type GateDecisionLockFaultPoint="after-owner-read"|"before-lock-retire"|"after-lock-retire";
...
export function createFileGateDecisionSink(root:string,options:FileGateDecisionSinkOptions={}):GateDecisionSink {
  ...
  let rootIdentity=inspectDecisionRoot(resolvedRoot);
  function assertCurrentRoot():void{const current=inspectDecisionRoot(resolvedRoot);if(!rootIdentity||!current||current.dev!==rootIdentity.dev||current.ino!==rootIdentity.ino)throw new Error("decision root changed");}
  async function prepareRoot():Promise<void>{if(!rootIdentity){await mkdir(resolvedRoot,{recursive:true});rootIdentity=inspectDecisionRoot(resolvedRoot);if(!rootIdentity)throw new Error("decision root unavailable");}assertCurrentRoot();}
  async function load():Promise<GateDecisionRecord[]>{if(!rootIdentity){const current=inspectDecisionRoot(resolvedRoot);if(!current)return[];rootIdentity=current;}assertCurrentRoot();let bytes:Buffer;try{bytes=await readFile(file);}catch(error){if(hasCode(error,"ENOENT"))return[];throw error;}try{const parsed=JSON.parse(bytes.toString("utf8")) as Record<string,unknown>;assertKeys(parsed,["records","v"]);if(parsed.v!=="reelier.gate-decision-store/internal-v1"||!Array.isArray(parsed.records))throw new DecisionStoreCorruption();const records=parsed.records.map(parseGateDecisionRecord);if(new Set(records.map(item=>item.gateEvent.eventId)).size!==records.length)throw new DecisionStoreCorruption();if(new Set(records.filter(item=>item.role==="primary").map(item=>item.ingressClaimDigest)).size!==records.filter(item=>item.role==="primary").length)throw new DecisionStoreCorruption();if(new Set(records.filter(item=>item.reservationId!==null).map(item=>item.reservationId)).size!==records.filter(item=>item.reservationId!==null).length)throw new DecisionStoreCorruption();return records;}catch(error){if(error instanceof DecisionStoreCorruption)throw error;throw new DecisionStoreCorruption();}}
  async function withLock<T>(callback:()=>Promise<T>):Promise<T>{await prepareRoot();const owner=await acquireLock();try{assertCurrentRoot();return await callback();}finally{await releaseLock(owner);}}
  async function acquireLock():Promise<DecisionLockOwner>{const deadline=Date.now()+timeout;for(;;){const owner:DecisionLockOwner={host:hostname(),nonce:randomBytes(32).toString("hex"),pid:process.pid,v:"reelier.gate-decision-lock/internal-v1"};try{await mkdir(lock);const handle=await open(ownerFile,"wx",0o600);try{await handle.writeFile(authorityCanonicalBytes(owner));await handle.sync();}finally{await handle.close();}await syncRoot();return owner;}catch(error){if(!hasCode(error,"EEXIST")&&!isTransientLockError(error))throw error;}let ownerBytes:Buffer;try{ownerBytes=await readFile(ownerFile);}catch(error){if(Date.now()<deadline&&(hasCode(error,"ENOENT")||isTransientLockError(error))){await delay(2);continue;}throw error;}let existing:DecisionLockOwner;try{existing=parseDecisionLockOwner(ownerBytes);}catch{if(Date.now()<deadline){await delay(2);continue;}throw new Error("decision lock owner unavailable");}if(existing.host!==hostname())throw new Error("decision lock owner unavailable");const liveness=processLiveness(existing.pid);if(liveness==="unverifiable")throw new Error("decision lock owner unavailable");if(liveness==="alive"){if(Date.now()>=deadline)throw new Error("decision lock busy");await delay(2);continue;}options.lockFaultInjector?.("after-owner-read");if(!await retireOwnedLock(ownerBytes,deadline))continue;}}
  async function releaseLock(owner:DecisionLockOwner):Promise<void>{try{await retireOwnedLock(authorityCanonicalBytes(owner),Date.now()+timeout);}catch{/* A failed atomic retirement leaves the complete original lock owner in place. */}}
  async function retireOwnedLock(expected:Buffer,deadline:number):Promise<boolean>{const tombstone=path.join(resolvedRoot,`.gate-decisions-lock-${process.pid}-${randomBytes(16).toString("hex")}.retired`);for(;;){const current=await readFile(ownerFile);if(!current.equals(expected))return false;options.lockFaultInjector?.("before-lock-retire");try{await rename(lock,tombstone);break;}catch(error){if(Date.now()<deadline&&isTransientLockError(error)){await delay(2);continue;}throw error;}}options.lockFaultInjector?.("after-lock-retire");await syncRoot();const retiredOwner=path.join(tombstone,"owner.json"),retiredBytes=await readFile(retiredOwner);if(!retiredBytes.equals(expected)){try{await rename(tombstone,lock);}catch{}throw new Error("retired decision lock owner changed");}let retiredOwnerRemoved=false;for(;;){try{if(!retiredOwnerRemoved){await unlink(retiredOwner);retiredOwnerRemoved=true;}await rmdir(tombstone);await syncRoot();return true;}catch(error){if(Date.now()<deadline&&(hasCode(error,"ENOENT")||isTransientLockError(error))){await delay(2);continue;}throw error;}}}
  ...
  async function replaceAtomically(source:string,target:string):Promise<void>{const deadline=Date.now()+timeout;for(;;){try{await rename(source,target);return;}catch(error){if(process.platform!=="win32"||!(hasCode(error,"EPERM")||hasCode(error,"EACCES")||hasCode(error,"EBUSY"))||Date.now()>=deadline)throw error;await new Promise(resolve=>setTimeout(resolve,2));}}}
