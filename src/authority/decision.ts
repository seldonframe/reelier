...
export function createFileGateDecisionSink(root:string,options:FileGateDecisionSinkOptions={}):GateDecisionSink {
  ...
  function classifyExisting(records:GateDecisionRecord[],record:GateDecisionRecord):GateDecisionAppendResult|null{const canonical=authorityCanonicalBytes(record),event=records.find(item=>item.gateEvent.eventId===record.gateEvent.eventId);if(event)return canonical.equals(authorityCanonicalBytes(event))?Object.freeze({ok:true as const,status:"idempotent" as const,recordDigest:authorityDigest(record)}):Object.freeze({ok:false as const,reason:"event-id-conflict" as const});if(record.role==="primary"&&records.some(item=>item.role==="primary"&&item.ingressClaimDigest===record.ingressClaimDigest))return Object.freeze({ok:false as const,reason:"primary-ingress-conflict" as const});if(record.reservationId!==null&&records.some(item=>item.reservationId===record.reservationId))return Object.freeze({ok:false as const,reason:"reservation-conflict" as const});return null;}
