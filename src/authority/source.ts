...
export function materializeSourceBundle(registry:SourceRegistry,input:Readonly<SourceValidationAuthority&{sourceRefs:Readonly<Record<string,string>>;observedAt:Date;validationNow:Date;plans:readonly PlannedSourceRead[];observations:readonly RawSourceObservation[]}>):ValidatedSourceBundle{
  ...
  try{for(const observation of input.observations){if(byDigest.has(observation.planDigest)) throw new TypeError("duplicate source observation");if(!planned.some(p=>p.planDigest===observation.planDigest)) throw new TypeError("unknown or extra source observation");byDigest.set(observation.planDigest,Uint8Array.from(observation.rawBytes));}if(byDigest.size!==planned.length) throw new TypeError("missing source observation");}catch(error){if(error instanceof TypeError)throw new AuthorityBoundaryError("source","source-observation-invalid",error.message);throw error;}
  ...
  let projected:SourceProjection;try{projected=resolver.project(Object.freeze({plans:planned,observations:resolverObservations,observedAt}));}catch(error){if(error instanceof TypeError)throw new AuthorityBoundaryError("source","source-projection-invalid",error.message);throw error;}let claims:ReturnType<typeof canonicalClaims>;try{claims=canonicalClaims(projected.claims,projected.projection,input.authorizedProjectionPointers,input.requiredGroundedPointers);}catch(error){if(error instanceof TypeError)throw new AuthorityBoundaryError("source","source-ungrounded",error.message);throw error;}
