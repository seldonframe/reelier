...
export function verifyPortableOutcomeEvidencePublication(value: unknown, options: PortableOutcomeEvidenceVerificationOptions): Readonly<{ status: "verified"; digest: string }> {
  ...
  if (!["matched", "conflict", "not-attempted"].includes(reconciliation.verdict) || !Number.isSafeInteger(reconciliation.providerWriteCount) || reconciliation.providerWriteCount < 0 || !Number.isSafeInteger(reconciliation.resendCount) || reconciliation.resendCount < 0) throw new TypeError("portable reconciliation verdict or counters are invalid");
...
function portableQueryContainsConfidential(query: string): boolean { const auth = /(?:access[-_ ]?token|auth(?:orization)?|bearer|credential|password|secret|api[-_ ]?key)/i; for (const pair of query.split("&")) { const [rawKey, ...rawValue] = pair.split("="); let key = rawKey ?? "", value = rawValue.join("="); try { for (let i = 0; i < 3; i += 1) { const nextKey = decodeURIComponent(key), nextValue = decodeURIComponent(value); if (nextKey === key && nextValue === value) break; key = nextKey; value = nextValue; } } catch { return true; } if (auth.test(key) || auth.test(value)) return true; } return false; }
