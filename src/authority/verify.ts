...
function portableExact(value: unknown, fields: readonly string[], label: string): Record<string, any> { assertPortableInertRecord(value, label); const keys = Object.keys(value); if (keys.length !== fields.length || fields.some((field) => !Object.hasOwn(value, field))) throw new TypeError(`portable ${label} is not closed or canonical`); return value; }
