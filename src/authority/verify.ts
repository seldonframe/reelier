...
export function verifyPortableOutcomeEvidencePublication(value: unknown, options: PortableOutcomeEvidenceVerificationOptions): Readonly<{ status: "verified"; digest: string }> {
  ...
  if (route.portableMaterializedRequestDigest !== authorityDigest(request) || route.responseSemanticsProfileDigest !== authorityDigest(profile)) throw new TypeError("portable materialized request projection or response profile is not route-authorized");
  if (request.v !== "reelier.materialized-http-request/v1" || !["POST", "PUT", "PATCH", "DELETE"].includes(request.method) || typeof request.origin !== "string" || request.normalizedPath !== "/repos/{owner}/{repository}/issues/{issueNumber}/labels" || typeof request.normalizedQuery !== "string" || !request.reviewedHeaders || typeof request.reviewedHeaders !== "object" || typeof request.bodyDigest !== "string" || !digest.test(request.bodyDigest)) throw new TypeError("portable non-identifying materialized request projection is invalid");
  if (profile.v !== "reelier.http-response-semantics/v1" || typeof profile.profileId !== "string" || !Array.isArray(profile.acknowledgedStatuses) || profile.acknowledgedStatuses.length === 0 || profile.acknowledgedStatuses.some((status: unknown) => !Number.isInteger(status) || (status as number) < 200 || (status as number) > 299) || new Set(profile.acknowledgedStatuses).size !== profile.acknowledgedStatuses.length) throw new TypeError("portable response semantics profile statuses are invalid or duplicated");
