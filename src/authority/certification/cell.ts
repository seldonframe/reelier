...
import { consumeCertificationLifecycleAuthority, registerCertificationLifecycleTrustContext, registerCertificationLifecycleTrustRefreshSource, type CertificationArtifactKeyBindingCommitmentV1, type CertificationArtifactKeyBindingV1, type CertificationLifecycleAuthorityHandle, type CertificationLifecycleAuthorityMaterial } from "./lifecycle-authority.js";
...
async function activateCertificationRootTask(input: Readonly<{
  ...
  await input.delegationAuthority.registerRoot({ taskId: activation.taskId, allocationId: activation.rootAllocationId, rootGrant: signedRootGrant, signerDescriptor: delegation, effects: input.effects });
...
export interface CertificationCellHostInternalState {
  ...
  observeHermeticGitHubState(bearerToken: string): Promise<Readonly<{ digest: string; adapterContractDigest: string; preimage: Readonly<Record<string, unknown>> }>>;
  revalidateHermeticGitHubPermit(permit: object): Promise<void>;
  hermeticGitHubPermitSnapshot(permit: object): Readonly<{ digest: string; adapterContractDigest: string; preimage: Readonly<Record<string, unknown>> }>;
...
export async function createCertificationCellHost(input: Readonly<{ workspace: string; currentTrustPinPath: string; delegationAuthority: DelegationAuthority; principalRegistry: PrincipalRegistry; now?: () => Date; lifecycleAuthority?: CertificationLifecycleAuthorityInput }>): Promise<CertificationCellHost> {
  ...
  registerCertificationLifecycleTrustRefreshSource(hermeticAuthority.lifecycle,async()=>{const currentPath=await canonicalExternalTrustPin(workspace,configuredTrustPinPath);if(trustPinPathDigest(currentPath)!==currentTrustPinPathDigest)throw new TypeError("operator current trust pin path changed after host configuration");const jobCardTrustPin=JSON.parse((await readUnlinkedFile(currentPath)).toString("utf8")) as JobCardTrustPinV1;return Object.freeze({tenant:loaded.initialization.identifiers.authorityCellId,jobCardTrustPin,expectedAuthorityCellId:loaded.initialization.identifiers.authorityCellId,expectedTaskId:loaded.initialization.identifiers.taskId,observedAt:input.now?.()??new Date()});});
...
interface DispatchSnapshot { readonly digest: string; readonly adapterContractDigest: string; readonly preimage: Readonly<Record<string, unknown>>; readonly scenarioId: CertificationScenarioId; readonly runnerId: string; readonly metadataDigest: string; readonly endpointManifestDigest: string; readonly capabilities: readonly EndpointCapability[] }
...
function hermeticGitHubPermitSnapshot(permit: object): Readonly<{ digest: string; adapterContractDigest: string; preimage: Readonly<Record<string, unknown>> }> {
  const state = hermeticPermitState.get(permit);
  if (!state) throw new TypeError("hermetic GitHub dispatch permit is invalid or already used");
  return Object.freeze({ digest: state.snapshot.digest, adapterContractDigest: state.snapshot.adapterContractDigest, preimage: state.snapshot.preimage });
}
...
async function bindHermeticGitHubAuthority(pinPath: string, input: CertificationLifecycleAuthorityInput, identifiers: CertificationIdentifiers, now: Date): Promise<CertificationHermeticGitHubAuthorityState> {
  ...
  registerCertificationLifecycleTrustContext(lifecycle,{tenant:identifiers.authorityCellId,jobCardTrustPin:pin,expectedAuthorityCellId:identifiers.authorityCellId,expectedTaskId:identifiers.taskId,observedAt:now});
...
async function dispatchSnapshot(input: Readonly<{ workspace: string; scenario: CertificationScenarioId; bearerToken: string; currentTrustPinPath: string; currentTrustPinPathDigest: string; delegationAuthority: DelegationAuthority; principalRegistry: PrincipalRegistry; now?: () => Date }>, mode: "ordinary" | "hermetic-github", requireEffect = true): Promise<DispatchSnapshot> {
  ...
  if (!allocation || allocation.revoked || allocation.taskId !== activation.taskId || (requireEffect && allocation.remaining < 1)) throw new TypeError("certification effect allocation is exhausted or inactive");
