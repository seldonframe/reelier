...
async function priorBundle(root: string, effectDigest: string): Promise<AuthorityReceiptBundle | undefined> { const directory = path.join(root, "portable"); try { for (const name of await readdir(directory)) { if (!name.endsWith(".json")) continue; const bundle = parseAuthorityReceiptBundle(JSON.parse(await readFile(path.join(directory, name), "utf8"))); if (bundle.transportEffect.digest === effectDigest) return bundle; } return undefined; } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined; throw error; } }
async function save(root: string, bundle: AuthorityReceiptBundle): Promise<void> { await mkdir(path.join(root, "portable"), { recursive: true }); await writeFile(path.join(root, "portable", `${bundle.receipt.value.receiptId}.json`), `${JSON.stringify(bundle)}\n`, { flag: "wx", mode: 0o600 }); }
async function chainTail(root: string): Promise<AuthorityReceiptBundle | undefined> { const directory = path.join(root, "portable"); try { const bundles = await Promise.all((await readdir(directory)).filter(name => name.endsWith(".json")).map(async name => parseAuthorityReceiptBundle(JSON.parse(await readFile(path.join(directory, name), "utf8"))))); const prior = new Set(bundles.map(item => item.receipt.value.priorReceiptDigest).filter(Boolean)); return bundles.find(item => !prior.has(authorityDigest(item.receipt.value))); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return undefined; throw error; } }
