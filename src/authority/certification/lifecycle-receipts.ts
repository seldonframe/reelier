...
async function chainTail(root: string): Promise<AuthorityReceiptBundle | undefined> { const directory = await confinedExistingDirectory(root, ["portable"]); if (!directory) return undefined; const bundles = await Promise.all((await listConfinedFileNames(root, directory)).filter(name => name.endsWith(".json")).map(async name => parseAuthorityReceiptBundle(JSON.parse((await readConfinedFile(root, directory, name)).toString("utf8"))))); const prior = new Set(bundles.map(item => item.receipt.value.priorReceiptDigest).filter(Boolean)); return bundles.find(item => !prior.has(authorityDigest(item.receipt.value))); }
