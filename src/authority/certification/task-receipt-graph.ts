...
export function verifyCertificationTaskReceiptGraph(value: unknown, options?: Readonly<{ trustPin: JobCardTrustPinV1 }>): Readonly<{ status: "verified"; digest: string }> {
  ...
  if (Object.keys(g.leases ?? {}).join("\0") !== "status\0entries" || g.leases.status !== "absent" || !Array.isArray(g.leases.entries) || g.leases.entries.length !== 0) throw new TypeError("receipt graph leases cannot be verified without signed lease evidence");
...
function exactRecord(value: unknown, fields: readonly string[], label: string): any { if (!value || typeof value !== "object" || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype || Object.keys(value).join("\0") !== fields.join("\0")) throw new TypeError(`${label} is not an exact canonical object`); return value as any; }
