...
function parseProjectedConfig(value: unknown, manifestScenarios: readonly CertificationScenarioId[]): any {
  ...
  const metadata = Object.freeze(inertArray(raw.metadata, "certification export metadata").map((item: unknown) => { const entry = object(item, "certification export metadata item"); closed(entry, ["section", "digest", "status"], "certification export metadata item"); if (typeof entry.section !== "string" || entry.status !== "configured") throw new TypeError("certification export metadata item is invalid"); return Object.freeze({ section: entry.section, digest: assertDigest(entry.digest, "certification export metadata digest"), status: "configured" as const }); }));
...
function stringList(value: unknown, label: string): readonly string[] { const result = inertArray(value, label); if (result.some(item => typeof item !== "string")) throw new TypeError(`${label} is invalid`); return Object.freeze([...(result as string[])]); }
...
function inertJson(value: unknown, label: string): unknown { if (value === null || typeof value === "string" || typeof value === "boolean" || (typeof value === "number" && Number.isSafeInteger(value))) return value; if (Array.isArray(value)) return Object.freeze(inertArray(value, label).map((item, index) => inertJson(item, `${label}[${index}]`))); const raw = object(value, label); return Object.freeze(Object.fromEntries(Object.keys(raw).sort().map(key => [key, inertJson(raw[key], `${label}.${key}`)]))); }
