...
export async function createGitHubIssueLabelsHermeticComposition(cell: CertificationCellHost): Promise<GitHubIssueLabelsHermeticComposition> {
  ...
  return Object.freeze({ run, recover,
    async exportGraph(value: Readonly<{ bearerToken: string }>) { closed(value, ["bearerToken"], "GitHub graph export call"); await state.principalRegistry.resolve(value.bearerToken, state.now?.() ?? new Date()); const portable = await confinedExistingDirectory(receiptRoot, ["portable"]); if (!portable) throw new TypeError("portable receipt store is absent"); const receipts = await Promise.all((await listConfinedFileNames(receiptRoot, portable)).filter(name => name.endsWith(".json")).map(async name => parseAuthorityReceiptBundle(JSON.parse((await readConfinedFile(receiptRoot, portable, name)).toString("utf8"))))); const outcomes: Journal[] = []; for (const name of await readdir(journalRoot)) if (name.endsWith(".journal.json")) outcomes.push((await loadJournal(journalRoot, name.slice(0, -13), journalAuthority))!); const allocation = await state.delegationAuthority.budget.get(activation.allocationId); if (!allocation) throw new TypeError("GitHub graph allocation is absent"); const evidence = journalAuthority.lifecycle.direct.get("authority-evidence"); if (!evidence) throw new TypeError("authority evidence signer is absent"); return createCertificationTaskReceiptGraph({ taskId: activation.taskId, authorityCellId: activation.authorityCellId, rootGrant: activation.signedRootGrant, grants: [activation.signedRootGrant], principals: [{ principalId: activation.principalId, runtimeSessionId: activation.runtimeSessionId }], allocations: [allocation], budgetEvents: [{ kind: "consumed", allocationId: activation.allocationId, effects: allocation.consumed }], outcomes, exceptions: outcomes.filter(item => item.phase === "failed" || item.phase === "pending-reconciliation"), receipts, binding: journalAuthority.binding, commitment: journalAuthority.commitment, keyDescriptors: journalAuthority.keyDescriptors, signedReadiness: journalAuthority.signedReadiness, terminalSigner: { signerId: evidence.descriptor.keyId, sign: digest => signAuthorityDigest(evidence.privateKey, "authority-evidence", digest) } }); },
