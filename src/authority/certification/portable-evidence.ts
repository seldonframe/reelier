...
export function verifyCertificationPortableEvidence(input: Readonly<{ taskAuthorities: readonly unknown[]; postStateEvidence: readonly unknown[]; policyEvidence: readonly unknown[]; taskStatusEvidence: readonly unknown[]; duplicateDecisions: readonly unknown[] }>, context: Readonly<{ verifier: CertificationEvidenceVerifier; taskId: string; receipts: readonly any[]; outcomes: readonly any[]; budgetEvents: readonly any[]; adapterContractDigest: string }>): void {
  ...
  for (const raw of input.postStateEvidence) { const post = signedExact(raw, ["v","requestId","dispatchRequestDigest","permitSnapshotDigest","expectedProjectionDigest","preSourceBundleDigest","projectionSchemaId","projectionSchemaDigest","preProjectionDigest","observedProjectionDigest","observationMethod","observedAt","confidence","signerId","signature"], context.verifier, "portable post-state") as CertificationPostStateEvidenceV1; validatePostStateBody(post); if (!journalSnapshots.has(post.permitSnapshotDigest) || !context.outcomes.some(item => item.requestId === post.requestId && item.requestDigest === post.dispatchRequestDigest)) throw new TypeError("portable post-state is not linked to authorized dispatch"); const sourceBundle = context.receipts.find(bundle => bundle.sourceBundle?.digest === post.preSourceBundleDigest), source = sourceBundle?.sourceBundle, sourceAuthority = sourceBundle?.contract?.value?.sourceAuthority; if (post.preSourceBundleDigest !== null && (!source || authorityDigest(source.value?.projection?.labels) !== post.preProjectionDigest || sourceAuthority?.projectionSchemaId !== post.projectionSchemaId || !Array.isArray(sourceAuthority?.authorizedProjectionPointers) || !sourceAuthority.authorizedProjectionPointers.includes("/labels"))) throw new TypeError("portable post-state pre-read SourceBundle is omitted, substituted, or non-comparable"); if (post.projectionSchemaDigest !== authorityDigest({ schemaId: post.projectionSchemaId, pointers: ["/labels"] })) throw new TypeError("portable post-state projection schema is substituted"); }
  ...
  if ((input.taskStatusEvidence[0] as any).durableHistoryDigest !== authorityDigest(rawBudgetEvents.slice(0, 2)) || (input.taskStatusEvidence[1] as any).durableHistoryDigest !== authorityDigest(rawBudgetEvents)) throw new TypeError("portable task status durable history is substituted");
