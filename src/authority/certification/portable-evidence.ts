...
export function createCertificationDuplicateDecision(input: Omit<CertificationDuplicateDecisionV1, "v" | "observedAuthorityStateDigest" | "budgetDelta" | "providerWriteDelta" | "signerId" | "signature">, signer: CertificationEvidenceSigner): CertificationDuplicateDecisionV1 {
  const body = { v: "reelier.certification-duplicate-decision/v1" as const, attemptRequestId: input.attemptRequestId, originalRequestId: input.originalRequestId, originalRequestDigest: input.originalRequestDigest, originalEffectDigest: input.originalEffectDigest, observedAuthorityState: input.observedAuthorityState, observedAuthorityStateDigest: authorityDigest(input.observedAuthorityState), observedAt: input.observedAt, budgetDelta: 0 as const, providerWriteDelta: 0 as const, signerId: signer.signerId };
  validateDuplicateBody(body);
  return Object.freeze({ ...body, signature: signer.sign(authorityDigest(body)) });
}

export function verifyCertificationPortableEvidence(input: Readonly<{ taskAuthorities: readonly unknown[]; postStateEvidence: readonly unknown[]; policyEvidence: readonly unknown[]; taskStatusEvidence: readonly unknown[]; duplicateAttempts: readonly unknown[]; duplicateDecisions: readonly unknown[] }>, context: Readonly<{ verifier: CertificationEvidenceVerifier; journalVerifier: CertificationEvidenceVerifier; trustPin: JobCardTrustPinV1; taskId: string; authorityCellId: string; grants: readonly any[]; allocations: readonly any[]; receipts: readonly any[]; outcomes: readonly any[]; budgetEvents: readonly any[]; adapterContractDigest: string }>): void {
  ...
  for (const status of input.taskStatusEvidence) { verifyCertificationTaskStatusEvidence(status, context.verifier); const s = status as any, allocation = context.allocations.find(item => item.parentAllocationId !== null), child = context.grants[1]?.grant; if (s.taskId !== context.taskId || s.grantExpiresAt !== child?.expiresAt || s.allocationRevoked !== allocation?.revoked || s.currentActiveClaim !== (s.lifecycleState === "active" && !allocation?.revoked && Date.parse(child.expiresAt) > Date.parse(s.observedAt))) throw new TypeError("portable task status is not tied to graph authority"); }
  ...
  for (const raw of input.duplicateDecisions) { const node = signedExact(raw, ["v","attemptRequestId","originalRequestId","originalRequestDigest","originalEffectDigest","observedAuthorityState","observedAuthorityStateDigest","observedAt","budgetDelta","providerWriteDelta","signerId","signature"], context.verifier, "portable duplicate") as CertificationDuplicateDecisionV1; validateDuplicateBody(node); if (authorityDigest(node.observedAuthorityState) !== node.observedAuthorityStateDigest || duplicateIds.has(node.attemptRequestId) || !context.outcomes.some(item => item.requestId === node.originalRequestId && item.requestDigest === node.originalRequestDigest && item.effectDigest === node.originalEffectDigest)) throw new TypeError("portable duplicate is omitted, duplicated, altered, or not linked to the original effect"); duplicateIds.add(node.attemptRequestId); }
  if (input.duplicateAttempts.length !== input.duplicateDecisions.length || input.duplicateAttempts.some((raw: any) => !input.duplicateDecisions.some((decision: any) => decision.attemptRequestId === raw.attemptRequestId && decision.originalRequestId === raw.originalRequestId && decision.observedAt === raw.observedAt && decision.observedAuthorityStateDigest === raw.observedAuthorityStateDigest))) throw new TypeError("portable duplicate attempt/decision collection is incomplete");
}
...
function validateDuplicateBody(value: any): void { if (value.v !== "reelier.certification-duplicate-decision/v1" || typeof value.attemptRequestId !== "string" || typeof value.originalRequestId !== "string" || !value.observedAuthorityState || typeof value.observedAuthorityState !== "object" || !DIGEST.test(value.originalRequestDigest) || !DIGEST.test(value.originalEffectDigest) || !DIGEST.test(value.observedAuthorityStateDigest) || !validTime(value.observedAt) || value.budgetDelta !== 0 || value.providerWriteDelta !== 0 || typeof value.signerId !== "string") throw new TypeError("portable duplicate decision is invalid or has nonzero effect"); }
