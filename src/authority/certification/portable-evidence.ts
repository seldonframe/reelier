...
export interface CertificationEvidenceVerifier { readonly signerId: string; readonly publicKey: KeyObject; readonly purpose?: "authority-evidence" | "authority-journal" }
...
export interface CertificationDuplicateAttemptV1 { readonly v: "reelier.certification-duplicate-attempt/v1"; readonly attemptId: string; readonly attemptRequestId: string; readonly originalRequestId: string; readonly observedAuthorityStateDigest: string; readonly observedAt: string; readonly signerId: string; readonly signature: AuthoritySignature }
export interface CertificationDuplicateAttemptHeadV1 { readonly v: "reelier.certification-duplicate-attempt-head/v1"; readonly count: number; readonly historyDigest: string; readonly signerId: string; readonly signature: AuthoritySignature }

export function createCertificationDuplicateAttempt(input: Omit<CertificationDuplicateAttemptV1, "v" | "signerId" | "signature">, signer: CertificationEvidenceSigner): CertificationDuplicateAttemptV1 { const body = { v: "reelier.certification-duplicate-attempt/v1" as const, ...input, signerId: signer.signerId }; if (!input.attemptId || !input.attemptRequestId || !input.originalRequestId || !DIGEST.test(input.observedAuthorityStateDigest) || !validTime(input.observedAt)) throw new TypeError("portable duplicate attempt is invalid"); return Object.freeze({ ...body, signature: signer.sign(authorityDigest(body)) }); }
export function createCertificationDuplicateAttemptHead(attempts: readonly CertificationDuplicateAttemptV1[], signer: CertificationEvidenceSigner): CertificationDuplicateAttemptHeadV1 { const body = { v: "reelier.certification-duplicate-attempt-head/v1" as const, count: attempts.length, historyDigest: authorityDigest(attempts), signerId: signer.signerId }; return Object.freeze({ ...body, signature: signer.sign(authorityDigest(body)) }); }
...
export function createCertificationTaskAuthorityEvidence(input: Omit<CertificationTaskAuthorityEvidenceV1, "v" | "signedJobCardDigest" | "activationDigest" | "dispatchSnapshotDigest" | "adapterContractDigest" | "intentDigest" | "triggerDigest" | "signerId" | "signature">, signer: CertificationEvidenceSigner): CertificationTaskAuthorityEvidenceV1 {
  const body = { v: "reelier.certification-task-authority-evidence/v1" as const, ...input, signedJobCardDigest: signedJobCardDigest(normalizeSignedJobCard(input.signedJobCard)), activationDigest: authorityDigest(input.activation), dispatchSnapshotDigest: authorityDigest(input.dispatchSnapshotPreimage), adapterContractDigest: AUTHORITY_ADAPTER_CONTRACT_V1_DIGEST, intentDigest: authorityDigest(input.declaredIntent), triggerDigest: authorityDigest(input.declaredTrigger), signerId: signer.signerId };
...
export function createCertificationPolicyEvidence(input: Readonly<{ outcomeContract: Readonly<{ schemaId: string; jcsBase64: string; digest: string }>; localGatePolicyDigest: string; authorityStatePreimage: Readonly<Record<string, unknown>> }>, signer: CertificationEvidenceSigner): readonly CertificationPolicyEvidenceV1[] {
  const authorityStateDigest = authorityDigest(input.authorityStatePreimage);
  const outcome = signPolicy({ v: "reelier.certification-policy-evidence/v1", artifact: "outcome-contract", status: "verified", schemaId: input.outcomeContract.schemaId, jcsBase64: input.outcomeContract.jcsBase64, policyDigest: input.outcomeContract.digest, authorityStatePreimage: null, authorityStateDigest: null, signerId: signer.signerId }, signer);
  const local = signPolicy({ v: "reelier.certification-policy-evidence/v1", artifact: "local-gate-policy", status: "unchecked", schemaId: null, jcsBase64: null, policyDigest: input.localGatePolicyDigest, authorityStatePreimage: input.authorityStatePreimage, authorityStateDigest, signerId: signer.signerId }, signer);
...
export function createCertificationDuplicateDecision(input: Omit<CertificationDuplicateDecisionV1, "v" | "observedAuthorityStateDigest" | "budgetDelta" | "providerWriteDelta" | "signerId" | "signature">, signer: CertificationEvidenceSigner): CertificationDuplicateDecisionV1 {
  const body = { v: "reelier.certification-duplicate-decision/v1" as const, attemptId: input.attemptId, attemptRequestId: input.attemptRequestId, originalRequestId: input.originalRequestId, originalRequestDigest: input.originalRequestDigest, originalEffectDigest: input.originalEffectDigest, observedAuthorityState: input.observedAuthorityState, observedAuthorityStateDigest: authorityDigest(input.observedAuthorityState), observedAt: input.observedAt, budgetDelta: 0 as const, providerWriteDelta: 0 as const, signerId: signer.signerId };
  validateDuplicateBody(body);
  return Object.freeze({ ...body, signature: signer.sign(authorityDigest(body)) });
}

export function verifyCertificationPortableEvidence(input: Readonly<{ taskAuthorities: readonly unknown[]; postStateEvidence: readonly unknown[]; policyEvidence: readonly unknown[]; taskStatusEvidence: readonly unknown[]; duplicateAttemptHead: unknown; duplicateAttempts: readonly unknown[]; duplicateDecisions: readonly unknown[] }>, context: Readonly<{ verifier: CertificationEvidenceVerifier; journalVerifier: CertificationEvidenceVerifier; trustPin: JobCardTrustPinV1; taskId: string; authorityCellId: string; grants: readonly any[]; allocations: readonly any[]; receipts: readonly any[]; outcomes: readonly any[]; budgetEvents: readonly any[]; adapterContractDigest: string }>): void {
  ...
  const job = normalizeSignedJobCard(task.signedJobCard), trustedJob = jobCardTrustMaterialFromPin(job, context.trustPin), trustedJobKey = createPublicKey({ key: Buffer.from(trustedJob.signer.publicKeySpkiBase64, "base64"), format: "der", type: "spki" });
  ...
  const preimage = task.dispatchSnapshotPreimage as any, activation = task.activation as any;
  if (preimage.jobCard !== task.signedJobCardDigest || preimage.activation !== task.activationDigest || task.operatorConfigDigest !== preimage.operatorConfig || task.taskShapeDigest !== job.taskShapeDigest || task.instructionsDigest !== job.instructionsDigest || task.runnerDigest !== preimage.runner || task.planDigest !== preimage.plan || task.endpointDigest !== preimage.endpoint || task.trustHeadDigest !== preimage.trustHead || task.trustHeadDigest !== activation.currentTrustHeadDigest || task.principalSessionDigest !== authorityDigest({ principalId: activation.principalId, runtimeSessionId: activation.runtimeSessionId }) || task.grantAllocationDigest !== authorityDigest({ rootGrantDigest: activation.signedRootGrant.digest, childGrantDigest: activation.signedChildGrant.digest, rootAllocationId: activation.rootAllocationId, allocationId: activation.allocationId })) throw new TypeError("portable task authority links were substituted");
  ...
  for (const raw of input.postStateEvidence) { const post = signedExact(raw, ["v","requestId","dispatchRequestDigest","permitSnapshotDigest","expectedProjectionDigest","preSourceBundleDigest","projectionSchemaId","projectionSchemaDigest","preProjectionDigest","observedProjectionDigest","observationMethod","observedAt","confidence","signerId","signature"], context.verifier, "portable post-state") as CertificationPostStateEvidenceV1; validatePostStateBody(post); if (post.confidence !== "exact" && post.confidence !== "partial") throw new TypeError("portable post-state pending or absent evidence cannot pass"); if (postIds.has(post.requestId)) throw new TypeError("portable post-state evidence is duplicated"); postIds.add(post.requestId); if (!journalSnapshots.has(post.permitSnapshotDigest) || !context.outcomes.some(item => item.requestId === post.requestId && item.requestDigest === post.dispatchRequestDigest) || authorityDigest({ v: "reelier.outcome-request/v1", requestId: post.requestId, sourceRefs: task.declaredTrigger, choices: {} }) !== post.dispatchRequestDigest) throw new TypeError("portable post-state trigger is not linked to authorized dispatch"); const sourceBundle = context.receipts.find(bundle => bundle.sourceBundle?.digest === post.preSourceBundleDigest), source = sourceBundle?.sourceBundle, contract = sourceBundle?.contract?.value, sourceAuthority = contract?.sourceAuthority, policy = parseGitHubIssueLabelsPolicy(JSON.parse(Buffer.from(contract?.policyCommitment?.jcsBase64 ?? "", "base64").toString("utf8"))); if (post.preSourceBundleDigest !== null && (!source || authorityDigest(source.value?.projection?.labels) !== post.preProjectionDigest || sourceAuthority?.projectionSchemaId !== post.projectionSchemaId || !Array.isArray(sourceAuthority?.authorizedProjectionPointers) || !sourceAuthority.authorizedProjectionPointers.includes("/labels"))) throw new TypeError("portable post-state pre-read SourceBundle is omitted, substituted, or non-comparable"); if ((task.declaredIntent as any).definitionAlias !== contract?.alias || authorityDigest([...(task.declaredIntent as any).desiredLabels].sort()) !== authorityDigest([...policy.desiredLabels].sort()) || post.expectedProjectionDigest !== authorityDigest([...policy.desiredLabels].sort()) || post.projectionSchemaDigest !== authorityDigest({ schemaId: post.projectionSchemaId, pointers: ["/labels"] })) throw new TypeError("portable declared intent, expected projection, or schema is substituted"); }
  if (input.postStateEvidence.length === 0 || input.postStateEvidence.length !== context.receipts.filter(bundle => bundle.receipt?.value?.claims?.dispatch === "verified" && !String(bundle.receipt.value.decisionContext.requestId).endsWith(".cleanup")).map(bundle => bundle.receipt.value.decisionContext.requestId).filter((id, index, all) => all.indexOf(id) === index).length) throw new TypeError("portable post-state evidence cardinality is incomplete");

  if (input.policyEvidence.length !== 2) throw new TypeError("portable policy evidence is incomplete");
  const policies = input.policyEvidence.map(raw => { const item = signedExact(raw, ["v","artifact","status","schemaId","jcsBase64","policyDigest","authorityStatePreimage","authorityStateDigest","signerId","signature"], context.verifier, "portable policy") as CertificationPolicyEvidenceV1; validatePolicyBody(item); return item; });
  ...
  if (!contractCommitments.has(authorityDigest({ schemaId: policies[0]!.schemaId, jcsBase64: policies[0]!.jcsBase64, digest: policies[0]!.policyDigest })) || task.policyDigest !== policies[0]!.policyDigest || task.sourceDigest !== context.receipts[0]?.sourceBundle?.digest) throw new TypeError("portable policy or source commitment is not linked to the authorized receipt");
  const local = policies[1]!, authorityStateDigests = new Set(context.receipts.map(bundle => bundle.receipt?.value?.decisionContext?.snapshots?.authorityStateDigest));
  ...
  for (const status of input.taskStatusEvidence) { verifyCertificationTaskStatusEvidence(status, context.verifier); const s = status as any, allocation = context.allocations.find(item => item.parentAllocationId !== null), child = context.grants[1]?.grant, expectedRevoked = s.phase === "dispatch" ? false : allocation?.revoked; if (s.taskId !== context.taskId || s.grantExpiresAt !== child?.expiresAt || s.allocationRevoked !== expectedRevoked || s.currentActiveClaim !== (s.lifecycleState === "active" && !expectedRevoked && Date.parse(child.expiresAt) > Date.parse(s.observedAt))) throw new TypeError("portable task status is not tied to its signed graph authority observation"); }
  const rawBudgetEvents = context.budgetEvents.map(node => node.event);
  const statusHistory = (s: any, budget: any[]) => authorityDigest({ task: { taskId: context.taskId, authorityCellId: context.authorityCellId, lifecycleState: s.lifecycleState, grantExpiresAt: s.grantExpiresAt, allocationRevoked: s.allocationRevoked }, budgetEvents: budget });
  if ((input.taskStatusEvidence[0] as any).durableHistoryDigest !== statusHistory(input.taskStatusEvidence[0], rawBudgetEvents.slice(0, 2)) || (input.taskStatusEvidence[1] as any).durableHistoryDigest !== statusHistory(input.taskStatusEvidence[1], rawBudgetEvents)) throw new TypeError("portable task status durable task/budget history is substituted");

  const attemptIds = new Set<string>();
  for (const raw of input.duplicateAttempts) { const attempt = signedExact(raw, ["v","attemptId","attemptRequestId","originalRequestId","observedAuthorityStateDigest","observedAt","signerId","signature"], context.journalVerifier, "portable duplicate attempt") as any; if (attempt.v !== "reelier.certification-duplicate-attempt/v1" || attemptIds.has(attempt.attemptId) || !DIGEST.test(attempt.observedAuthorityStateDigest) || !validTime(attempt.observedAt)) throw new TypeError("portable duplicate attempts are invalid or duplicated"); attemptIds.add(attempt.attemptId); }
  const head = signedExact(input.duplicateAttemptHead, ["v","count","historyDigest","signerId","signature"], context.journalVerifier, "portable duplicate attempt head") as any;
  if (head.v !== "reelier.certification-duplicate-attempt-head/v1" || head.count !== input.duplicateAttempts.length || head.historyDigest !== authorityDigest(input.duplicateAttempts)) throw new TypeError("portable duplicate attempt head count or history is incomplete");
  const duplicateIds = new Set<string>();
  for (const raw of input.duplicateDecisions) { const node = signedExact(raw, ["v","attemptId","attemptRequestId","originalRequestId","originalRequestDigest","originalEffectDigest","observedAuthorityState","observedAuthorityStateDigest","observedAt","budgetDelta","providerWriteDelta","signerId","signature"], context.verifier, "portable duplicate") as CertificationDuplicateDecisionV1; validateDuplicateBody(node); if (authorityDigest(node.observedAuthorityState) !== node.observedAuthorityStateDigest || duplicateIds.has(node.attemptId) || !context.outcomes.some(item => item.requestId === node.originalRequestId && item.requestDigest === node.originalRequestDigest && item.effectDigest === node.originalEffectDigest)) throw new TypeError("portable duplicate is omitted, duplicated, altered, or not linked to the original effect"); duplicateIds.add(node.attemptId); }
  if (input.duplicateAttempts.length !== input.duplicateDecisions.length || input.duplicateAttempts.some((raw: any) => !input.duplicateDecisions.some((decision: any) => decision.attemptId === raw.attemptId && decision.attemptRequestId === raw.attemptRequestId && decision.originalRequestId === raw.originalRequestId && decision.observedAt === raw.observedAt && decision.observedAuthorityStateDigest === raw.observedAuthorityStateDigest))) throw new TypeError("portable duplicate attempt/decision collection is incomplete");
}
