...
export function verifyCertificationPortableEvidence(
  ...
  if (head.count > 0 && !terminalJournalHeads.includes(authorityDigest(head))) throw new TypeError("portable duplicate current checkpoint is absent from durable signed journal history; rollback detected");
