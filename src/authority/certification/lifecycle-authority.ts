...
export type CertificationLifecycleTrustContextV1=Readonly<{tenant:string;jobCardTrustPin:JobCardTrustPinV1;expectedAuthorityCellId:string;expectedTaskId:string;observedAt:Date}>;
const lifecycleTrustContexts=new WeakMap<object,CertificationLifecycleTrustContextV1>();
const lifecycleTrustRefreshSources=new WeakMap<object,()=>Promise<CertificationLifecycleTrustContextV1>>();
...
export function createCertificationArtifactKeyBinding(handle: CertificationLifecycleAuthorityHandle, input: Readonly<{ authorityCellId: string; taskId: string; readinessDigest: string; humanDescriptor: AuthorityKeyDescriptorV1; humanPrivateKey: KeyObject; issuedAt: string; expiresAt: string }>): Readonly<{ binding: CertificationArtifactKeyBindingV1; humanCommitment: CertificationArtifactKeyBindingCommitmentV1 }> {
  ...
  const humanCommitment=Object.freeze({ ...commitmentBody, signature: commitmentSignature });
  handles.set(handle as object, Object.freeze({ ...material, bindingDigest: authorityDigest(binding), artifactAuthorization:Object.freeze({binding:detached(binding),commitment:detached(humanCommitment)}) }));
  return Object.freeze({ binding, humanCommitment });
}
...
/** Cell-only refresh binding. The genuine material identity is the capability. Not barrel-exported. */
export function registerCertificationLifecycleTrustRefreshSource(material:CertificationLifecycleAuthorityMaterial,source:()=>Promise<CertificationLifecycleTrustContextV1>):void{const initial=certificationLifecycleTrustContext(material);if(typeof source!=="function"||lifecycleTrustRefreshSources.has(material as object))throw new TypeError("certification lifecycle trust refresh source is invalid or already registered");lifecycleTrustRefreshSources.set(material as object,async()=>{const current=await source();if(current.tenant!==initial.tenant||current.expectedAuthorityCellId!==initial.expectedAuthorityCellId||current.expectedTaskId!==initial.expectedTaskId)throw new TypeError("certification lifecycle trust refresh identity changed");return current;});}
export async function refreshCertificationLifecycleTrustContext(material:CertificationLifecycleAuthorityMaterial):Promise<CertificationLifecycleTrustContextV1>{const source=lifecycleTrustRefreshSources.get(material as object);if(!source)throw new TypeError("Cell-owned certification lifecycle trust refresh source required");const current=await source();registerCertificationLifecycleTrustContext(material,current);return certificationLifecycleTrustContext(material);}
