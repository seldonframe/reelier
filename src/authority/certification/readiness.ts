...
export interface CertificationReadinessCandidate {
  ...
  readonly commitments: Readonly<{
    ...
    endpoints: CertificationInputSet;
...
export function createCertificationReadinessCandidate(preflight: CertificationPreflightV2): Readonly<{ candidate: CertificationReadinessCandidate; digest: string }> {
  ...
  const candidate: CertificationReadinessCandidate = Object.freeze({
    ...
    commitments: Object.freeze({ resources: preflight.resources, cleanup: preflight.cleanup, credentials: preflight.credentialReferences, runners: preflight.inputs.runners, tests: preflight.inputs.tests, plans: preflight.inputs.plans, endpoints: preflight.inputs.endpoints, runnerRegistryDigest: preflight.runnerRegistryDigest, topology: preflight.topology, signatureStatus: "absent" }),
  });
...
export function parseCertificationReadinessCandidate(value: unknown, preflightValue: unknown): CertificationReadinessCandidate {
  ...
  closed(commitmentsRaw, ["resources", "cleanup", "credentials", "runners", "tests", "plans", "endpoints", "runnerRegistryDigest", "topology", "signatureStatus"], "readiness commitments");
  ...
  const candidate: CertificationReadinessCandidate = Object.freeze({
    ...
    commitments: Object.freeze({ resources: commitmentList(commitmentsRaw.resources, "resources"), cleanup: commitmentList(commitmentsRaw.cleanup, "cleanup"), credentials: credentialList(commitmentsRaw.credentials), runners: inputSet(commitmentsRaw.runners), tests: inputSet(commitmentsRaw.tests), plans: inputSet(commitmentsRaw.plans), endpoints: inputSet(commitmentsRaw.endpoints), runnerRegistryDigest: assertDigest(commitmentsRaw.runnerRegistryDigest, "runner registry digest"), topology: enumValue(commitmentsRaw.topology, ["configured", "absent"], "readiness topology"), signatureStatus: "absent" }),
  });
...
function stringList(value: unknown, label: string): readonly string[] { const result = inertArray(value, `certification ${label}`); if (result.some(item => typeof item !== "string")) throw new TypeError(`certification ${label} is invalid`); assertUniqueSorted(result as string[], label); return Object.freeze([...(result as string[])]); }
