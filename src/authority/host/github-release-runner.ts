...
async function safeRead<T>(read: () => Promise<unknown>, parse: (value: unknown) => T): Promise<T | null> { try { return parse(await read()); } catch (error) { if (error instanceof TypeError) throw error; return null; } }
