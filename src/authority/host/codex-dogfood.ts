...
export function createCodexDogfoodPlan(input: Readonly<{ taskId: string; endpoint: string }>): CodexDogfoodPlan {
  ...
  const profiles = CODEX_DOGFOOD_PROFILES.map((profile, index) => Object.freeze({ profile, principalId: `codex_${profile}`, runtimeSessionId: `${input.taskId}_session_${index}`, tokenEnv: `REELIER_CODEX_${profile.toUpperCase()}_TOKEN`, mcpEndpoint: input.endpoint, providerCredentials: "absent" as const }));
...
export function assertCodexDogfoodPlan(value: unknown): asserts value is CodexDogfoodPlan {
  ...
  for (const profile of plan.profiles) {
    if (!ID.test(profile.principalId) || !ID.test(profile.runtimeSessionId) || !/^REELIER_CODEX_[A-Z0-9_]+_TOKEN$/.test(profile.tokenEnv) || profile.providerCredentials !== "absent" || principals.has(profile.principalId) || sessions.has(profile.runtimeSessionId)) throw new TypeError("Codex dogfood profile identity is invalid");
