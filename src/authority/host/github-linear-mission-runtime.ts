...
function foldPlans(events: readonly import("./signed-journal.js").SignedJournalEventV1[]): RequestPlan | null { let prior: RequestPlan | null = null; const rank: Record<string, number> = { claimed: 0, pending: 1, reconciled: 2 }; for (const event of events) { const plan = event.phase === "mission-plan" && event.data.plan && typeof event.data.plan === "object" ? event.data.plan as RequestPlan : null; if (!plan || typeof plan.claimedAt !== "string" || !Number.isFinite(Date.parse(plan.claimedAt)) || new Date(Date.parse(plan.claimedAt)).toISOString() !== plan.claimedAt || plan.semanticsDigest !== event.semanticsDigest || authorityDigest({ requestId: plan.requestId }).slice(7) !== event.requestId.slice("request_".length) || !(plan.lifecycleState in rank) || prior && (plan.requestId !== prior.requestId || plan.missionId !== prior.missionId || plan.claimedAt !== prior.claimedAt || plan.mode !== prior.mode || plan.semanticsDigest !== prior.semanticsDigest || rank[plan.lifecycleState]! < rank[prior.lifecycleState]! || plan.joins.length < prior.joins.length || prior.joins.some((join, index) => authorityDigest(join) !== authorityDigest(plan.joins[index])))) throw new TypeError("signed mission lifecycle index conflicts or regresses"); prior = plan; } return prior; }
