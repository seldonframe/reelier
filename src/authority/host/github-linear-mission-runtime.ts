...
import { compileEffectTransportV1, type CompiledEffectTransportV1, type EffectTransportHostBindingsV1, type TrustedEffectTransportExecutorV1 } from "./effect-transports.js";
...
export interface GenuineGitHubLinearMissionRuntimeInputV1 {
  ...
  readonly linearExecutor: TrustedEffectTransportExecutorV1;
...
export async function createGitHubLinearMissionRuntimeV1(input: GenuineGitHubLinearMissionRuntimeInputV1 | LegacyGitHubLinearMissionRuntimeInputV1): Promise<Readonly<{ agentTools: AuthorityAgentToolsV1; reviewOutcomes(requestIds: readonly string[]): Promise<void>; inspectEvidence(): Promise<Readonly<{ activationConfirmations: number; routineApprovals: number; requests: readonly RequestPlan[]; reviews: readonly OutcomeReview[] }>> }>> {
  ...
  for (const alias of ["linear_evidence_comment_v1", "linear_status_transition_v1"] as const) {
    ...
    compiled.set(alias, compileEffectTransportV1({ contract: operation.contract, binding: operation.binding, modelInput, observationAuthKey: raw.observationAuthKey, resolveHostBindings: raw.resolveHostBindings, executor: raw.linearExecutor }));
  }
  ...
  const kernel = createOutcomeKernel({ storage, ledger: components.ledger, coordinator: components.coordinator, now: raw.now, authorization: async () => "active", predecessorPolicy });
  ...
  return Object.freeze({ agentTools: createAuthorityAgentTools(backend), async reviewOutcomes(requestIds) { if (!Array.isArray(requestIds) || requestIds.length < 1 || new Set(requestIds).size !== requestIds.length) throw new TypeError("Outcome review request IDs are invalid"); for (const id of requestIds) { const plan = await loadPlan(raw.journal, id); if (!plan || plan.lifecycleState !== "reconciled" || !plan.receiptRef) throw new TypeError("Outcome review requires reconciled durable receipts"); } reviews.push(Object.freeze({ reviewId: `review_${authorityDigest({ requestIds }).slice(7)}`, requestIds: Object.freeze([...requestIds]) })); }, async inspectEvidence() { const requests = (await raw.journal.listRequestIds()).filter(id => id.startsWith("request_")).map(async id => lastPlan(await raw.journal.load(id))).filter(Boolean); return Object.freeze({ activationConfirmations: 1, routineApprovals: 0, requests: Object.freeze((await Promise.all(requests)).filter((item): item is RequestPlan => Boolean(item))), reviews: Object.freeze([...reviews]) }); } });
}

function exactInput(value: unknown): GenuineGitHubLinearMissionRuntimeInputV1 { const keys = ["config", "profile", "githubReleaseRunner", "linearExecutor", "resolveHostBindings", "journal", "outcomeReceiptPublication", "localOptions", "observationAuthKey", "now"]; if (!value || typeof value !== "object" || Array.isArray(value) || isProxy(value) || Reflect.ownKeys(value).length !== keys.length || Reflect.ownKeys(value).some(key => typeof key !== "string" || !keys.includes(key))) throw new TypeError("legacy or raw governed mission runtime options are prohibited"); for (const key of keys) { const descriptor = Object.getOwnPropertyDescriptor(value, key); if (!descriptor?.enumerable || !Object.hasOwn(descriptor, "value")) throw new TypeError("genuine governed mission runtime options must be inert data properties"); } return value as GenuineGitHubLinearMissionRuntimeInputV1; }
...
async function loadPlan(journal: SignedJournal, requestId: string): Promise<RequestPlan | null> { return lastPlan(await journal.load(planKey(requestId))); }
function lastPlan(events: readonly import("./signed-journal.js").SignedJournalEventV1[]): RequestPlan | null { const event = events.at(-1); return event?.phase === "mission-plan" && event.data.plan && typeof event.data.plan === "object" ? event.data.plan as RequestPlan : null; }
