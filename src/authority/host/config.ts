...
export interface AuthorityHostConfig {
  ...
  readonly ingress?: { readonly bearerRef?: string; readonly allowedRequester?: string; readonly principalRegistryFile?: string; readonly stdioPrincipalCredentialRef?: `env:${string}` | `file:${string}` };
...
function validateIngress(value: unknown, baseDir: string): AuthorityHostConfig["ingress"] { if (!value || typeof value !== "object" || Array.isArray(value)) throw new TypeError("ingress must be an object"); const raw = value as Record<string, unknown>; const allowed = ["bearerRef", "allowedRequester", "principalRegistryFile", "stdioPrincipalCredentialRef"]; if (Object.keys(raw).some(key => !allowed.includes(key))) throw new TypeError("ingress is closed"); if (raw.bearerRef !== undefined && typeof raw.bearerRef !== "string") throw new TypeError("invalid ingress bearer reference"); if (raw.allowedRequester !== undefined && typeof raw.allowedRequester !== "string") throw new TypeError("invalid ingress requester"); if (raw.principalRegistryFile !== undefined && (typeof raw.principalRegistryFile !== "string" || !raw.principalRegistryFile)) throw new TypeError("invalid ingress principal registry"); if (raw.bearerRef && raw.principalRegistryFile) throw new TypeError("ingress bearer and principal registry are mutually exclusive"); const stdioPrincipalCredentialRef = raw.stdioPrincipalCredentialRef === undefined ? undefined : parseStdioPrincipalCredentialRef(raw.stdioPrincipalCredentialRef); if (stdioPrincipalCredentialRef && !raw.principalRegistryFile) throw new TypeError("stdio principal credential reference requires a principal registry"); return Object.freeze({ ...(raw.bearerRef ? { bearerRef: raw.bearerRef } : {}), ...(raw.allowedRequester ? { allowedRequester: raw.allowedRequester } : {}), ...(raw.principalRegistryFile ? { principalRegistryFile: path.resolve(baseDir, raw.principalRegistryFile as string) } : {}), ...(stdioPrincipalCredentialRef ? { stdioPrincipalCredentialRef } : {}) }); }
function parseStdioPrincipalCredentialRef(value: unknown): `env:${string}` | `file:${string}` { if (typeof value !== "string" || /[\0\r\n]/.test(value)) throw new TypeError("stdio principal credential reference is invalid"); if (/^env:[A-Za-z_][A-Za-z0-9_]{0,127}$/.test(value)) return value as `env:${string}`; const file = value.startsWith("file:") ? value.slice(5) : ""; if (file && (path.isAbsolute(file) || path.win32.isAbsolute(file)) && !file.split(/[\\/]/).includes("..")) return value as `file:${string}`; throw new TypeError("stdio principal credential reference is invalid"); }
