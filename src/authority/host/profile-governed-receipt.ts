...
export function createProfileGovernedAuthorityReceiptPublication(input:GovernedReceiptPublicationInputV1):DispatchPublication{
  ...
  return Object.freeze({
    ...
    async loadDurableHead(query:DurableDispatchPublicationQueryV1){assertExactDataRecord(query,["v","identity","ledgerState","sendStarted"],"durable governed receipt query");if(query.v!=="reelier.durable-dispatch-publication-query/v1"||!(["dispatched","ambiguous"] as const).includes(query.ledgerState)||query.sendStarted!==true)throw new TypeError("durable governed receipt query is invalid");assertDurableIdentity(query.identity,input.verification.expectedTenant);const chain=await load(query.identity);if(chain.length===0)return null;const head=chain.at(-1)!;return Object.freeze({v:"reelier.durable-dispatch-publication-head/v1",identity:head.identity,receiptRef:innerRef(head.receipt),evidenceDigest:head.receipt.authorityReceiptBundle.evidence.digest,reservationReceiptRef:head.reservationReceiptRef,priorReceiptRef:head.priorReceiptRef,phase:head.phase,terminalKind:head.terminalKind}) as DurableDispatchPublicationHeadV1;},
  });
}
