export type SupportedAuthorityHost = "codex" | "claude-code" | "cursor" | "eve" | "hermes" | "openclaw" | "herdr";
...
export function createAuthorityHostAdapters(command = "reelier authority serve"): readonly AuthorityHostAdapter[] {
  const hosts: readonly [SupportedAuthorityHost, string][] = [["codex", "~/.codex/config.toml"], ["claude-code", "~/.claude.json"], ["cursor", "~/.cursor/mcp.json"], ["eve", "connections/reelier.ts"], ["hermes", "~/.hermes/mcp.json"], ["openclaw", "~/.openclaw/mcp.json"], ["herdr", "~/.herdr/mcp.json"]];
