...
export interface CertifiedIdentityVerifier { readonly purpose: "authority-evidence"; readonly verify: (input: Readonly<{ identityDigest: string; identity: AuthenticatedProviderIdentityV1; purpose: "authority-evidence" }>) => Promise<boolean> | boolean; }
export interface CertifiedDispatchOptions { readonly identityProbe: () => Promise<AuthenticatedProviderIdentityV1>; readonly verifyIdentity?: CertifiedIdentityVerifier | ((identity: AuthenticatedProviderIdentityV1) => Promise<boolean> | boolean); readonly revalidator: DispatchAuthorityRevalidator; readonly onPhase?: (phase: "identity-probe" | "route-reread" | "authority-validation-before-prepare" | "prepare" | "authority-validation-after-prepare" | "dispatch-commit-cas" | "authority-send-boundary" | "send-started" | "send") => void; }
