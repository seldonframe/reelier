  ...
  return Object.freeze({
    async dispatch(handle: ReservedDispatchHandle): Promise<DispatchOutcome> {
      ...
      if (adapter.prepare && ledger.commitPreparedDispatch) {
        ...
        if (certified) { certified.onPhase?.("authority-validation-after-prepare"); const authorityAfter = await certified.revalidator.revalidate(state); if (!authorityBefore || authorityAfter.authorityGeneration !== authorityBefore.authorityGeneration || authorityAfter.routeAuthorityDigest !== authorityBefore.routeAuthorityDigest || description.authorityGeneration !== authorityAfter.authorityGeneration || authorityAfter.providerId !== undefined && authorityAfter.providerId !== routeAuthority!.providerId || authorityAfter.connectorId !== undefined && authorityAfter.connectorId !== routeAuthority!.connectorId || authorityAfter.accountId !== undefined && authorityAfter.accountId !== routeAuthority!.accountId || authorityAfter.endpointId !== undefined && authorityAfter.endpointId !== routeAuthority!.endpointId) throw new Error("dispatch authority changed during preparation"); }
