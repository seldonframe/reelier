...
export async function initializeAgentProject(options: InitializeAgentProjectOptions): Promise<BootstrapPreparationReport> {
  ...
  const governed = importedGovernance as { governanceRef: string | null; manifestDigest: string | null; trustHeadDigest: string | null };
  const project = { v: "reelier.agent-project/v1", agentName: options.agentName, projectId: `project_${options.agentName}`, tenant: governed.governanceRef === null ? null : "operator", reelierVersion: options.exactVersion, installedBuildDigest: await computeInstalledBuildDigest(process.cwd()), packageTarballIntegrityDigest: null, authorityContractDigest: AUTHORITY_ADAPTER_CONTRACT_V1_DIGEST, continuityContractDigest: digest({ contract: "continuity" }), outcomeProfileContractDigest: OUTCOME_PROFILE_CONTRACT_V1_DIGEST, bootstrapContractDigest: BOOTSTRAP_CONTRACT_V1_DIGEST, initializationReportDigest: digest(inspection.report), runtimeDescriptorDigest: reportFields.runtimeDescriptorDigest, routeCoverageDigest: reportFields.routeCoverageDigest, profileGovernanceRef: governed.governanceRef, profileGovernanceManifestDigest: governed.manifestDigest, profileTrustHeadDigest: governed.trustHeadDigest, authorityMode: governed.governanceRef === null ? "unconfigured" : "managed-cell" } as const;
