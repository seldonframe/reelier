...
function validateArtifact(id: InitCheckpointId, value: unknown): void {
  ...
  const expectedConnectionClass = (connection: Record<string, unknown>): PathCClassification => connection.status === "unsupported" ? "unsupported" : connection.status === "shadow-only" || connection.connectionKind === "host-private" ? "shadow-only" : connection.status === "usable" && connection.outcomeInvocation === "supported" ? "outcome-capable" : "shadow-only";
  const expectedCandidateClass = (candidate: Record<string, unknown>): PathCClassification => candidate.shadowStatus === "ready" ? "outcome-capable" : candidate.shadowStatus === "needs_human_definition" ? "shadow-only" : "unsupported";
  let valid = false;
