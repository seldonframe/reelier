...
test("declared-probe: pre+post captured, exact confidence, honest delta", async () => {
  ...
  const rec = await runSkill(parseSkill(SKILL(ATTEST)), { tools: w.tools, allowWrites: true, dryRun: true });
...
test("probe timeout degrades to absent + reason, write still executes", async () => {
  ...
  const rec = await runSkill(parseSkill(SKILL(ATTEST)), { tools: w.tools, allowWrites: true, dryRun: true, probeTimeoutMs: 50 });
  const a = rec.steps[0].attest!;
...
test("unknown probe tool / non-read probe tool degrade with reasons, never fail the step", async () => {
  ...
  const rec1 = await runSkill(parseSkill(SKILL(`- attest: {"tool":"nope.read","args":{}}`)), { tools: w1.tools, allowWrites: true, dryRun: true });
  ...
  const rec2 = await runSkill(parseSkill(SKILL(`- attest: {"tool":"fake.update","args":{}}`)), { tools: w2.tools, allowWrites: true, dryRun: true });
...
test("declared-probe: explicit projection over a boolean field round-trips into the hash", async () => {
  ...
  const rec = await runSkill(parseSkill(SKILL(attest)), { tools, allowWrites: true, dryRun: true });
  const a = rec.steps[0].attest!;
  assert.equal(a.confidence, "exact");
