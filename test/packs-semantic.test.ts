...
test("semantic outcome classes remain stable while unsupported provider packs stay explicit", () => {
  ...
  assert.equal(semanticOutcomeForAlias("github_release_candidate_publish_v1")?.semanticClass, "deployment_release_v1");
  assert.equal(semanticOutcomeForAlias("github_release_pr_ensure_v1")?.semanticClass, "deployment_release_v1");
  assert.equal(semanticOutcomeForAlias("github_release_pr_merge_v1")?.semanticClass, "deployment_release_v1");
  assert.equal(semanticOutcomeForAlias("github_release_tag_create_v1")?.semanticClass, "deployment_release_v1");
