...
test("approval hash unchanged for attest-less steps, changes when attest added", () => {
  ...
  assert.equal(computeApprovalHash({ emit: undefined, ...plain, attest: plain.attest, expect: plain.expect }), digestSha256({ args: plain.actionArgs, tool: plain.actionTool }));
  const withAttest = parseSkill(BASE(DECL)).steps[0];
  assert.notEqual(computeApprovalHash({ emit: undefined, ...withAttest, attest: withAttest.attest, expect: withAttest.expect }), computeApprovalHash({ emit: undefined, ...plain, attest: plain.attest, expect: plain.expect }));
});
