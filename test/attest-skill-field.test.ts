...
test("approval hash unchanged for attest-less steps, changes when attest added", () => {
  ...
  assert.equal(computeApprovalHash(plain), digestSha256({ args: plain.actionArgs, tool: plain.actionTool }));
  const withAttest = parseSkill(BASE(DECL)).steps[0];
  assert.notEqual(computeApprovalHash(withAttest), computeApprovalHash(plain));
});
