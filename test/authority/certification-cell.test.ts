...
async function fixture(scenarios: readonly ("github-issue-labels" | "slack-topic")[] = ["github-issue-labels"]) {
  ...
  const descriptors = [human, jobSigner, ...ceremony.publicDescriptors];
  const events: any[] = [];
  ...
  const signedReadiness = createSignedCertificationReadiness({ readinessCandidate: sealed.candidate, readinessCandidateDigest: sealed.digest, preflight, humanKeyDescriptor: human, cellKeyDescriptors: ceremony.publicDescriptors, jobCardKeyDescriptors: [jobSigner], trustEvents: events, humanPrivateKey: readinessKey.privateKey, authorizedAt: "2026-08-11T20:01:00.000Z" });
  ...
  const bound = createCertificationArtifactKeyBinding(ceremony.opaqueHandle, { authorityCellId: initialized.identifiers.authorityCellId, taskId: initialized.identifiers.taskId, readinessDigest: authorityDigest(signedReadiness), humanDescriptor: human, humanPrivateKey: readinessKey.privateKey, issuedAt: at, expiresAt: expiry });
  const host = await cell.createCertificationCellHost({ workspace: initialized.workspace, currentTrustPinPath, delegationAuthority: delegation, principalRegistry: principals, now: () => new Date("2026-08-11T20:10:00.000Z"), lifecycleAuthority: { handle: ceremony.opaqueHandle, binding: bound.binding, commitment: bound.humanCommitment } });
  return { root, initialized, pin, currentTrustPinPath, jobCard, constraints, delegation, principals, host };
}
...
test("signed Job Card activates exact durable root state and a derived restart-safe principal", async () => {
  ...
  try {
    ...
    assert.equal(first.grantId, `${f.initialized.identifiers.rootGrantId}_child`);
    assert.equal(first.allocationId, `${f.initialized.identifiers.rootGrantId}_child`);
    ...
    const restartedHost = await cell.createLegacyCertificationActivationHost({ workspace: f.initialized.workspace, currentTrustPinPath: f.currentTrustPinPath, delegationAuthority: f.delegation, principalRegistry: restarted, now: () => new Date("2026-08-11T20:11:00.000Z") });
...
test("Task 4A runner metadata cannot issue a dispatch permit and consumes no budget access", async () => {
  const f = await fixture();
  try {
    await f.host.activateRootTask({ jobCard: f.jobCard, jobCardTrustPin: f.pin, constraints: f.constraints, effects: 2, issuedAt: at, expiresAt: expiry });
    const credential = await f.host.activatePrincipalSession();
    ...
    assert.equal((await f.delegation.budget.get(`${f.initialized.identifiers.rootGrantId}_child`))?.remaining, 2);
...
test("Task 4A cannot issue a permit before later task revocation", async () => {
  const f = await fixture();
  try {
    await f.host.activateRootTask({ jobCard: f.jobCard, jobCardTrustPin: f.pin, constraints: f.constraints, effects: 1, issuedAt: at, expiresAt: expiry });
    const credential = await f.host.activatePrincipalSession();
...
test("host runtime owns the immutable trust path and activation only commits its digest", async () => {
  ...
  try {
    const host = f.host;
    const activationInput = { jobCard: f.jobCard, jobCardTrustPin: f.pin, constraints: f.constraints, effects: 2, issuedAt: at, expiresAt: expiry };
    for (const extra of [
