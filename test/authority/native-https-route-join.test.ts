...
test("certified dispatch enforces route reread, double authority validation, commit, and send ordering", async () => {
  ...
  try {
    ...
    const coordinator = createDispatchCoordinator(ledger, adapter, undefined, undefined, undefined, { latencyRecorder, identityProbe: async () => identityFor(durableRoute), verifyIdentity: verifier, revalidator: { async routeReread() { events.push("route-reread"); return durableRoute; }, async revalidate() { validationCount += 1; events.push(validationCount === 1 ? "authority-validation-before-prepare" : validationCount === 2 ? "authority-validation-after-prepare" : validationCount === 3 ? "authority-validation-before-cas" : "authority-validation-before-send"); return { authorityGeneration: durableRoute.authorityGeneration, authorityExpiresAt: durableRoute.authorityExpiresAt, routeAuthorityDigest: authorityDigest(durableRoute) }; } } });
    await coordinator.dispatch(createReservedDispatchHandle({ reservation, effect: {}, effectCanonicalBase64: "e30=", effectDigest: sha("3") }));
    assert.deepEqual(events, ["route-reread", "authority-validation-before-prepare", "prepare", "authority-validation-after-prepare", "authority-validation-before-cas", "dispatch-commit-cas", "authority-validation-before-send", "send-started", "send"]);
    assert.deepEqual(latencyRecorder.finish().phases.map(item => item.name), ["identity-probe", "route-reread", "authority-validation-before-prepare", "prepare", "authority-validation-after-prepare", "authority-validation-before-cas", "dispatch-commit-cas", "authority-validation-before-send", "authority-send-boundary", "terminal-transition"]);
  } finally { restore(); }
});
