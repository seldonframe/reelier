...
/** Every call site here relies on REELIER_RELEASE_BARREL to point at the test build
 * (dist-test/src/authority/index.js) instead of the real dist/. That override now requires the
 * explicit --unsafe-test-barrel flag (see the "REELIER_RELEASE_BARREL without --unsafe-test-barrel"
 * test below), so it is threaded through by default here rather than at each call site. */
function runSigner(space: Workspace, extra: string[] = []): SpawnSyncReturns<string> {
  return spawnSync(process.execPath, [
    ...
    "--out", space.outDir, "--repo", repoRoot, "--unsafe-test-barrel", ...extra,
