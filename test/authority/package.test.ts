...
test("public production export parses DecisionContext and its portable evidence against packaged schemas", async () => {
  ...
  assert.deepEqual(Object.keys(authority).sort(), [
    ...
    "decisionContextPresence", "deriveMandatedMissionV1", "deriveMandatedMissionV2", "digestAgentMandateV1", "digestAgentMandateV2", "digestAuthorityReceiptBundle", "digestGovernedOutcomeV1", "digestToolEffectContractV1", "normalizeSignedJobCard",
    "parseAgentDocumentV1", "parseAgentMandate", "parseAgentMandateV1", "parseAgentMandateV2", "parseAuthorityDeploymentSnapshot", "parseAuthorityKeyDescriptor", "parseAuthorityReceiptBundle", "parseAuthorityRouteScope", "parseAuthorityWire",
    ...
    "parseCustomerApprovalProofV1", "parseCustomerAuthorityPayloadV1", "parseGovernedOutcomeV1", "parseHostedAuthorityEnvelopeV1", "parseMandateLockV1", "parseMissionChildGrantV1", "parseOutcomeProfileDraft", "parsePortableAuthorityEvidence",
  ...
  assert.equal(normalizedDeclarations,[
    readFileSync(path.join(process.cwd(), "src", "authority", "index.ts"), "utf8").replace(/\s+/g," ").trim(),
