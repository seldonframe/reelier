...
test("native candidate creation is hermetic and verification is content addressed", () => {
  ...
  assert.deepEqual(verifyNativeCandidate(created.candidate, { tarballBytes: bytes, publicCommitSha: "03ac48e", task8BaselineDigest: digest("8"), portableEvidenceContractDigest: digest("e") }), { status: "verified", candidateDigest: created.digest });
});
...
test("candidate creation refuses missing acceptance and verifier refuses every binding mutation", () => {
  ...
  const expected = { tarballBytes: bytes, publicCommitSha: "03ac48e", task8BaselineDigest: digest("8"), portableEvidenceContractDigest: digest("e") };
...
test("non-JCS candidate bytes and stale or unknown provenance refuse", () => {
  ...
  assert.throws(() => verifyNativeCandidate(raw, { tarballBytes: bytes, publicCommitSha: "03ac48e", task8BaselineDigest: digest("8"), portableEvidenceContractDigest: digest("e") }), /canonical|JCS/i);
  assert.throws(() => verifyNativeCandidate({ ...candidate, laneCommits: [...candidate.laneCommits, { laneId: "unknown", commitSha: "d".repeat(40) }] }, { tarballBytes: bytes, publicCommitSha: "03ac48e", task8BaselineDigest: digest("8"), portableEvidenceContractDigest: digest("e") }), /lane|unknown|closed/i);
});
