...
test("signed five-definition composite stops at ambiguous merge then recreates and reconciles without resend", async () => {
  ...
  try {
    const pack = createGitHubLinearOutcomePackV1(reviewed), profile = createGovernedOutcomeCompositionProfileV1({ aliases: governedOutcomeCompositionAliasesV1, pack, operations: orderedGitHubLinearOperationsV1(pack, "github-linear") }), fixture = await fiveDefinitionDeployment(root, pack, release.authorizationHandle);
...
async function fiveDefinitionDeployment(root: string, pack: ReturnType<typeof createGitHubLinearOutcomePackV1>, authorizationHandle: string) {
  ...
  const localOptions: any = { jobCardTrustPin: pin, routeAuthority(input: any) { if (input.connectorId === "github") { const body = authorityCanonicalBytes({ authorizationHandle }), projection = { v: "reelier.materialized-http-request/v1" as const, method: "POST" as const, origin: "https://reelier.authority-cell.internal", normalizedPath: "/internal/github-release", normalizedQuery: "", reviewedHeaders: { "content-type": "application/json" }, bodyDigest: `sha256:${createHash("sha256").update(body).digest("hex")}` }; return { v: "reelier.route-authority-snapshot/v1", connectorRegistrationDigest: sha("1"), operatorConfigurationDigest: sha("2"), routeDigest: authorityDigest({ v: "reelier.github-release-internal-route/v1", alias: input.definitionAlias, endpointId: input.endpointId }), providerId: "github-release-runner", connectorId: "github", accountId: "account_github", providerAccountIdentity: "github-owner", endpointId: input.endpointId, credentialSlotId: "internal", slotInstanceId: "internal", slotVersion: "internal", authenticatedProviderIdentityDigest: sha("4"), sourceReadRouteDigest: sha("5"), projectionSchemaDigest: sha("6"), expectedMaterializedRequestDigest: materializedHttpRequestDigest(projection), authorityGeneration: input.authorityGeneration, authorityExpiresAt: input.authorityExpiresAt }; } const operation = input.definitionAlias === governedOutcomeCompositionAliasesV1[3] ? pack.operations.linearEvidenceComment : pack.operations.linearStatusTransition, model = input.definitionAlias === governedOutcomeCompositionAliasesV1[3] ? { evidenceUrl: reviewedAuthority().linear.evidenceUrl } : { requestId: "host-bound-status" }, host = { account: reviewedAuthority().linear.workspace, destination: reviewedAuthority().linear.issue, limit: pack.linearPolicyDigest }, projection = { v: "reelier.prepared-effect-projection/v1", transport: operation.binding.kind, operationDigest: authorityDigest(operation.binding), requestDigest: authorityDigest({ v: "reelier.governed-effect-transport-request/v1", contractDigest: authorityDigest(operation.contract), bindingDigest: authorityDigest(operation.binding), model, ...host }) }; return { v: "reelier.route-authority-snapshot/v1", connectorRegistrationDigest: sha("1"), operatorConfigurationDigest: sha("2"), routeDigest: sha("3"), providerId: "linear", connectorId: "linear", accountId: "account_linear", providerAccountIdentity: "workspace_01", endpointId: input.endpointId, credentialSlotId: "slot", slotInstanceId: "instance", slotVersion: "version", authenticatedProviderIdentityDigest: sha("4"), sourceReadRouteDigest: sha("5"), projectionSchemaDigest: sha("6"), expectedMaterializedRequestDigest: authorityDigest(projection), authorityGeneration: input.authorityGeneration, authorityExpiresAt: input.authorityExpiresAt }; }, authenticatedProviderIdentity: async () => ({}) };
