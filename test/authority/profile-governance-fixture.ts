...
export function profileGovernanceFixture(activationOverrides: Partial<Pick<SignedTenantProfileActivationV1, "jobCardDigest" | "contractDigest" | "deploymentDigest" | "routeScopeDigest" | "authorityTrustHeadDigest" | "validFrom" | "validUntil">> = {}) {
...
export async function writeProfileGovernanceFixture(homedir: string, activationOverrides: Parameters<typeof profileGovernanceFixture>[0] = {}) {
  const fixture = profileGovernanceFixture(activationOverrides);
...
export function governedReceiptSigningFixture(now = verificationTime) {
  ...
  const human: any = { v: "reelier.authority-key-descriptor/v1", keyId: "readiness_human", role: "human-sponsor", purpose: "certification-readiness", algorithm: "ed25519", publicKeySpkiBase64: spki(readinessKey.publicKey) };
...
export async function writeGovernedPublicFactoryFixture(home: string) {
  ...
  const grant = { v: "reelier.delegation-grant/v1", tenant, grantId: "grant_1", parentDigest: null, sponsor: "operator", grantor: "operator", grantee: "operator", issuedAt: signing.observedAt, expiresAt: signing.expiresAt, constraints: { definitionAliases: [githubIssueLabelsAlias], audiences: ["operator"], connectorAccounts: [{ connectorId: "github", accountId: "123" }], projectionPointers: ["/owner", "/repo", "/issueNumber", "/issueState", "/labels"], riskClasses: [githubIssueLabelsRiskClass], limits } };
  ...
  const jobCard = signJobCard({ v: "reelier.signed-job-card/v1", jobId: signing.identifiers.jobCardId, title: "Governed labels", taskShapeDigest: sha("a"), semanticClasses: ["record_state_set_v1"], definitionAliases: [githubIssueLabelsAlias], connectorIds: ["github"], accountIdentities: ["github:fixture"], connectionDescriptorDigests: [sha("2")], adoptionCommitmentDigests: [sha("3")], sourceRefs: ["issue"], audiences: ["operator"], limitsDigest: authorityDigest(limits), instructionsDigest: sha("4"), packDigests: [githubIssueLabelsPackDigest], exceptionPolicy: ["ambiguous-reconcile"], coverage: "declared-surface" }, signing.job.keyId, signing.jobKey.privateKey);
  ...
  const enforcement = { completeness: "unchecked", declaredSurfaceExclusiveEnforcement: "unchecked", bypasses: [] };
  ...
  const deployment = { v: "reelier.authority-deployment/v1", tenant, states: [state], connectors: [connector], trust, sourceDirectory: "sources", jobCard, jobCardAuthority, connectionDescriptors: [], connectionAdoptions: [], enforcement };
  ...
  const deploymentSnapshot = parseAuthorityDeploymentSnapshot({ v: "reelier.authority-deployment-snapshot/v1", tenant, jobCardDigest: signedJobCardDigest(jobCard), jobCardAuthorityDigest: authorityDigest(jobCardAuthority), authorityStateDigest: authorityDigest(state), connectorRegistryDigest: authorityDigest([connectorRegistrationDigest(registry, tenant, "github", "123")]), trustRootSetDigest: trustRootSetDigest(createTrustRoots(directRoots), tenant), connectionDescriptorsDigest: authorityDigest([]), connectionAdoptionsDigest: authorityDigest([]), enforcementDigest: authorityDigest(enforcement), routeScopeDigest: authorityDigest(routeScope) });
  ...
  const options: any = {
    ...
    dispatchAdapter: { async prepare(stateValue: any) { return createPreparedDispatch({ description: { v: "reelier.prepared-dispatch-description/v1", routeDigest: routeAuthority.routeDigest, materializedRequestDigest: expectedMaterializedRequestDigest, projection, authorityGeneration: routeAuthority.authorityGeneration, authorityExpiresAt: routeAuthority.authorityExpiresAt, absoluteDeadlineMs: performance.now() + 60_000, reservationId: stateValue.reservation.reservationId, allocationId: stateValue.reservation.intent.executionContext?.allocationId ?? "allocation_1" }, send: async () => ({ kind: "acknowledged", resultDigest: sha("f"), materializedRequestDigest: expectedMaterializedRequestDigest }) }); }, async dispatch() { throw new Error("legacy dispatch path must not run"); } },
    certifiedDispatch: { identityProbe: async () => providerIdentity, verifyIdentity: { purpose: "authority-evidence", signerId: "provider_identity", publicKey: identityKey.publicKey }, revalidator: { async revalidate() { return { authorityGeneration: routeAuthority.authorityGeneration, authorityExpiresAt: routeAuthority.authorityExpiresAt, routeAuthorityDigest: authorityDigest(routeAuthority), providerId: "github", connectorId: "github", accountId: "123", endpointId: githubIssueLabelsWriteEndpointId }; }, async routeReread() { return routeAuthority; } } },
  };
