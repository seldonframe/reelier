...
test("provisional predecessor denial polls only the bigint-and-PID-text immediate predecessor",()=>withRoot(async root=>{
  ...
  assert.equal(injected,true);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.deepEqual(observed!.delays,[5,10,5]);assert.deepEqual(events,Array.from({length:4},()=>["select","validate","validate","reparse","liveness"]).flat());assert.equal(stageValidations,8);assert.deepEqual(probes,[2,2,2,2],"ticket ties use canonical decimal PID text, never nonce or numeric PID ordering");assert.deepEqual({fullClosures,removeAttempts,publicationRenames,callbackEntries},{fullClosures:0,removeAttempts:0,publicationRenames:0,callbackEntries:0});assert.deepEqual((await readdir(root)).sort(),[path.basename(ownStage),...peerStages.map(stage=>path.basename(stage))].sort());assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);for(let index=0;index<peerStages.length;index++)assert.deepEqual(readFileSync(path.join(peerStages[index],"owner.json")),peerBytes[index]);
}));
...
test("provisional invalidation permanently falls through to staged settlement in the same iteration",async t=>{
  ...
  for(const kind of cases)await t.test(kind,()=>withRoot(async root=>{const external=await tempRoot();try{
    const predecessor={host:hostname(),nonce:"6".repeat(64),pid:31,v:1 as const,ticket:"0000000000000002"},distant={host:hostname(),nonce:"7".repeat(64),pid:32,v:1 as const,ticket:"0000000000000001"},predecessorStage=path.join(root,publicationStageName(predecessor)),distantStage=path.join(root,publicationStageName(distant)),livePids=new Set([predecessor.pid,distant.pid]),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),selection=0,provisionalRoot=0,provisionalLiveness=0,stagedBoundaries=0,delayCount=0,firstFallbackDelay=-1,removeAttempts=0,publicationRenames=0,callbackEntries=0,mutated=false;
    const replace=(target:string,bytes:Buffer,label:string)=>{const prepared=path.join(external,label);mkdirSync(prepared);writeFileSync(path.join(prepared,"owner.json"),bytes);renameSync(target,path.join(external,`${label}-old`));renameSync(prepared,target);};Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===predecessor.pid&&kind==="predecessor-dead"&&mutated)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===predecessor.pid&&kind==="predecessor-unverifiable"&&mutated)throw Object.assign(new Error("unverifiable"),{code:"EPERM"});return livePids.has(pid)?true:originalKill.call(process,pid,0);}});let result;
    try{result=await rawLedgerWithAdmissionClock(root,()=>3n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&ownStage===""){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),publicationOwnerBytes(predecessor));mkdirSync(distantStage);writeFileSync(path.join(distantStage,"owner.json"),publicationOwnerBytes(distant));}if(point==="after-lock-publication-provisional-predecessor-selection")selection++;if(point==="before-lock-publication-provisional-root-reenumeration"&&!mutated){provisionalRoot++;mutated=true;if(kind==="own-missing")rmSync(ownStage,{recursive:true});else if(kind==="own-replaced")replace(ownStage,ownBytes,"own");else if(kind==="predecessor-missing")rmSync(predecessorStage,{recursive:true});else if(kind==="predecessor-replaced")replace(predecessorStage,publicationOwnerBytes(predecessor),"pred");else if(kind==="membership-add"){const added={host:hostname(),nonce:"8".repeat(64),pid:33,v:1 as const,ticket:"0000000000000004"},stage=path.join(root,publicationStageName(added));mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),publicationOwnerBytes(added));livePids.add(added.pid);}else if(kind==="membership-remove")rmSync(distantStage,{recursive:true});else if(kind==="malformed-name")mkdirSync(path.join(root,".authority-ledger-lock-publication-malformed"));else if(kind==="foreign-provenance"){const foreign={host:"foreign.invalid",nonce:"9".repeat(64),pid:34,v:1 as const,ticket:"0000000000000001"},stage=path.join(root,publicationStageName(foreign));mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),publicationOwnerBytes(foreign));}else if(kind==="duplicate-provenance"){const duplicate={...predecessor,nonce:"a".repeat(64),ticket:"0000000000000001"},stage=path.join(root,publicationStageName(duplicate));mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),publicationOwnerBytes(duplicate));}}if(point==="before-lock-publication-provisional-predecessor-liveness"){provisionalLiveness++;if(kind==="predecessor-dead"||kind==="predecessor-unverifiable")mutated=true;}if(point==="before-staged-publication-settlement"){stagedBoundaries++;if(firstFallbackDelay<0)firstFallbackDelay=delayCount;assert.equal(removeAttempts,0);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);}if(point==="before-publication-stage-remove-attempt")removeAttempts++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}catch(error){result=error;}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(mutated,true,`${kind} reaches its intended provisional invalidation branch`);assert.equal(selection,1);assert.equal(stagedBoundaries>=1,true);assert.equal(firstFallbackDelay,0,"fallback occurs in the same iteration before acquisition backoff");assert.equal(selection,1,"provisional mode never re-enters after fallback");assert.ok(provisionalRoot>=0&&provisionalLiveness>=0);void result;
