...
test("an invalidated mixed generation removes dead MAX before ticket allocation",()=>withRoot(async root=>{
  ...
  let enumerations=0,samples=0,semanticClockReads=0,callbackEntries=0,creatorStage="",creatorCreates=0,publicationRenames=0;
  ...
  assert.deepEqual({result:observed!.result,enumerations,order,samples,semanticClockReads,callbackEntries,creatorCreates,publicationRenames,deadMaxPresent:existsSync(deadStage),temporaryPresent:existsSync(temporaryStage),livePresent:existsSync(liveStage),creatorTicketTwo},{result:{ok:false,reason:"busy"},enumerations:5,order:["add-temporary","remove-temporary","dead-remove","dead-root-sync","sample","creator-create"],samples:1,semanticClockReads:0,callbackEntries:0,creatorCreates:1,publicationRenames:0,deadMaxPresent:false,temporaryPresent:false,livePresent:true,creatorTicketTwo:true});
