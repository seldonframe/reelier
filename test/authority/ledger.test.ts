...
const publicationCleanupFaultPoints=["after-lock-publication-rename-collision","before-publication-stage-root-reenumeration","before-publication-stage-final-validation","before-publication-stage-final-liveness","before-publication-stage-remove-attempt"] as const;
...
test("a lower live predecessor blocks promotion without publication",()=>withRoot(async root=>{const predecessor={host:hostname(),nonce:"0".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),originalKill=process.kill;let ownStage="",publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);assert.ok(path.basename(predecessorStage)<ownName,"fixture predecessor is lexicographically lower");mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);}if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);assert.equal(existsSync(path.join(root,"lock")),false);assert.equal(existsSync(ownStage),true);assert.equal(existsSync(predecessorStage),true);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);}));

test("a new lower predecessor invalidates promotion and forces re-election",()=>withRoot(async root=>{const predecessor={host:hostname(),nonce:"1".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),originalKill=process.kill;let ownStage="",generationClosures=0,publicationRenames=0,callbackEntries=0,introduced=false;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);assert.ok(path.basename(predecessorStage)<ownName);}if(point==="after-lock-publication-generation-closed"){generationClosures++;if(!introduced){introduced=true;mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);}}if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(introduced,true);assert.ok(generationClosures>=2,"root membership change forces a new closed generation");assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);assert.equal(existsSync(path.join(root,"lock")),false);assert.equal(existsSync(ownStage),true);assert.equal(existsSync(predecessorStage),true);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);}));

test("a non-head contender polls only its exact predecessor between closed generations",()=>withRoot(async root=>{const peers:Array<{host:string;nonce:string;pid:number;v:1}>=[],peerStages:string[]=[],peerPids=new Set<number>(),originalKill=process.kill;let ownStage="",generationClosures=0,predecessorPolls=0,publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>peerPids.has(pid)?true:originalKill.call(process,pid,0)});let observed;try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:30,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);for(let pid=1;pid<process.pid&&peers.length<3;pid++){const owner={host:hostname(),nonce:String(peers.length+2).repeat(64),pid,v:1 as const},stage=path.join(root,publicationStageName(owner));if(path.basename(stage)>=ownName)continue;peers.push(owner);peerStages.push(stage);peerPids.add(pid);}assert.equal(peers.length,3,"fixture selects three distinct lexicographically lower owners");for(let index=0;index<peers.length;index++){mkdirSync(peerStages[index]);writeFileSync(path.join(peerStages[index],"owner.json"),authorityCanonicalBytes(peers[index]));}}if(point==="after-lock-publication-generation-closed")generationClosures++;if(point==="before-lock-publication-predecessor-validation")predecessorPolls++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.equal(generationClosures,1,"stable non-head waiting does not rescan all peers");assert.ok(predecessorPolls>1,"stable non-head waiting polls its exact predecessor");assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);assert.equal(existsSync(path.join(root,"lock")),false);assert.equal(existsSync(ownStage),true);for(let index=0;index<peerStages.length;index++){assert.equal(existsSync(peerStages[index]),true);assert.deepEqual(readFileSync(path.join(peerStages[index],"owner.json")),authorityCanonicalBytes(peers[index]));}}));
