...
test("an invalidated mixed generation removes dead MAX before ticket allocation",()=>withRoot(async root=>{
  const dead={host:hostname(),nonce:"b".repeat(64),pid:42001,v:1 as const,ticket:"ffffffffffffffff"},live={host:hostname(),nonce:"c".repeat(64),pid:42002,v:1 as const,ticket:"0000000000000001"},temporary={host:hostname(),nonce:"d".repeat(64),pid:42003,v:1 as const,ticket:"0000000000000003"};
  assert.equal(new Set([`${dead.host}:${dead.pid}`,`${live.host}:${live.pid}`,`${temporary.host}:${temporary.pid}`]).size,3,"fixture host+PID identities are distinct");
