...
import * as fsLedgerModule from "../../src/authority/host/fs-ledger.js";

const discoveredAdmissionClockOption=(fsLedgerModule as unknown as Record<PropertyKey,unknown>).__testAdmissionClockOption;
const admissionClockOption:symbol=typeof discoveredAdmissionClockOption==="symbol"?discoveredAdmissionClockOption:Symbol("__testAdmissionClockOption");
function rawLedgerWithAdmissionClock(root:string,clock:()=>unknown,options:Record<string,unknown>={}):RawFsAuthorityLedger{return new RawFsAuthorityLedger(root,{...options,[admissionClockOption]:clock} as never);}
...
test("terminal provisional busy withdraws the exact creator so a same-PID retry can progress",()=>withRoot(async root=>{
  ...
  const ledger=new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{
    if(point==="after-lock-publication-stage-sync"&&!injected){injected=true;const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));const ownOwner=JSON.parse(ownBytes.toString("utf8")) as {pid:number};assert.equal(ownOwner.pid,process.pid);mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);}
...
test("publication stages reject malformed topology and owner bindings without target mutation",async t=>{
  ...
  await t.test("wrong-host-digest",()=>withRoot(async root=>{const stage=path.join(root,`.authority-ledger-lock-publication-${"0".repeat(64)}-0000000000000001-${owner.pid}-${owner.nonce}.tmp`),ownerBytes=authorityCanonicalBytes(owner);await mkdir(stage);await writeFile(path.join(stage,"owner.json"),ownerBytes);assert.deepEqual(await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20}).recover(),{ok:false,reason:"corruption"});assert.deepEqual(await readFile(path.join(stage,"owner.json")),ownerBytes);}));
  await t.test("pid-name-mismatch",()=>withRoot(async root=>{const stage=path.join(root,`.authority-ledger-lock-publication-${publicationHostDigest(owner.host)}-0000000000000001-${owner.pid+1}-${owner.nonce}.tmp`),ownerBytes=authorityCanonicalBytes(owner);await mkdir(stage);await writeFile(path.join(stage,"owner.json"),ownerBytes);assert.deepEqual(await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20}).recover(),{ok:false,reason:"corruption"});assert.deepEqual(await readFile(path.join(stage,"owner.json")),ownerBytes);}));
...
test("lock and publication enumeration races restart the whole bounded snapshot",async t=>{
  ...
  await t.test("sustained canonical active-lock identity churn exhausts as busy",()=>withRoot(async root=>{const external=await tempRoot(),directory=path.join(root,"lock");try{const initial={host:hostname(),nonce:"a".repeat(64),pid:process.pid,v:1 as const};await mkdir(directory);await writeFile(path.join(directory,"owner.json"),authorityCanonicalBytes(initial));let snapshots=0,semanticClockCalls=0,callbackEntries=0,finalBytes=authorityCanonicalBytes(initial);const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point!=="after-active-lock-metadata")return;snapshots++;const owner={host:hostname(),nonce:(snapshots.toString(16)).repeat(64),pid:process.pid,v:1 as const},prepared=path.join(external,`replacement-${snapshots}`);mkdirSync(prepared);finalBytes=authorityCanonicalBytes(owner);writeFileSync(path.join(prepared,"owner.json"),finalBytes);renameSync(directory,path.join(external,`displaced-${snapshots}`));renameSync(prepared,directory);}} as never).recover());assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.equal(snapshots,6);assert.deepEqual(observed.delays,[5,10,20,40,25]);assert.equal(observed.elapsedMs,100);assert.equal(semanticClockCalls,0);assert.equal(callbackEntries,0);assert.deepEqual(await readdir(root),["lock"]);assert.deepEqual(await readFile(path.join(directory,"owner.json")),finalBytes);}finally{await rm(external,{recursive:true,force:true});}}));
  await t.test("publication generation replacement preserves a dead candidate and new live stage",()=>withRoot(async root=>{const external=await tempRoot(),[vanishedPid,deadPid]=await exitedConcurrentChildPids(),vanishedOwner={host:hostname(),nonce:"2".repeat(64),pid:vanishedPid,v:1 as const},deadOwner={host:hostname(),nonce:"3".repeat(64),pid:deadPid,v:1 as const},liveOwner={host:hostname(),nonce:"6".repeat(64),pid:process.pid,v:1 as const},vanishedStage=await writePublicationStage(root,vanishedOwner,authorityCanonicalBytes(vanishedOwner)),deadBytes=authorityCanonicalBytes(deadOwner),deadStage=await writePublicationStage(root,deadOwner,deadBytes),liveBytes=authorityCanonicalBytes(liveOwner),liveStage=path.join(root,publicationStageName(liveOwner)),preparedLiveStage=path.join(external,"live-stage");try{await mkdir(preparedLiveStage);await writeFile(path.join(preparedLiveStage,"owner.json"),liveBytes);let fired=false,callbackEntries=0,ownPublicationHooks=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:5,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if((publicationCrashPoints as readonly string[]).includes(point))ownPublicationHooks++;if(!fired&&point==="after-publication-stage-enumeration"){fired=true;renameSync(vanishedStage,path.join(external,"vanished-stage"));renameSync(preparedLiveStage,liveStage);}}} as never).recover());assert.equal(fired,true);assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.deepEqual(observed.delays,[5]);assert.equal(observed.elapsedMs,5);assert.equal(callbackEntries,0);assert.equal(ownPublicationHooks,0,"a new live stage prevents contender publication");assert.equal(existsSync(deadStage),true);assert.equal(existsSync(liveStage),true);assert.deepEqual(await readFile(path.join(deadStage,"owner.json")),deadBytes);assert.deepEqual(await readFile(path.join(liveStage,"owner.json")),liveBytes);}finally{await rm(external,{recursive:true,force:true});}}));
});

test("publication admission tickets allocate after the visible maximum without changing owner authority",async()=>{
  await withRoot(async root=>{
    let earlierPid=process.pid+1;while(String(earlierPid)<=String(process.pid))earlierPid++;const earlier={host:hostname(),nonce:"1".repeat(64),pid:earlierPid,v:1 as const,ticket:"00000000000000ff"},earlierBytes=authorityCanonicalBytes({host:earlier.host,nonce:earlier.nonce,pid:earlier.pid,v:earlier.v}),earlierStage=await writePublicationStage(root,earlier,earlierBytes),earlierIdentity=exactPublicationIdentity(earlierStage),oldCallerName=`.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-${process.pid}-${"0".repeat(64)}.tmp`,oldEarlierName=`.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-${earlier.pid}-${earlier.nonce}.tmp`,originalKill=process.kill;
    assert.ok(earlier.pid>process.pid);assert.ok(String(earlier.pid)>String(process.pid));assert.ok(oldCallerName<oldEarlierName,"the old PID-first grammar would overtake the earlier publisher");
    let admissionSamples=0,callbackEntries=0,publicationRenames=0,created="",createdBytes=Buffer.alloc(0),withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===earlier.pid?true:originalKill.call(process,pid,0)});
    try{
      const ledger=rawLedgerWithAdmissionClock(root,()=>{admissionSamples++;return 0xffn;},{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(name=>name!==path.basename(earlierStage)&&name.startsWith(".authority-ledger-lock-publication-"))??"";if(point==="after-lock-publication-stage-sync")createdBytes=readFileSync(path.join(root,created,"owner.json"));if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;}});
      assert.deepEqual(await ledger.observeClock(),{ok:false,reason:"busy"});
    }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(admissionSamples,1);assert.equal(callbackEntries,0);assert.equal(publicationRenames,0);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1});assert.match(created,new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-0000000000000100-${process.pid}-[0-9a-f]{64}\\.tmp$`));assert.ok(path.basename(earlierStage)<created);const ownOwner=JSON.parse(createdBytes.toString("utf8"));assert.deepEqual(Object.keys(ownOwner).sort(),["host","nonce","pid","v"]);assert.deepEqual(createdBytes,publicationOwnerBytes(ownOwner));assert.equal(existsSync(path.join(root,created)),false,"terminal denial withdraws the exact creator stage");assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(earlierStage)]);assert.deepEqual(exactPublicationIdentity(earlierStage),earlierIdentity);assert.deepEqual(await readFile(path.join(earlierStage,"owner.json")),earlierBytes);
  });
});

test("publication ticket validation is closed while an arbitrary valid ticket is owner-independent",async t=>{
  const owner={host:hostname(),nonce:"2".repeat(64),pid:2_147_483_647,v:1 as const},ownerBytes=authorityCanonicalBytes(owner);
  for(const ticket of ["0000000000000000","000000000000001","00000000000000000","000000000000000G","00000000000000FF"]){await t.test(ticket,()=>withRoot(async root=>{const stage=path.join(root,`.authority-ledger-lock-publication-${publicationHostDigest(owner.host)}-${ticket}-${owner.pid}-${owner.nonce}.tmp`);await mkdir(stage);await writeFile(path.join(stage,"owner.json"),ownerBytes);assert.deepEqual(await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20}).recover(),{ok:false,reason:"corruption"});assert.deepEqual(await readFile(path.join(stage,"owner.json")),ownerBytes);}));}
  await t.test("valid ticket",()=>withRoot(async root=>{const stage=await writePublicationStage(root,{...owner,ticket:"0123456789abcdef"},ownerBytes);assert.equal((await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20}).recover()).ok,true);assert.equal(existsSync(stage),false);}));
});

test("admission-clock seam allocates literal tickets and keeps the public string option inert",async t=>{
  assert.equal("__testAdmissionClockOption" in authorityModule,false,"the test-only admission clock is not a package-boundary export");
  const capture=async(clock:()=>unknown,expected:string)=>withRoot(async root=>{let stage="";const ledger=rawLedgerWithAdmissionClock(root,clock,{now:()=>t0,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create"){stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"))??"";throw new Error("captured-stage");}}});await assert.rejects(()=>ledger.recover(),/captured-stage/);assert.match(stage,new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-${expected}-${process.pid}-[0-9a-f]{64}\\.tmp$`));});
  await t.test("raw zero becomes ticket one",()=>capture(()=>0n,"0000000000000001"));
  await t.test("raw MAX remains 16 lowercase hex",()=>capture(()=>0xffffffffffffffffn,"ffffffffffffffff"));
  for(const [name,raw] of [["non-bigint",1],["negative",-1n],["overflow",0x10000000000000000n]] as const)await t.test(name,()=>withRoot(async root=>{let callbackEntries=0;const result=await rawLedgerWithAdmissionClock(root,()=>raw,{now:()=>t0,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;}}).recover();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackEntries,0);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[]);}));
  await t.test("default adapter reads hrtime",()=>withRoot(async root=>{const original=process.hrtime.bigint;let stage="";Object.defineProperty(process.hrtime,"bigint",{configurable:true,value:()=>0x42n});try{const ledger=new RawFsAuthorityLedger(root,{now:()=>t0,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create"){stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"))??"";throw new Error("captured-default");}}} as never);await assert.rejects(()=>ledger.recover(),/captured-default/);}finally{Object.defineProperty(process.hrtime,"bigint",{configurable:true,value:original});}assert.match(stage,/-0000000000000042-/);}));
  await t.test("string-keyed clock is ignored",()=>withRoot(async root=>{let calls=0;const ledger=new RawFsAuthorityLedger(root,{now:()=>t0,admissionClock:()=>{calls++;return 1n;}} as never);assert.equal((await ledger.recover()).ok,true);assert.equal(calls,0);}));
});

test("same-ticket distinct publishers elect by decimal PID text",()=>withRoot(async root=>{const ownText=String(process.pid),peerText=`${ownText}0`,peerPid=Number(peerText),peer={host:hostname(),nonce:"0".repeat(64),pid:peerPid,v:1 as const},peerBytes=publicationOwnerBytes(peer),originalKill=process.kill;assert.ok(Number.isSafeInteger(peer.pid)&&peer.pid>0,"fixture peer is a valid safe positive PID");assert.notEqual(peer.pid,process.pid);assert.ok(ownText<peerText,"canonical decimal PID text elects the caller");let ownStage="",peerStage="",publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===peer.pid?true:originalKill.call(process,pid,0)});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>9n,{now:()=>t0,lockTimeoutMs:50,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);const match=/^\.authority-ledger-lock-publication-[0-9a-f]{64}-([0-9a-f]{16})-\d+-([0-9a-f]{64})\.tmp$/.exec(ownName),ticket=match?.[1]??"0000000000000009";ownStage=path.join(root,ownName);peerStage=path.join(root,publicationStageName({...peer,ticket}));mkdirSync(peerStage);writeFileSync(path.join(peerStage,"owner.json"),peerBytes);if(match)assert.ok(ownName<path.basename(peerStage),"decimal PID text selects the caller as the actual elected head");}if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(publicationRenames,1);assert.equal(callbackEntries,1);assert.equal(existsSync(ownStage),false);assert.equal(existsSync(peerStage),true);assert.deepEqual(readFileSync(path.join(peerStage,"owner.json")),peerBytes);}));

test("publication admission allocates after one visible maximum regardless of publisher suffix order",()=>withRoot(async root=>{
  const ownPidText=String(process.pid),peerPidText=`${ownPidText}0`,peerPid=Number(peerPidText),peer={host:hostname(),nonce:"f".repeat(64),pid:peerPid,v:1 as const,ticket:"000000000000000a"};
  assert.ok(Number.isSafeInteger(peerPid)&&peerPid>0&&peerPid!==process.pid,"fixture peer is a distinct safe positive PID");
  assert.ok(ownPidText<peerPidText,"the caller's PID suffix would sort before the predecessor at an equal ticket");
  const peerBytes=publicationOwnerBytes(peer),peerStage=await writePublicationStage(root,peer,peerBytes),peerIdentity=exactPublicationIdentity(peerStage),originalKill=process.kill;
  let created="",createdBytes=Buffer.alloc(0),samples=0,semanticClockReads=0,callbackEntries=0,publicationRenames=0,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===peer.pid?true:originalKill.call(process,pid,0)});
  let result;
  try{result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 1n;},{now:()=>{semanticClockReads++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{
    if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")&&name!==path.basename(peerStage))??"";
    if(point==="after-lock-publication-stage-sync")createdBytes=readFileSync(path.join(root,created,"owner.json"));
    if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}
    if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;
    if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;
    if(point==="after-lock-publication-rename")publicationRenames++;
    if(point==="before-ledger-operation-callback")callbackEntries++;
  }}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.deepEqual(result,{ok:false,reason:"busy"});
  assert.equal(samples,1);
  assert.equal(semanticClockReads,0);
  assert.equal(callbackEntries,0);
  assert.equal(publicationRenames,0);
  assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1});
  assert.match(created,/-000000000000000b-/,"max(raw one, visible ticket ten plus one) allocates ticket eleven");
  assert.ok(path.basename(peerStage)<created,"ticket order wins even though the caller's PID suffix sorts first");
  assert.ok(createdBytes.length>0,"the exact creator bytes are captured before terminal withdrawal");
  assert.equal(existsSync(path.join(root,created)),false);
  assert.deepEqual(exactPublicationIdentity(peerStage),peerIdentity);
  assert.deepEqual(readFileSync(path.join(peerStage,"owner.json")),peerBytes);
  assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(peerStage)]);
}));

test("admission clock compares literal raw values with the exact visible maximum",async t=>{for(const [name,raw,expected] of [["below",2n,"0000000000000011"],["equal",0x10n,"0000000000000011"],["above",0x12n,"0000000000000012"]] as const)await t.test(name,()=>withRoot(async root=>{const peer={host:hostname(),nonce:"5".repeat(64),pid:7,v:1 as const,ticket:"0000000000000010"},peerBytes=publicationOwnerBytes(peer),peerStage=await writePublicationStage(root,peer,peerBytes),originalKill=process.kill;let created="",samples=0,semanticClockReads=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===peer.pid?true:originalKill.call(process,pid,0)});try{assert.deepEqual(await rawLedgerWithAdmissionClock(root,()=>{samples++;return raw;},{now:()=>{semanticClockReads++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(entry=>entry!==path.basename(peerStage)&&entry.startsWith(".authority-ledger-lock-publication-"))??"";if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock(),{ok:false,reason:"busy"});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(callbackEntries,0);assert.match(created,new RegExp(`-${expected}-${process.pid}-`));assert.deepEqual(readFileSync(path.join(peerStage,"owner.json")),peerBytes);}));});

test("visible MAX and invalid admission clocks refuse before publication progress",async t=>{await t.test("live visible MAX",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"6".repeat(64),pid:8,v:1 as const,ticket:"ffffffffffffffff"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),before=await snapshotRootArtifacts(root),originalKill=process.kill;let samples=0,semanticClockReads=0,stageHooks=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?true:originalKill.call(process,pid,0)});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 0n;},{now:()=>{semanticClockReads++;return t0;},faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))stageHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"});assert.equal(samples,0);assert.equal(semanticClockReads,0);assert.equal(stageHooks,0);assert.equal(callbackEntries,0);assert.deepEqual(await snapshotRootArtifacts(root),before);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);}));for(const [name,raw] of [["non-bigint",1],["negative",-1n],["overflow",0x10000000000000000n]] as const)await t.test(name,()=>withRoot(async root=>{let samples=0,semanticClockReads=0,stageHooks=0,callbackEntries=0;const result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return raw;},{now:()=>{semanticClockReads++;return t0;},faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))stageHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(stageHooks,0);assert.equal(callbackEntries,0);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[]);}));await t.test("throws",()=>withRoot(async root=>{let samples=0,semanticClockReads=0,stageHooks=0,callbackEntries=0;const result=await rawLedgerWithAdmissionClock(root,()=>{samples++;throw new Error("admission-clock-fault");},{now:()=>{semanticClockReads++;return t0;},faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))stageHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(stageHooks,0);assert.equal(callbackEntries,0);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[]);}));});

test("dead MAX is removed and root-synced before a fresh ticket is allocated",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"7".repeat(64),pid:await exitedChildPid(),v:1 as const,ticket:"ffffffffffffffff"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),order:string[]=[];let samples=0,created="",createdIdentity:ExactFsIdentity|undefined,withdrawalActive=false,withdrawalValidations=0,deadRemoveAttempts=0,deadRootSyncs=0,creatorRemoveAttempts=0,creatorRootSyncs=0;const ledger=rawLedgerWithAdmissionClock(root,()=>{samples++;order.push("sample");return 0n;},{now:()=>t0,faultInjector:(point:string)=>{if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(point==="before-publication-stage-remove-attempt"){if(withdrawalActive){creatorRemoveAttempts++;order.push("creator-remove");}else{deadRemoveAttempts++;assert.equal(existsSync(stage),true);order.push("dead-remove");}}if(point==="after-publication-stage-cleanup-root-sync"){if(withdrawalActive){creatorRootSyncs++;order.push("creator-root-sync");}else{deadRootSyncs++;order.push("dead-root-sync");}}if(point==="after-lock-publication-stage-create"){order.push("create");created=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"))??"";createdIdentity=exactFsIdentity(path.join(root,created));throw new Error("captured-dead-max-successor");}}});await assert.rejects(()=>ledger.observeClock(),/captured-dead-max-successor/);assert.equal(samples,1);assert.deepEqual(order,["dead-remove","dead-root-sync","sample","create","creator-remove","creator-root-sync"]);assert.deepEqual({withdrawalValidations,deadRemoveAttempts,deadRootSyncs,creatorRemoveAttempts,creatorRootSyncs},{withdrawalValidations:1,deadRemoveAttempts:1,deadRootSyncs:1,creatorRemoveAttempts:1,creatorRootSyncs:1});assert.ok(createdIdentity,"the exact created stage identity is captured before the sentinel fault");assert.equal(existsSync(stage),false);assert.match(created,/-0000000000000001-/);assert.equal(existsSync(path.join(root,created)),false);}));

test("an invalidated mixed generation removes dead MAX before ticket allocation",()=>withRoot(async root=>{
  const deadPid=process.pid*10+1,livePid=process.pid*10+2,temporaryPid=process.pid*10+3;assert.equal([deadPid,livePid,temporaryPid].every(pid=>Number.isSafeInteger(pid)&&pid>0),true,"derived peer PIDs are safe positive integers");
  const dead={host:hostname(),nonce:"b".repeat(64),pid:deadPid,v:1 as const,ticket:"ffffffffffffffff"},live={host:hostname(),nonce:"c".repeat(64),pid:livePid,v:1 as const,ticket:"0000000000000001"},temporary={host:hostname(),nonce:"d".repeat(64),pid:temporaryPid,v:1 as const,ticket:"0000000000000003"};
  assert.equal(new Set([`${dead.host}:${dead.pid}`,`${live.host}:${live.pid}`,`${temporary.host}:${temporary.pid}`,`${hostname()}:${process.pid}`]).size,4,"dead, live, temporary, and caller host+PID identities are distinct");
  const deadBytes=publicationOwnerBytes(dead),liveBytes=publicationOwnerBytes(live),temporaryBytes=publicationOwnerBytes(temporary),deadStage=await writePublicationStage(root,dead,deadBytes),liveStage=await writePublicationStage(root,live,liveBytes),liveIdentity=exactPublicationIdentity(liveStage),temporaryStage=path.join(root,publicationStageName(temporary)),originalKill=process.kill,order:string[]=[];
  let enumerations=0,samples=0,semanticClockReads=0,callbackEntries=0,creatorStage="",creatorBytes=Buffer.alloc(0),creatorCreates=0,creatorSynced=false,provisionalSelections=0,stagedBoundariesAfterCreator=0,publicationRenames=0,withdrawalActive=false,withdrawalValidations=0,deadRemoveAttempts=0,deadRootSyncs=0,creatorRemoveAttempts=0,creatorRootSyncs=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===dead.pid)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===live.pid||pid===temporary.pid)return true;return originalKill.call(process,pid,0);}});
  let observed;
  try{
    observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>{samples++;order.push("sample");return 0n;},{now:()=>{semanticClockReads++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{
      if(point==="after-publication-stage-enumeration"){
        enumerations++;
        if(enumerations===1){mkdirSync(temporaryStage);writeFileSync(path.join(temporaryStage,"owner.json"),temporaryBytes);order.push("add-temporary");}
        else if(enumerations===2){assert.equal(existsSync(temporaryStage),true);rmSync(temporaryStage,{recursive:true});order.push("remove-temporary");}
      }
      if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}
      if(point==="before-publication-stage-remove-attempt"){if(withdrawalActive){creatorRemoveAttempts++;assert.equal(existsSync(path.join(root,creatorStage)),true);order.push("creator-remove");}else{deadRemoveAttempts++;assert.equal(existsSync(deadStage),true);order.push("dead-remove");}}
      if(point==="after-publication-stage-cleanup-root-sync"){if(withdrawalActive){creatorRootSyncs++;order.push("creator-root-sync");}else{deadRootSyncs++;order.push("dead-root-sync");}}
      if(point==="after-lock-publication-stage-create"){creatorCreates++;creatorStage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")&&name!==path.basename(deadStage)&&name!==path.basename(liveStage))??"";order.push("creator-create");}
      if(point==="after-lock-publication-stage-sync"&&creatorCreates>0){creatorSynced=true;creatorBytes=readFileSync(path.join(root,creatorStage,"owner.json"));}
      if(point==="after-lock-publication-provisional-predecessor-selection"&&creatorSynced)provisionalSelections++;
      if(point==="before-staged-publication-settlement"&&creatorSynced)stagedBoundariesAfterCreator++;
      if(point==="after-lock-publication-rename")publicationRenames++;
      if(point==="before-ledger-operation-callback")callbackEntries++;
    }}).observeClock());
  }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  const creatorTicketTwo=new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-0000000000000002-${process.pid}-[0-9a-f]{64}\\.tmp$`).test(creatorStage);
  assert.deepEqual({result:observed!.result,enumerations,order,samples,semanticClockReads,callbackEntries,creatorCreates,stagedBoundariesAfterCreator,publicationRenames,deadMaxPresent:existsSync(deadStage),temporaryPresent:existsSync(temporaryStage),livePresent:existsSync(liveStage),creatorTicketTwo},{result:{ok:false,reason:"busy"},enumerations:4,order:["add-temporary","remove-temporary","dead-remove","dead-root-sync","sample","creator-create","creator-remove","creator-root-sync"],samples:1,semanticClockReads:0,callbackEntries:0,creatorCreates:1,stagedBoundariesAfterCreator:0,publicationRenames:0,deadMaxPresent:false,temporaryPresent:false,livePresent:true,creatorTicketTwo:true});
  assert.ok(provisionalSelections>=1,"the post-create lower-live cohort remains in denial-only provisional waiting");
  assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(callbackEntries,0);assert.equal(creatorCreates,1);assert.equal(publicationRenames,0);assert.deepEqual({withdrawalValidations,deadRemoveAttempts,deadRootSyncs,creatorRemoveAttempts,creatorRootSyncs},{withdrawalValidations:1,deadRemoveAttempts:1,deadRootSyncs:1,creatorRemoveAttempts:1,creatorRootSyncs:1});
  assert.equal(existsSync(deadStage),false);assert.equal(existsSync(temporaryStage),false);assert.equal(existsSync(path.join(root,creatorStage)),false);assert.equal(existsSync(liveStage),true);assert.deepEqual(exactPublicationIdentity(liveStage),liveIdentity);assert.deepEqual(await readFile(path.join(liveStage,"owner.json")),liveBytes);
  assert.match(creatorStage,new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-0000000000000002-${process.pid}-[0-9a-f]{64}\\.tmp$`));
  const creatorOwner=JSON.parse(creatorBytes.toString("utf8")) as {host:string;nonce:string;pid:number;v:1};
  assert.deepEqual(creatorBytes,publicationOwnerBytes(creatorOwner));assert.deepEqual(Object.keys(creatorOwner).sort(),["host","nonce","pid","v"]);assert.equal(creatorOwner.host,hostname());assert.equal(creatorOwner.pid,process.pid);
  assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(liveStage)]);
}));

test("malformed and unverifiable stages refuse before admission allocation",async t=>{await t.test("malformed",()=>withRoot(async root=>{const stage=path.join(root,".authority-ledger-lock-publication-malformed.tmp"),sentinel=Buffer.from("malformed-publication-sentinel");await mkdir(stage);await writeFile(path.join(stage,"owner.json"),sentinel);let samples=0,semanticClockReads=0,stageHooks=0,callbackEntries=0;const result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 0n;},{now:()=>{semanticClockReads++;return t0;},faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))stageHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(samples,0);assert.equal(semanticClockReads,0);assert.equal(stageHooks,0);assert.equal(callbackEntries,0);assert.deepEqual(await readFile(path.join(stage,"owner.json")),sentinel);}));await t.test("unverifiable",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"8".repeat(64),pid:12,v:1 as const,ticket:"0000000000000002"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),originalKill=process.kill;let probes=0,samples=0,semanticClockReads=0,stageHooks=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===owner.pid){probes++;throw Object.assign(new Error("unverifiable"),{code:"EPERM"});}return originalKill.call(process,pid,0);}});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 0n;},{now:()=>{semanticClockReads++;return t0;},faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))stageHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(probes,1);assert.equal(samples,0);assert.equal(semanticClockReads,0);assert.equal(stageHooks,0);assert.equal(callbackEntries,0);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);}));});

test("rename collision retains the exact ticket and samples admission once",()=>withRoot(async root=>{const blocker={host:hostname(),nonce:"9".repeat(64),pid:process.pid,v:1 as const},blockerBytes=publicationOwnerBytes(blocker);let samples=0,creates=0,collisions=0,callbackEntries=0,stageName="";const result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 0x1234n;},{now:()=>t0,lockTimeoutMs:200,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create"){creates++;stageName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"))??"";}if(point==="after-lock-publication-stage-sync"){mkdirSync(path.join(root,"lock"));writeFileSync(path.join(root,"lock","owner.json"),blockerBytes);}if(point==="after-lock-publication-rename-collision"){collisions++;assert.equal(readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")),stageName);rmSync(path.join(root,"lock"),{recursive:true});}if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();assert.deepEqual(result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(samples,1);assert.equal(creates,1);assert.equal(collisions,1);assert.equal(callbackEntries,1);assert.match(stageName,/-0000000000001234-/);}));

test("creator ticket replacement never publishes or deletes the replacement",()=>withRoot(async root=>{const external=await tempRoot();let ownStage="",replacementStage="",replacementBytes=Buffer.alloc(0),renames=0,callbackEntries=0;try{const result=await rawLedgerWithAdmissionClock(root,()=>5n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);replacementBytes=readFileSync(path.join(ownStage,"owner.json"));const owner=JSON.parse(replacementBytes.toString("utf8")) as {host:string;nonce:string;pid:number;v:1};replacementStage=path.join(root,publicationStageName({...owner,ticket:"0000000000000006"}));renameSync(ownStage,path.join(external,"original-creator"));mkdirSync(replacementStage);writeFileSync(path.join(replacementStage,"owner.json"),replacementBytes);}if(point==="after-lock-publication-rename")renames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(renames,0);assert.equal(callbackEntries,0);assert.equal(existsSync(replacementStage),true);assert.deepEqual(readFileSync(path.join(replacementStage,"owner.json")),replacementBytes);assert.equal(existsSync(path.join(root,"lock")),false);}finally{await rm(external,{recursive:true,force:true});}}));

test("identical canonical owner bytes are valid under arbitrary tickets in separate generations",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"a".repeat(64),pid:await exitedChildPid(),v:1 as const},bytes=publicationOwnerBytes(owner);for(const ticket of ["0000000000000001","fedcba9876543210"]){const stage=await writePublicationStage(root,{...owner,ticket},bytes);assert.equal((await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200}).recover()).ok,true);assert.equal(existsSync(stage),false);assert.deepEqual(bytes,publicationOwnerBytes(owner));}}));

test("persistent publication snapshot sharing violations exhaust the monotonic deadline as busy without mutation",{timeout:5_000},async t=>{
  for(const code of ["EPERM","EACCES","EBUSY"] as const)for(const boundary of ["before-active-lock-content-read","before-publication-stage-validation"] as const)await t.test(`${boundary}:${code}`,()=>withRoot(async root=>{const owner={host:hostname(),nonce:(boundary==="before-active-lock-content-read"?"3":"4").repeat(64),pid:process.pid,v:1 as const},artifact=boundary==="before-active-lock-content-read"?path.join(root,"lock"):await writePublicationStage(root,owner,authorityCanonicalBytes(owner));if(boundary==="before-active-lock-content-read"){await mkdir(artifact);await writeFile(path.join(artifact,"owner.json"),authorityCanonicalBytes(owner));}const ownerPath=path.join(artifact,"owner.json"),before=await readFile(ownerPath),rootBefore=await snapshotRootArtifacts(root),activeLock=boundary==="before-active-lock-content-read";let attempts=0,semanticClockCalls=0,callbackEntries=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point===boundary){attempts++;throw Object.assign(new Error(code),{code});}}} as never).recover());assert.equal(attempts,activeLock?6:21,`${code} takes the bounded ${activeLock?"acquisition snapshot":"generation-settle"} attempts`);assert.deepEqual(observed.delays,activeLock?[5,10,20,40,25]:Array(20).fill(5));assert.equal(semanticClockCalls,0);assert.equal(callbackEntries,0);assert.equal(observed.elapsedMs,100);assert.equal(observed.physicalElapsedMs<5_000,true);assert.deepEqual(await snapshotRootArtifacts(root),rootBefore);assert.deepEqual(await readFile(ownerPath),before);assert.deepEqual(observed.result,{ok:false,reason:"busy"});}));
});

test("acquisition publication-membership churn exhausts as busy without entering the ledger callback",()=>withRoot(async root=>{const first={host:hostname(),nonce:"b".repeat(64),pid:42421,v:1 as const},second={host:hostname(),nonce:"c".repeat(64),pid:42422,v:1 as const},firstStage=path.join(root,publicationStageName(first)),secondStage=path.join(root,publicationStageName(second)),originalKill=process.kill;let current=first,currentStage=firstStage,generations=0,callbackEntries=0,semanticClockCalls=0;await mkdir(firstStage);await writeFile(path.join(firstStage,"owner.json"),authorityCanonicalBytes(first));Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===first.pid||pid===second.pid?true:originalKill.call(process,pid,0)});try{const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point!=="before-publication-stage-root-reenumeration")return;generations++;const next=current===first?second:first,nextStage=currentStage===firstStage?secondStage:firstStage;rmSync(currentStage,{recursive:true});mkdirSync(nextStage);writeFileSync(path.join(nextStage,"owner.json"),authorityCanonicalBytes(next));current=next;currentStage=nextStage;}} as never).recover());assert.equal(generations,21);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.equal(callbackEntries,0);assert.equal(semanticClockCalls,0);assert.deepEqual(await snapshotRootArtifacts(root),[{name:path.basename(currentStage),type:"directory"},{name:path.posix.join(path.basename(currentStage),"owner.json"),type:"file",bytes:authorityCanonicalBytes(current).toString("base64")}]);assert.deepEqual(observed.result,{ok:false,reason:"busy"});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}}));

test("pre-publication same-name foreign stage identity churn remains corruption",()=>withRoot(async root=>{const external=await tempRoot(),owner={host:hostname(),nonce:"9".repeat(64),pid:process.pid,v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes);let attempts=0,callbackEntries=0,publicationHooks=0,semanticClockCalls=0,finalIno=lstatSync(stage,{bigint:true}).ino;try{const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if((publicationCrashPoints as readonly string[]).includes(point))publicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;if(point!=="after-lock-publication-generation-closed")return;attempts++;const replacement=path.join(external,`replacement-${attempts}`);mkdirSync(replacement);writeFileSync(path.join(replacement,"owner.json"),bytes);const previousIno=lstatSync(stage,{bigint:true}).ino;renameSync(stage,path.join(external,`displaced-${attempts}`));renameSync(replacement,stage);finalIno=lstatSync(stage,{bigint:true}).ino;assert.notEqual(finalIno,previousIno,"fixture atomically replaces the foreign stage identity while preserving canonical bytes");}} as never).recover());assert.equal(attempts,21);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.equal(callbackEntries,0);assert.equal(publicationHooks,0,"same-name integrity replacement never reaches contender publication");assert.equal(semanticClockCalls,0);assert.deepEqual(await snapshotRootArtifacts(root),[{name:path.basename(stage),type:"directory"},{name:path.posix.join(path.basename(stage),"owner.json"),type:"file",bytes:bytes.toString("base64")}]);assert.equal(lstatSync(stage,{bigint:true}).ino,finalIno);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);assert.deepEqual(observed.result,{ok:false,reason:"corruption"});}finally{await rm(external,{recursive:true,force:true});}}));

test("post-publication housekeeping churn exhausts as busy and retires the caller lock",()=>withRoot(async root=>{const first={host:hostname(),nonce:"d".repeat(64),pid:42423,v:1 as const},second={host:hostname(),nonce:"e".repeat(64),pid:42424,v:1 as const},firstStage=path.join(root,publicationStageName(first)),secondStage=path.join(root,publicationStageName(second)),originalKill=process.kill;let injected=false,current=first,currentStage=firstStage,generations=0,callbackEntries=0,semanticClockCalls=0,callerBytes=Buffer.alloc(0);let callerOwner:{host:string;nonce:string;pid:number;v:1}|undefined;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===first.pid||pid===second.pid?true:originalKill.call(process,pid,0)});try{const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-lock-publication-root-sync"&&!injected){injected=true;callerBytes=readFileSync(path.join(root,"lock","owner.json"));callerOwner=JSON.parse(callerBytes.toString("utf8"));assert.deepEqual(callerBytes,authorityCanonicalBytes(callerOwner));mkdirSync(firstStage);writeFileSync(path.join(firstStage,"owner.json"),authorityCanonicalBytes(first));}if(!injected||point!=="before-publication-stage-root-reenumeration")return;generations++;const next=current===first?second:first,nextStage=currentStage===firstStage?secondStage:firstStage;rmSync(currentStage,{recursive:true});mkdirSync(nextStage);writeFileSync(path.join(nextStage,"owner.json"),authorityCanonicalBytes(next));current=next;currentStage=nextStage;}} as never).observeClock());assert.equal(injected,true);assert.ok(callerOwner);assert.equal(generations,21);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.equal(callbackEntries,0);assert.equal(semanticClockCalls,0);const released=retirementMarkerName(callerOwner,"released"),foreignName=path.basename(currentStage);assert.deepEqual(await snapshotRootArtifacts(root),[{name:released,type:"directory"},{name:path.posix.join(released,"owner.json"),type:"file",bytes:callerBytes.toString("base64")},{name:foreignName,type:"directory"},{name:path.posix.join(foreignName,"owner.json"),type:"file",bytes:authorityCanonicalBytes(current).toString("base64")},{name:"claims",type:"directory"},{name:"ingress",type:"directory"},{name:"journal",type:"directory"},{name:"tombstones",type:"directory"},{name:"transactions",type:"directory"}]);assert.deepEqual(observed.result,{ok:false,reason:"busy"});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}}));
...
test("a live publication generation is closed before callback entry",async t=>{for(const mutation of ["membership","identity","type","bytes"] as const)await t.test(mutation,()=>withRoot(async root=>{const external=await tempRoot(),livePid=55555,owner={host:hostname(),nonce:"7".repeat(64),pid:livePid,v:1 as const},bytes=authorityCanonicalBytes(owner),stage=path.join(root,publicationStageName(owner)),membershipOwner={...owner,nonce:"6".repeat(64)},membershipBytes=authorityCanonicalBytes(membershipOwner),membershipStage=path.join(root,publicationStageName(membershipOwner)),replacementBytes=Buffer.from("mutated-live-owner"),originalKill=process.kill;let callbackEntries=0,postPublicationEnumerations=0,mutated=false,injected=false,originalStageIno=0n,replacementIno=0n;try{if(mutation==="identity"){const prepared=path.join(external,"replacement-live-stage");mkdirSync(prepared);writeFileSync(path.join(prepared,"owner.json"),bytes);}Object.defineProperty(process,"kill",{configurable:true,value:(observedPid:number)=>{if(observedPid!==livePid)return originalKill.call(process,observedPid,0);if(!mutated){mutated=true;originalStageIno=lstatSync(stage,{bigint:true}).ino;if(mutation==="membership"){rmSync(stage,{recursive:true});mkdirSync(membershipStage);writeFileSync(path.join(membershipStage,"owner.json"),membershipBytes);replacementIno=lstatSync(membershipStage,{bigint:true}).ino;}else if(mutation==="identity"){rmSync(stage,{recursive:true});renameSync(path.join(external,"replacement-live-stage"),stage);replacementIno=lstatSync(stage,{bigint:true}).ino;assert.notEqual(replacementIno,originalStageIno,"fixture replaces the live stage inode with canonical-same bytes");}else if(mutation==="type"){rmSync(stage,{recursive:true});writeFileSync(stage,"replacement-live-stage-file");}else writeFileSync(path.join(stage,"owner.json"),replacementBytes);}return true;}});let result;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-root-sync"&&!injected){injected=true;mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),bytes);}if(injected&&point==="after-publication-stage-enumeration")postPublicationEnumerations++;if(point==="before-ledger-operation-callback"){if(mutation==="membership"){assert.equal(mutated,true,"membership invalidation precedes callback entry");assert.ok(postPublicationEnumerations>=2,"replacement generation is fully re-enumerated before callback entry");}callbackEntries++;}}} as never).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(mutated,true);if(mutation==="membership"){assert.deepEqual(result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(callbackEntries,1);assert.ok(postPublicationEnumerations>=2,"membership withdrawal forces complete re-enumeration before callback");assert.equal(existsSync(stage),false);assert.equal(existsSync(membershipStage),true);assert.equal(lstatSync(membershipStage,{bigint:true}).ino,replacementIno);assert.deepEqual(readFileSync(path.join(membershipStage,"owner.json")),membershipBytes);}else{assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackEntries,0);assert.equal(existsSync(stage),true);if(mutation==="identity"){assert.notEqual(replacementIno,originalStageIno);assert.equal(lstatSync(stage,{bigint:true}).ino,replacementIno);assert.deepEqual(readFileSync(path.join(stage,"owner.json")),bytes);}else if(mutation==="type")assert.deepEqual(readFileSync(stage),Buffer.from("replacement-live-stage-file"));else assert.deepEqual(readFileSync(path.join(stage,"owner.json")),replacementBytes);}}finally{await rm(external,{recursive:true,force:true});}}));});
test("active owner re-enumerates an exact contender withdrawal before callback",()=>withRoot(async root=>{const livePid=55556,owner={host:hostname(),nonce:"8".repeat(64),pid:livePid,v:1 as const},bytes=authorityCanonicalBytes(owner),stage=path.join(root,publicationStageName(owner)),ownerPath=path.join(stage,"owner.json"),originalKill=process.kill;let injected=false,withdrawn=false,postPublicationEnumerations=0,callbackEntries=0,directoryIdentity:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>|undefined,ownerIdentity:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>|undefined;const identity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>):Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink});Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===livePid?true:originalKill.call(process,pid,0)});let result;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-lock-publication-root-sync"&&!injected){injected=true;mkdirSync(stage);writeFileSync(ownerPath,bytes);const directory=lstatSync(stage,{bigint:true}),ownerFile=lstatSync(ownerPath,{bigint:true});assert.equal(directory.isDirectory(),true);assert.equal(ownerFile.isFile(),true);directoryIdentity=identity(directory);ownerIdentity=identity(ownerFile);}if(injected&&point==="after-publication-stage-enumeration")postPublicationEnumerations++;if(injected&&!withdrawn&&point==="before-publication-stage-root-reenumeration"){assert.deepEqual(identity(lstatSync(stage,{bigint:true})),directoryIdentity);assert.deepEqual(identity(lstatSync(ownerPath,{bigint:true})),ownerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);rmSync(stage,{recursive:true});withdrawn=true;}if(point==="before-ledger-operation-callback"){assert.equal(withdrawn,true,"exact withdrawal precedes callback entry");assert.ok(postPublicationEnumerations>=2,"stable post-withdrawal generation is fully re-enumerated before callback entry");callbackEntries++;}}} as never).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(injected,true);assert.equal(withdrawn,true);assert.deepEqual(result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(callbackEntries,1);assert.ok(postPublicationEnumerations>=2,"withdrawal invalidates the first snapshot and forces full-root re-enumeration");assert.equal(existsSync(stage),false);const coordinationArtifacts=(await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")||name.includes("queue")||name.includes("handoff"));assert.deepEqual(coordinationArtifacts,[],"withdrawal leaves no publication, queue, or handoff artifact");}));
...
function publicationPeersRelativeTo(ownName:string,direction:"lower"|"higher",count:number):Array<{host:string;nonce:string;pid:number;v:1;ticket:string}>{const match=/^\.authority-ledger-lock-publication-[0-9a-f]{64}-([0-9a-f]{16})-\d+-[0-9a-f]{64}\.tmp$/.exec(ownName);assert.ok(match);const ownTicket=BigInt(`0x${match[1]}`),pids:number[]=[];if(direction==="lower")for(let pid=1;pid<process.pid&&pids.length<count;pid++)pids.push(pid);else for(let suffix=1;suffix<=count;suffix++){const pid=Number(`${process.pid}${suffix}`);assert.ok(Number.isSafeInteger(pid));pids.push(pid);}assert.equal(pids.length,count,`fixture selects ${count} distinct ${direction} ticket peers`);return pids.map((pid,index)=>{const ticket=direction==="lower"?ownTicket-BigInt(count-index):ownTicket+BigInt(index+1);assert.ok(ticket>0n&&ticket<=0xffffffffffffffffn);const owner={host:hostname(),nonce:(index+2).toString(16).repeat(64),pid,v:1 as const} as {host:string;nonce:string;pid:number;v:1;ticket:string};Object.defineProperty(owner,"ticket",{enumerable:false,value:ticket.toString(16).padStart(16,"0")});assert.deepEqual(Object.keys(owner).sort(),["host","nonce","pid","v"]);return owner;});}

test("a lower live predecessor blocks promotion without publication",()=>withRoot(async root=>{
  const predecessor={host:hostname(),nonce:"0".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),predecessorIdentity:ReturnType<typeof exactPublicationIdentity>|undefined,publicationRenames=0,callbackEntries=0,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;
  try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));assert.ok(path.basename(predecessorStage)<ownName,"fixture predecessor is lexicographically lower");mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);predecessorIdentity=exactPublicationIdentity(predecessorStage);}if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(predecessorStage)],"busy election preserves only the exact peer");assert.ok(ownBytes.length>0);assert.deepEqual(exactPublicationIdentity(predecessorStage),predecessorIdentity);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);
}));

test("a new lower predecessor invalidates promotion and forces re-election",()=>withRoot(async root=>{
  const predecessor={host:hostname(),nonce:"1".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),predecessorIdentity:ReturnType<typeof exactPublicationIdentity>|undefined,generationClosures=0,publicationRenames=0,callbackEntries=0,introduced=false,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;
  try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));assert.ok(path.basename(predecessorStage)<ownName);}if(point==="after-lock-publication-generation-closed"){generationClosures++;if(!introduced){introduced=true;mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);predecessorIdentity=exactPublicationIdentity(predecessorStage);}}if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(introduced,true);assert.ok(generationClosures>=2,"root membership change forces a new closed generation");assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(predecessorStage)],"re-election preserves only the exact peer");assert.ok(ownBytes.length>0);assert.deepEqual(exactPublicationIdentity(predecessorStage),predecessorIdentity);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);
}));

test("a non-head contender re-elects when its exact predecessor identity changes",()=>withRoot(async root=>{const external=await tempRoot(),peers:Array<{host:string;nonce:string;pid:number;v:1}>=[],peerStages:string[]=[],peerIdentities:Array<ReturnType<typeof exactPublicationIdentity>>=[],peerPids=new Set<number>(),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),predecessorStage="",predecessorBytes=Buffer.alloc(0),higherStage="",higherBytes:Buffer=Buffer.alloc(0),higherIdentity:ReturnType<typeof exactPublicationIdentity>|undefined,originalPredecessorIno=0n,replacementPredecessorIno=0n,provisionalSelections=0,provisionalInvalidations=0,stagedBoundaries=0,generationClosures=0,predecessorPolls=0,publicationRenames=0,callbackEntries=0,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;let observed;try{Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>peerPids.has(pid)?true:originalKill.call(process,pid,0)});observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));peers.push(...publicationPeersRelativeTo(ownName,"lower",3));for(const owner of peers){const stage=path.join(root,publicationStageName(owner));peerStages.push(stage);peerPids.add(owner.pid);mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),authorityCanonicalBytes(owner));peerIdentities.push(exactPublicationIdentity(stage));}const higher=publicationPeersRelativeTo(ownName,"higher",1)[0];higherStage=path.join(root,publicationStageName(higher));higherBytes=authorityCanonicalBytes(higher);peerPids.add(higher.pid);predecessorStage=[...peerStages].sort().at(-1)!;predecessorBytes=readFileSync(path.join(predecessorStage,"owner.json"));const prepared=path.join(external,"replacement-predecessor");mkdirSync(prepared);writeFileSync(path.join(prepared,"owner.json"),predecessorBytes);}if(point==="after-lock-publication-provisional-predecessor-selection")provisionalSelections++;if(point==="before-lock-publication-provisional-root-reenumeration"&&provisionalInvalidations===0){mkdirSync(higherStage);writeFileSync(path.join(higherStage,"owner.json"),higherBytes);higherIdentity=exactPublicationIdentity(higherStage);provisionalInvalidations++;}if(point==="before-staged-publication-settlement")stagedBoundaries++;if(point==="after-lock-publication-generation-closed")generationClosures++;if(point==="before-lock-publication-predecessor-validation"){predecessorPolls++;if(predecessorPolls===1){originalPredecessorIno=lstatSync(predecessorStage,{bigint:true}).ino;renameSync(predecessorStage,path.join(external,"original-predecessor"));renameSync(path.join(external,"replacement-predecessor"),predecessorStage);replacementPredecessorIno=lstatSync(predecessorStage,{bigint:true}).ino;assert.notEqual(replacementPredecessorIno,originalPredecessorIno);}}if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.ok(observed.elapsedMs<5_000);assert.deepEqual({provisionalSelections,provisionalInvalidations,stagedBoundaries},{provisionalSelections:1,provisionalInvalidations:1,stagedBoundaries:1});assert.equal(generationClosures,2,"predecessor replacement forces exactly one new full generation closure");assert.ok(predecessorPolls>1);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.notEqual(replacementPredecessorIno,originalPredecessorIno);assert.equal(lstatSync(predecessorStage,{bigint:true}).ino,replacementPredecessorIno);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),[...peerStages.map(stage=>path.basename(stage)),path.basename(higherStage)].sort(),"optimized waiting preserves exact peers only");assert.ok(ownBytes.length>0);for(let index=0;index<peerStages.length;index++){if(peerStages[index]!==predecessorStage)assert.deepEqual(exactPublicationIdentity(peerStages[index]),peerIdentities[index]);assert.deepEqual(readFileSync(path.join(peerStages[index],"owner.json")),authorityCanonicalBytes(peers[index]));}assert.deepEqual(exactPublicationIdentity(higherStage),higherIdentity);assert.deepEqual(readFileSync(path.join(higherStage,"owner.json")),higherBytes);}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});await rm(external,{recursive:true,force:true});}}));

test("stable non-head predecessor polling clamps deterministic backoff to the monotonic acquisition budget",{timeout:2_000},()=>withRoot(async root=>{const timeoutMs=100,predecessor={host:hostname(),nonce:"5".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),livePids=new Set<number>([predecessor.pid]),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),predecessorIdentity:ReturnType<typeof exactPublicationIdentity>|undefined,higherStage="",higherBytes:Buffer=Buffer.alloc(0),higherIdentity:ReturnType<typeof exactPublicationIdentity>|undefined,provisionalSelections=0,provisionalInvalidations=0,stagedBoundaries=0,predecessorPolls=0,publicationRenames=0,callbackEntries=0,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>livePids.has(pid)?true:originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:timeoutMs,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);predecessorIdentity=exactPublicationIdentity(predecessorStage);const higher=publicationPeersRelativeTo(ownName,"higher",1)[0];higherStage=path.join(root,publicationStageName(higher));higherBytes=authorityCanonicalBytes(higher);livePids.add(higher.pid);}if(point==="after-lock-publication-provisional-predecessor-selection")provisionalSelections++;if(point==="before-lock-publication-provisional-root-reenumeration"&&provisionalInvalidations===0){mkdirSync(higherStage);writeFileSync(path.join(higherStage,"owner.json"),higherBytes);higherIdentity=exactPublicationIdentity(higherStage);provisionalInvalidations++;}if(point==="before-staged-publication-settlement")stagedBoundaries++;if(point==="before-lock-publication-predecessor-validation")predecessorPolls++;if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.deepEqual({provisionalSelections,provisionalInvalidations,stagedBoundaries},{provisionalSelections:1,provisionalInvalidations:1,stagedBoundaries:1});assert.deepEqual(observed!.delays,[5,10,20,40,25],"the final request is exactly the positive monotonic acquisition budget remaining");assert.equal(observed!.delays.reduce((sum,value)=>sum+value,0),timeoutMs,"requested sleeps exactly consume but never widen the configured acquisition timeout");assert.ok(observed!.delays.every(value=>Number.isFinite(value)&&value>0),"every sleep requests positive remaining acquisition time");assert.equal(observed!.elapsedMs,timeoutMs,"virtual monotonic elapsed time stops exactly at the acquisition deadline");assert.ok(observed!.physicalElapsedMs<1_000,`physical fixture time remains bounded independently of virtual acquisition time (${observed!.physicalElapsedMs}ms)`);assert.equal(predecessorPolls,observed!.delays.length);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),[path.basename(predecessorStage),path.basename(higherStage)].sort());assert.ok(ownBytes.length>0);assert.deepEqual(exactPublicationIdentity(predecessorStage),predecessorIdentity);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.deepEqual(exactPublicationIdentity(higherStage),higherIdentity);assert.deepEqual(readFileSync(path.join(higherStage,"owner.json")),higherBytes);}));

test("provisional fallback resets capped backoff only after its first full settlement",()=>withRoot(async root=>{
  const blocker={host:hostname(),nonce:"7".repeat(64),pid:process.pid,v:1 as const},blockerBytes=authorityCanonicalBytes(blocker),lock=path.join(root,"lock"),originalKill=process.kill,livePids=new Set<number>();
  let predecessorStage="",provisionalSelections=0,stagedBoundaries=0,generationClosures=0,blockerCreated=false,blockerRemoved=false,collisions=0,publicationRenames=0,callbackEntries=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>livePids.has(pid)?true:originalKill.call(process,pid,0)});
  let observed;
  try{
    observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:300,faultInjector:(point:string)=>{
      if(point==="after-lock-publication-stage-sync"&&predecessorStage===""){
        const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);
        const predecessor=publicationPeersRelativeTo(ownName,"lower",1)[0];livePids.add(predecessor.pid);predecessorStage=path.join(root,publicationStageName(predecessor));mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),authorityCanonicalBytes(predecessor));
      }
      if(point==="after-lock-publication-provisional-predecessor-selection"){
        provisionalSelections++;
        if(provisionalSelections===6){rmSync(predecessorStage,{recursive:true});livePids.clear();}
      }
      if(point==="before-staged-publication-settlement")stagedBoundaries++;
      if(point==="after-lock-publication-generation-closed"){
        generationClosures++;
        if(stagedBoundaries===1&&!blockerCreated){mkdirSync(lock);writeFileSync(path.join(lock,"owner.json"),blockerBytes);blockerCreated=true;}
      }
      if(point==="after-lock-publication-rename-collision")collisions++;
      if(point==="after-lock-publication-rename")publicationRenames++;
      if(point==="before-ledger-operation-callback")callbackEntries++;
    }} as never).observeClock(),()=>{
      if(blockerCreated&&!blockerRemoved&&collisions===1){rmSync(lock,{recursive:true});blockerRemoved=true;}
    });
  }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.deepEqual(observed!.delays.slice(0,6),[5,10,20,40,50,5],"the first collision wait after fallback settlement restarts at 5ms");
  assert.deepEqual({provisionalSelections,stagedBoundaries,blockerCreated,blockerRemoved,collisions,publicationRenames,callbackEntries},{provisionalSelections:6,stagedBoundaries:1,blockerCreated:true,blockerRemoved:true,collisions:1,publicationRenames:1,callbackEntries:1});
  assert.ok(generationClosures>=2,"fallback and post-collision retry each close a full generation");
  assert.deepEqual(observed!.result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});
  assert.equal(existsSync(lock),false);assert.equal(existsSync(predecessorStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[]);
}));
...
test("an interposed active lock retains capped delay until final-name re-election completes",{timeout:2_000},()=>withRoot(async root=>{const blocker={host:hostname(),nonce:"e".repeat(64),pid:3,v:1 as const},interposedOwner={host:hostname(),nonce:"f".repeat(64),pid:4,v:1 as const},blockerBytes=authorityCanonicalBytes(blocker),interposedBytes=authorityCanonicalBytes(interposedOwner),lock=path.join(root,"lock"),originalKill=process.kill,livePids=new Set<number>([interposedOwner.pid]);let ownStage="",peerStage="",peerBytes:Buffer=Buffer.alloc(0),collisions=0,closures=0,mutationArmed=false,membershipInstalled=false,activeObserved=false,interposedCleared=false,finalBlockerCreated=false,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>livePids.has(pid)?(pid===interposedOwner.pid&&(activeObserved=true),true):originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:1_000,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);const peer=publicationPeersRelativeTo(ownName,"higher",1)[0];livePids.add(peer.pid);peerStage=path.join(root,publicationStageName(peer));peerBytes=authorityCanonicalBytes(peer);}if(point==="after-lock-publication-generation-closed"){closures++;if(collisions<5){mkdirSync(lock);writeFileSync(path.join(lock,"owner.json"),blockerBytes);}else if(!mutationArmed)mutationArmed=true;else if(interposedCleared&&membershipInstalled&&!finalBlockerCreated){mkdirSync(lock);writeFileSync(path.join(lock,"owner.json"),blockerBytes);finalBlockerCreated=true;}}if(point==="before-publication-stage-validation"&&mutationArmed&&!membershipInstalled){mkdirSync(peerStage);writeFileSync(path.join(peerStage,"owner.json"),peerBytes);mkdirSync(lock);writeFileSync(path.join(lock,"owner.json"),interposedBytes);membershipInstalled=true;}if(point==="after-lock-publication-rename-collision"){collisions++;rmSync(lock,{recursive:true});}if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock(),()=>{if(membershipInstalled&&activeObserved&&!interposedCleared){rmSync(lock,{recursive:true});interposedCleared=true;}});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(mutationArmed,true);assert.equal(membershipInstalled,true,"finalNames changes after the original generation closes");assert.equal(activeObserved,true,"the interposed active owner is classified live before replacement re-election");assert.equal(interposedCleared,true);assert.equal(finalBlockerCreated,true,"replacement generation completion reaches a final collision");assert.equal(collisions,6);assert.equal(closures,9);assert.equal(callbackEntries,1);assert.deepEqual(observed!.result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(existsSync(lock),false);const rootNames=await readdir(root),publicationArtifacts=rootNames.filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),unexpectedCoordinationArtifacts=rootNames.filter(name=>name.startsWith(".authority-ledger-lock-")&&!name.startsWith(".authority-ledger-lock-publication-")&&!name.endsWith(".released"));assert.deepEqual(publicationArtifacts,[path.basename(peerStage)],"the exact peer stage is preserved");assert.deepEqual(unexpectedCoordinationArtifacts,[],"no queue or handoff artifact is created");assert.deepEqual(readFileSync(path.join(peerStage,"owner.json")),peerBytes);assert.deepEqual(observed!.delays,[5,10,20,40,50,50,5],"the interposed active wait retains 50ms and reset follows replacement generation completion");}));
...
test("a safely closed live generation admits one unique mutating contender stage",()=>withRoot(async root=>{const predecessor={host:hostname(),nonce:"8".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=await writePublicationStage(root,predecessor,predecessorBytes),predecessorIdentity=exactPublicationIdentity(predecessorStage),authenticated=authenticateOutcomeRequest({tenant:"tenant_1",requester:"requester_1",definitionAlias:"definition_1",request:{v:"reelier.outcome-request/v1",requestId:"request_1",sourceRefs:{source:"ref_1"},choices:{}}}),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),publicationRenames=0,callbackEntries=0,withdrawalActive=false,withdrawalValidations=0,creatorRemoveAttempts=0,creatorRootSyncs=0;const order:string[]=[];Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;try{observed=await underBackwardWallClock(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:30,faultInjector:(point:string)=>{if(point==="after-lock-publication-generation-closed")order.push("closed");if(point==="after-lock-publication-stage-create"){order.push("create");const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")&&name!==path.basename(predecessorStage));assert.ok(ownName);ownStage=path.join(root,ownName);}if(point==="after-lock-publication-stage-sync"){order.push("sync");assert.ok(ownStage);ownBytes=readFileSync(path.join(ownStage,"owner.json"));}if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")creatorRemoveAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).bindIngress(authenticated));}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.ok(observed!.elapsedMs<5_000);assert.equal(order.filter(value=>value==="create").length,1);assert.equal(order.filter(value=>value==="sync").length,1);assert.ok(order.indexOf("closed")>=0&&order.indexOf("closed")<order.indexOf("create")&&order.indexOf("create")<order.indexOf("sync"),order.join(","));assert.ok(ownBytes.length>0);const ownOwner=JSON.parse(ownBytes.toString("utf8"));assert.deepEqual(Object.keys(ownOwner).sort(),["host","nonce","pid","v"]);assert.deepEqual(ownBytes,authorityCanonicalBytes(ownOwner));const match=/^\.authority-ledger-lock-publication-([0-9a-f]{64})-([0-9a-f]{16})-([1-9][0-9]*)-([0-9a-f]{64})\.tmp$/.exec(path.basename(ownStage));assert.ok(match);const ticket=BigInt(`0x${match[2]}`);assert.ok(ticket>=1n&&ticket<=0xffffffffffffffffn);assert.equal(match[1],publicationHostDigest(ownOwner.host));assert.equal(Number(match[3]),ownOwner.pid);assert.equal(match[4],ownOwner.nonce);assert.deepEqual({withdrawalValidations,creatorRemoveAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{withdrawalValidations:1,creatorRemoveAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.ok(path.basename(predecessorStage)<path.basename(ownStage));assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")),[path.basename(predecessorStage)]);assert.deepEqual(exactPublicationIdentity(predecessorStage),predecessorIdentity);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);}));

test("provisional predecessor denial polls only the bigint-and-PID-text immediate predecessor",()=>withRoot(async root=>{
  const peers=[{host:hostname(),nonce:"1".repeat(64),pid:9,v:1 as const,ticket:"0000000000000001"},{host:hostname(),nonce:"2".repeat(64),pid:10,v:1 as const,ticket:"0000000000000002"},{host:hostname(),nonce:"3".repeat(64),pid:2,v:1 as const,ticket:"0000000000000002"}] as const,peerStages=peers.map(peer=>path.join(root,publicationStageName(peer))),peerBytes=peers.map(peer=>publicationOwnerBytes(peer)),peerIdentities:Array<ReturnType<typeof exactPublicationIdentity>>=[],originalKill=process.kill,probes:number[]=[],events:string[]=[];let injected=false,provisionalRoundOpen=false,ownStage="",ownBytes=Buffer.alloc(0),stageValidations=0,fullClosures=0,removeAttempts=0,creatorRootSyncs=0,withdrawalActive=false,withdrawalValidations=0,publicationRenames=0,callbackEntries=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(peers.some(peer=>peer.pid===pid)){probes.push(pid);return true;}return originalKill.call(process,pid,0);}});let observed;try{observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>3n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!injected){injected=true;const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));for(let index=0;index<peers.length;index++){mkdirSync(peerStages[index]);writeFileSync(path.join(peerStages[index],"owner.json"),peerBytes[index]);peerIdentities.push(exactPublicationIdentity(peerStages[index]));}}if(!injected)return;if(point==="after-lock-publication-provisional-predecessor-selection"){provisionalRoundOpen=true;events.push("select");}if(point==="before-publication-stage-validation"&&provisionalRoundOpen){stageValidations++;events.push("validate");}if(point==="before-lock-publication-provisional-root-reenumeration"){events.push("reparse");provisionalRoundOpen=false;}if(point==="before-lock-publication-provisional-predecessor-liveness")events.push("liveness");if(point==="after-lock-publication-generation-closed")fullClosures++;if(point==="before-creator-stage-withdrawal-validation"){withdrawalActive=true;withdrawalValidations++;}if(withdrawalActive&&point==="before-publication-stage-remove-attempt")removeAttempts++;if(withdrawalActive&&point==="after-publication-stage-cleanup-root-sync")creatorRootSyncs++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(injected,true);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.deepEqual(observed!.delays,[5,10,5]);assert.deepEqual(events,Array.from({length:3},()=>["select","validate","validate","reparse","liveness"]).flat());assert.equal(stageValidations,6);assert.deepEqual(probes,[2,2,2],"ticket ties use canonical decimal PID text, never nonce or numeric PID ordering");assert.deepEqual({fullClosures,withdrawalValidations,removeAttempts,creatorRootSyncs,publicationRenames,callbackEntries},{fullClosures:0,withdrawalValidations:1,removeAttempts:1,creatorRootSyncs:1,publicationRenames:0,callbackEntries:0});assert.equal(existsSync(ownStage),false);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),peerStages.map(stage=>path.basename(stage)).sort());assert.ok(ownBytes.length>0);for(let index=0;index<peerStages.length;index++){assert.deepEqual(exactPublicationIdentity(peerStages[index]),peerIdentities[index]);assert.deepEqual(readFileSync(path.join(peerStages[index],"owner.json")),peerBytes[index]);}
}));

test("raw provisional head bypasses selection and enters staged settlement before publication",()=>withRoot(async root=>{
  const peers=[{host:hostname(),nonce:"4".repeat(64),pid:21,v:1 as const,ticket:"0000000000000002"},{host:hostname(),nonce:"5".repeat(64),pid:22,v:1 as const,ticket:"0000000000000003"}] as const,peerStages=peers.map(peer=>path.join(root,publicationStageName(peer))),originalKill=process.kill,order:string[]=[];let injected=false,selections=0,provisionalCloses=0,provisionalLiveness=0,stagedBoundaries=0,fullClosures=0,publicationRenames=0,callbackEntries=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>peers.some(peer=>peer.pid===pid)?true:originalKill.call(process,pid,0)});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>1n,{now:()=>t0,lockTimeoutMs:50,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!injected){injected=true;for(let index=0;index<peers.length;index++){mkdirSync(peerStages[index]);writeFileSync(path.join(peerStages[index],"owner.json"),publicationOwnerBytes(peers[index]));}}if(point==="after-lock-publication-provisional-predecessor-selection")selections++;if(point==="before-lock-publication-provisional-root-reenumeration")provisionalCloses++;if(point==="before-lock-publication-provisional-predecessor-liveness")provisionalLiveness++;if(point==="before-staged-publication-settlement"){stagedBoundaries++;order.push("staged");}if(point==="after-lock-publication-generation-closed"){fullClosures++;order.push("closed");}if(point==="after-lock-publication-rename"){publicationRenames++;order.push("publish");}if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(injected,true);assert.equal(result!.ok,true);assert.deepEqual({selections,provisionalCloses,provisionalLiveness},{selections:0,provisionalCloses:0,provisionalLiveness:0});assert.equal(stagedBoundaries,1);assert.ok(fullClosures>=1);assert.equal(publicationRenames,1);assert.equal(callbackEntries,1);assert.ok(order.indexOf("staged")<order.indexOf("closed")&&order.indexOf("closed")<order.indexOf("publish"),order.join(","));for(const stage of peerStages)assert.equal(existsSync(stage),true);
}));

test("one provisional invalidation falls through to staged settlement in the same iteration",async t=>{
  const cases=["own-missing","own-replaced","predecessor-missing","predecessor-replaced","predecessor-dead","predecessor-unverifiable","membership-add","membership-remove","malformed-name","foreign-provenance","duplicate-provenance"] as const;
  for(const kind of cases)await t.test(kind,()=>withRoot(async root=>{const external=await tempRoot();try{
    const predecessor={host:hostname(),nonce:"6".repeat(64),pid:31,v:1 as const,ticket:"0000000000000002"},distant={host:hostname(),nonce:"7".repeat(64),pid:32,v:1 as const,ticket:"0000000000000001"},added={host:hostname(),nonce:"8".repeat(64),pid:33,v:1 as const,ticket:"0000000000000004"},foreign={host:"foreign.invalid",nonce:"9".repeat(64),pid:34,v:1 as const,ticket:"0000000000000001"},duplicate={...predecessor,nonce:"a".repeat(64),ticket:"0000000000000001"};
    const predecessorBytes=publicationOwnerBytes(predecessor),distantBytes=publicationOwnerBytes(distant),addedBytes=publicationOwnerBytes(added),foreignBytes=publicationOwnerBytes(foreign),duplicateBytes=publicationOwnerBytes(duplicate),predecessorStage=path.join(root,publicationStageName(predecessor)),distantStage=path.join(root,publicationStageName(distant)),addedStage=path.join(root,publicationStageName(added)),foreignStage=path.join(root,publicationStageName(foreign)),duplicateStage=path.join(root,publicationStageName(duplicate)),malformedStage=path.join(root,".authority-ledger-lock-publication-malformed"),livePids=new Set([predecessor.pid,distant.pid]),originalKill=process.kill;
    let ownStage="",ownBytes=Buffer.alloc(0),ownIdentity:Readonly<{dev:bigint;ino:bigint}>|undefined,predecessorIdentity:Readonly<{dev:bigint;ino:bigint}>|undefined,introducedIdentity:Readonly<{dev:bigint;ino:bigint}>|undefined,selection=0,provisionalRoot=0,provisionalLiveness=0,stagedBoundaries=0,delayCount=0,firstFallbackDelay=-1,removeAttempts=0,publicationRenames=0,callbackEntries=0,mutated=false;
    const identity=(target:string)=>{const value=lstatSync(target,{bigint:true});return {dev:value.dev,ino:value.ino};};
    ...
    const mutateIdentity=()=>{mutated=true;if(kind==="own-missing")rmSync(ownStage,{recursive:true});else if(kind==="own-replaced")replace(ownStage,ownBytes,"own");else if(kind==="predecessor-missing")rmSync(predecessorStage,{recursive:true});else if(kind==="predecessor-replaced")replace(predecessorStage,predecessorBytes,"pred");};
    const mutateMembership=()=>{mutated=true;if(kind==="membership-add"){mkdirSync(addedStage);writeFileSync(path.join(addedStage,"owner.json"),addedBytes);livePids.add(added.pid);introducedIdentity=identity(addedStage);}else if(kind==="membership-remove")rmSync(distantStage,{recursive:true});else if(kind==="malformed-name"){mkdirSync(malformedStage);introducedIdentity=identity(malformedStage);}else if(kind==="foreign-provenance"){mkdirSync(foreignStage);writeFileSync(path.join(foreignStage,"owner.json"),foreignBytes);introducedIdentity=identity(foreignStage);}else if(kind==="duplicate-provenance"){mkdirSync(duplicateStage);writeFileSync(path.join(duplicateStage,"owner.json"),duplicateBytes);introducedIdentity=identity(duplicateStage);}};
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===predecessor.pid&&kind==="predecessor-dead"&&mutated)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===predecessor.pid&&kind==="predecessor-unverifiable"&&mutated)throw Object.assign(new Error("unverifiable"),{code:"EPERM"});return livePids.has(pid)?true:originalKill.call(process,pid,0);}});
    let observed;
    try{observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>3n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{
      if(point==="after-lock-publication-stage-sync"&&ownStage===""){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);mkdirSync(distantStage);writeFileSync(path.join(distantStage,"owner.json"),distantBytes);ownIdentity=identity(ownStage);predecessorIdentity=identity(predecessorStage);}
      if(point==="after-lock-publication-provisional-predecessor-selection"){selection++;if(!mutated&&["own-missing","own-replaced","predecessor-missing","predecessor-replaced"].includes(kind))mutateIdentity();}
      if(point==="before-lock-publication-provisional-root-reenumeration"){provisionalRoot++;if(!mutated&&["membership-add","membership-remove","malformed-name","foreign-provenance","duplicate-provenance"].includes(kind))mutateMembership();}
      if(point==="before-lock-publication-provisional-predecessor-liveness"){provisionalLiveness++;if(!mutated&&(kind==="predecessor-dead"||kind==="predecessor-unverifiable"))mutated=true;}
      if(point==="before-staged-publication-settlement"){stagedBoundaries++;if(firstFallbackDelay<0)firstFallbackDelay=delayCount;assert.equal(removeAttempts,0);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);}
      if(point==="before-publication-stage-remove-attempt")removeAttempts++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;
    }}).observeClock(),()=>{delayCount++;});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    const corrupt=["own-missing","own-replaced","predecessor-unverifiable","malformed-name","foreign-provenance","duplicate-provenance"].includes(kind),expected=corrupt?{ok:false as const,reason:"corruption" as const}:{ok:false as const,reason:"busy" as const};
    assert.equal(mutated,true,`${kind} reaches its intended provisional invalidation branch`);assert.deepEqual(observed!.result,expected);assert.equal(selection,1,"a single invalidation cannot re-enter provisional mode before full settlement");assert.equal(stagedBoundaries,1);assert.equal(firstFallbackDelay,0,"fallback occurs in the same iteration before acquisition backoff");assert.deepEqual({publicationRenames,callbackEntries},{publicationRenames:0,callbackEntries:0});
    if(["own-missing","own-replaced","predecessor-missing","predecessor-replaced"].includes(kind))assert.deepEqual({provisionalRoot,provisionalLiveness},{provisionalRoot:0,provisionalLiveness:0});else if(kind==="predecessor-dead"||kind==="predecessor-unverifiable")assert.deepEqual({provisionalRoot,provisionalLiveness},{provisionalRoot:1,provisionalLiveness:1});else assert.deepEqual({provisionalRoot,provisionalLiveness},{provisionalRoot:1,provisionalLiveness:0});
    const expectedRemoveAttempts=kind==="predecessor-dead"?2:["own-missing","own-replaced"].includes(kind)?0:1;
    assert.equal(removeAttempts,expectedRemoveAttempts);if(kind==="own-missing")assert.equal(existsSync(ownStage),false);else if(kind==="own-replaced"){assert.equal(existsSync(ownStage),true);assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);assert.notDeepEqual(identity(ownStage),ownIdentity);}else assert.equal(existsSync(ownStage),false,"every terminal refusal withdraws only the exact creator stage");if(kind==="predecessor-missing")assert.equal(existsSync(predecessorStage),false);else if(kind==="predecessor-replaced"){assert.equal(existsSync(predecessorStage),true);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.notDeepEqual(identity(predecessorStage),predecessorIdentity);}else if(kind==="predecessor-dead")assert.equal(existsSync(predecessorStage),false);else{assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.deepEqual(identity(predecessorStage),predecessorIdentity);}if(kind==="membership-add"){assert.deepEqual(readFileSync(path.join(addedStage,"owner.json")),addedBytes);assert.deepEqual(identity(addedStage),introducedIdentity);}if(kind==="membership-remove")assert.equal(existsSync(distantStage),false);else assert.deepEqual(readFileSync(path.join(distantStage,"owner.json")),distantBytes);if(kind==="malformed-name"){assert.equal(existsSync(malformedStage),true);assert.deepEqual(identity(malformedStage),introducedIdentity);}if(kind==="foreign-provenance"){assert.deepEqual(readFileSync(path.join(foreignStage,"owner.json")),foreignBytes);assert.deepEqual(identity(foreignStage),introducedIdentity);}if(kind==="duplicate-provenance"){assert.deepEqual(readFileSync(path.join(duplicateStage,"owner.json")),duplicateBytes);assert.deepEqual(identity(duplicateStage),introducedIdentity);}
  }finally{await rm(external,{recursive:true,force:true});}}));
});

test("canonical membership churn during required settlement opens a fresh denial-only provisional epoch",()=>withRoot(async root=>{
  const predecessor={host:hostname(),nonce:"b".repeat(64),pid:process.pid+101,v:1 as const,ticket:"0000000000000003"};
  const distant={host:hostname(),nonce:"c".repeat(64),pid:process.pid+102,v:1 as const,ticket:"0000000000000001"};
  const higher={host:hostname(),nonce:"d".repeat(64),pid:process.pid+103,v:1 as const,ticket:"0000000000000005"};
  assert.ok([predecessor.pid,distant.pid,higher.pid].every(pid=>Number.isSafeInteger(pid)&&pid>0));
  const peers=[predecessor,distant,higher] as const;
  const peerBytes=peers.map(peer=>publicationOwnerBytes(peer));
  const peerStages=peers.map(peer=>path.join(root,publicationStageName(peer)));
  const livePids=new Set(peers.map(peer=>peer.pid));
  const originalKill=process.kill;
  const identity=(target:string)=>{const value=lstatSync(target,{bigint:true});return {dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink};};
  let ownStage="",ownBytes=Buffer.alloc(0),ownIdentity:ReturnType<typeof identity>|undefined;
  let firstEpochChurn=false,settlementChurn=false,provisionalSelections=0,provisionalCloses=0,provisionalLiveness=0,stagedBoundaries=0,settlementRootCloses=0,generationClosures=0,removeAttempts=0,publicationRenames=0,callbackEntries=0;
  const peerIdentities=new Map<string,ReturnType<typeof identity>>(),events:string[]=[];
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>livePids.has(pid)?true:originalKill.call(process,pid,0)});
  let observed;
  try{
    observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>4n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{
      if(point==="after-lock-publication-stage-sync"&&ownStage===""){
        const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);
        ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));ownIdentity=identity(ownStage);
        mkdirSync(peerStages[0]);writeFileSync(path.join(peerStages[0],"owner.json"),peerBytes[0]);peerIdentities.set(peerStages[0],identity(peerStages[0]));
      }
      if(point==="after-lock-publication-provisional-predecessor-selection"){provisionalSelections++;events.push(`select:${provisionalSelections}`);}
      if(point==="before-lock-publication-provisional-root-reenumeration"){
        provisionalCloses++;events.push(`provisional-close:${provisionalCloses}`);
        if(provisionalCloses===2&&!firstEpochChurn){firstEpochChurn=true;mkdirSync(peerStages[2]);writeFileSync(path.join(peerStages[2],"owner.json"),peerBytes[2]);peerIdentities.set(peerStages[2],identity(peerStages[2]));events.push("epoch-churn");}
      }
      if(point==="before-lock-publication-provisional-predecessor-liveness"){provisionalLiveness++;events.push(`provisional-live:${provisionalLiveness}`);}
      if(point==="before-staged-publication-settlement"){stagedBoundaries++;events.push("staged");assert.equal(removeAttempts,0);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);}
      if(point==="before-publication-stage-root-reenumeration"&&firstEpochChurn&&!settlementChurn){settlementRootCloses++;settlementChurn=true;mkdirSync(peerStages[1]);writeFileSync(path.join(peerStages[1],"owner.json"),peerBytes[1]);peerIdentities.set(peerStages[1],identity(peerStages[1]));events.push("settlement-churn");}
      if(point==="after-lock-publication-generation-closed"){generationClosures++;events.push("full-close");}
      if(point==="before-publication-stage-remove-attempt"){removeAttempts++;events.push("withdraw");}
      if(point==="after-lock-publication-rename")publicationRenames++;
      if(point==="before-ledger-operation-callback")callbackEntries++;
    }}).observeClock(),requested=>events.push(`delay:${requested}`));
  }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(firstEpochChurn,true);assert.equal(settlementChurn,true);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});
  assert.deepEqual(observed!.delays,[5,10,5],"fresh epochs preserve the accumulated backoff and the original monotonic deadline");
  assert.deepEqual(events,["select:1","provisional-close:1","provisional-live:1","delay:5","select:2","provisional-close:2","epoch-churn","staged","settlement-churn","select:3","provisional-close:3","provisional-live:2","delay:10","select:4","provisional-close:4","provisional-live:3","delay:5","withdraw"]);
  assert.deepEqual({provisionalSelections,provisionalCloses,provisionalLiveness,stagedBoundaries,settlementRootCloses,generationClosures,removeAttempts,publicationRenames,callbackEntries},{provisionalSelections:4,provisionalCloses:4,provisionalLiveness:3,stagedBoundaries:1,settlementRootCloses:1,generationClosures:0,removeAttempts:1,publicationRenames:0,callbackEntries:0});
  assert.equal(existsSync(ownStage),false,"terminal busy withdraws only the exact creator stage");assert.ok(ownIdentity);assert.ok(ownBytes.length>0);
  for(let index=0;index<peerStages.length;index++){assert.equal(existsSync(peerStages[index]),true);assert.deepEqual(readFileSync(path.join(peerStages[index],"owner.json")),peerBytes[index]);assert.deepEqual(identity(peerStages[index]),peerIdentities.get(peerStages[index]));}
...
test("saturated admission liveness memo is local to one acquisition",()=>withRoot(async root=>{
  const peers=[{host:hostname(),nonce:"1".repeat(64),pid:61101,v:1 as const,ticket:"0000000000001001"},{host:hostname(),nonce:"2".repeat(64),pid:61102,v:1 as const,ticket:"0000000000001002"}] as const,stages=await Promise.all(peers.map(async owner=>{const bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),ownerPath=path.join(stage,"owner.json"),identity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>)=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink});return {bytes,stage,stageIdentity:identity(lstatSync(stage,{bigint:true})),ownerIdentity:identity(lstatSync(ownerPath,{bigint:true}))};})),before=await snapshotRootArtifacts(root),originalKill=process.kill,probes=new Map<number,number>(),authenticated=authenticateOutcomeRequest({tenant:"tenant_1",requester:"requester_1",definitionAlias:"definition_1",request:{v:"reelier.outcome-request/v1",requestId:"request_1",sourceRefs:{source:"ref_1"},choices:{}}});let admissionEnumerations=0,stageValidations=0,generationClosures=0,admissionClockSamples=0,removeAttempts=0,creatorPublicationHooks=0,publicationRenames=0,callbackEntries=0;
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(peers.some(peer=>peer.pid===pid)){probes.set(pid,(probes.get(pid)??0)+1);return true;}return originalKill.call(process,pid,0);}});const ledger=rawLedgerWithAdmissionClock(root,()=>{admissionClockSamples++;return 1n;},{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-mutating-admission-enumeration")admissionEnumerations++;if(point==="before-publication-stage-validation")stageValidations++;if(point==="after-lock-publication-generation-closed")generationClosures++;if(point==="before-publication-stage-remove-attempt")removeAttempts++;if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}});try{for(let acquisition=1;acquisition<=2;acquisition++){const beforeEnumerations=admissionEnumerations,observed=await withRecordedDelays(()=>ledger.bindIngress(authenticated));assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.deepEqual(observed.delays,[5,10,5]);assert.equal(admissionEnumerations-beforeEnumerations,observed.delays.length+1);for(const peer of peers)assert.equal(probes.get(peer.pid),acquisition,`PID ${peer.pid} is probed once in acquisition ${acquisition}`);}}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.deepEqual({stageValidations,generationClosures,admissionClockSamples,removeAttempts,creatorPublicationHooks,publicationRenames,callbackEntries},{stageValidations:0,generationClosures:0,admissionClockSamples:0,removeAttempts:0,creatorPublicationHooks:0,publicationRenames:0,callbackEntries:0});assert.deepEqual(await snapshotRootArtifacts(root),before);for(const item of stages){const identity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>)=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink}),ownerPath=path.join(item.stage,"owner.json");assert.deepEqual(identity(lstatSync(item.stage,{bigint:true})),item.stageIdentity);assert.deepEqual(identity(lstatSync(ownerPath,{bigint:true})),item.ownerIdentity);assert.deepEqual(readFileSync(ownerPath),item.bytes);}
}));

test("saturated admission membership change enters full settlement in the same iteration",async t=>{for(const mutation of ["add","remove"] as const)await t.test(mutation,()=>withRoot(async root=>{
  const first={host:hostname(),nonce:"3".repeat(64),pid:61201,v:1 as const,ticket:"0000000000001101"},second={host:hostname(),nonce:"4".repeat(64),pid:61202,v:1 as const,ticket:"0000000000001102"},third={host:hostname(),nonce:"5".repeat(64),pid:61203,v:1 as const,ticket:"0000000000001103"},firstStage=await writePublicationStage(root,first,publicationOwnerBytes(first)),secondStage=await writePublicationStage(root,second,publicationOwnerBytes(second)),thirdStage=path.join(root,publicationStageName(third)),livePids=new Set([first.pid,second.pid]),originalKill=process.kill,authenticated=authenticateOutcomeRequest({tenant:"tenant_1",requester:"requester_1",definitionAlias:"definition_1",request:{v:"reelier.outcome-request/v1",requestId:"request_1",sourceRefs:{source:"ref_1"},choices:{}}}),identity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>)=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink});let admissionEnumerations=0,delayCount=0,firstValidationDelayCount=-1,stageValidations=0,generationClosures=0,admissionClockSamples=0,removeAttempts=0,creatorPublicationHooks=0,publicationRenames=0,callbackEntries=0,mutated=false;let preserved:Array<{stage:string;bytes:Buffer;stageIdentity:ReturnType<typeof identity>;ownerIdentity:ReturnType<typeof identity>}>=[];
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>livePids.has(pid)?true:originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>{admissionClockSamples++;return 1n;},{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-mutating-admission-enumeration")admissionEnumerations++;if(point==="before-publication-stage-validation"){stageValidations++;if(firstValidationDelayCount<0)firstValidationDelayCount=delayCount;throw Object.assign(new Error("full-settlement-sentinel"),{code:"EBUSY"});}if(point==="after-lock-publication-generation-closed")generationClosures++;if(point==="before-publication-stage-remove-attempt")removeAttempts++;if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).bindIngress(authenticated),requested=>{delayCount++;if(delayCount===1){assert.equal(requested,5);if(mutation==="add"){mkdirSync(thirdStage);writeFileSync(path.join(thirdStage,"owner.json"),publicationOwnerBytes(third));livePids.add(third.pid);}else{rmSync(firstStage,{recursive:true});livePids.delete(first.pid);}mutated=true;preserved=readdirSync(root).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort().map(name=>{const stage=path.join(root,name),ownerPath=path.join(stage,"owner.json");return {stage,bytes:readFileSync(ownerPath),stageIdentity:identity(lstatSync(stage,{bigint:true})),ownerIdentity:identity(lstatSync(ownerPath,{bigint:true}))};});}});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(mutated,true);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.equal(firstValidationDelayCount,1,"changed membership reaches full stage validation after exactly the first 5ms wait");assert.equal(admissionEnumerations,2,"only the original and changed saturated polls use the raw admission enumeration");assert.ok(stageValidations>=1);assert.deepEqual({generationClosures,admissionClockSamples,removeAttempts,creatorPublicationHooks,publicationRenames,callbackEntries},{generationClosures:0,admissionClockSamples:0,removeAttempts:0,creatorPublicationHooks:0,publicationRenames:0,callbackEntries:0});assert.deepEqual(readdirSync(root).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),preserved.map(item=>path.basename(item.stage)));for(const item of preserved){const ownerPath=path.join(item.stage,"owner.json");assert.deepEqual(identity(lstatSync(item.stage,{bigint:true})),item.stageIdentity);assert.deepEqual(identity(lstatSync(ownerPath,{bigint:true})),item.ownerIdentity);assert.deepEqual(readFileSync(ownerPath),item.bytes);}assert.equal(existsSync(secondStage),true);assert.equal(existsSync(thirdStage),mutation==="add");
}));});
...
test("all late publication snapshot boundaries use one bounded retry protocol",{timeout:5_000},async t=>{
  ...
  await t.test("persistent-final-validation",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"f".repeat(64),pid:await exitedChildPid(),v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes),rootBefore=await snapshotRootArtifacts(root);let finalValidations=0,generations=0,callbackEntries=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-publication-stage-enumeration")generations++;if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="before-publication-stage-final-validation"){finalValidations++;throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}}} as never).recover());assert.equal(finalValidations,21);assert.equal(generations,21);assert.equal(callbackEntries,0);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.deepEqual(await snapshotRootArtifacts(root),rootBefore);assert.deepEqual(observed.result,{ok:false,reason:"busy"});}));
  await t.test("persistent-root-reenumeration",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"1".repeat(64),pid:await exitedChildPid(),v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes),rootBefore=await snapshotRootArtifacts(root);let attempts=0,generations=0,callbackEntries=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-publication-stage-enumeration")generations++;if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="before-publication-stage-root-reenumeration"){attempts++;throw Object.assign(new Error("EACCES"),{code:"EACCES"});}}} as never).recover());assert.equal(attempts,21);assert.equal(generations,21);assert.equal(callbackEntries,0);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.deepEqual(await snapshotRootArtifacts(root),rootBefore);assert.deepEqual(observed.result,{ok:false,reason:"busy"});}));
});
...
test("a correct concurrent dead-stage remover makes non-authorizing cleanup progress",async t=>{
  const exactIdentity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>)=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink});
  await t.test("retained cleanup authorization accepts the same directory becoming empty",()=>withRoot(async root=>{
    const owner={host:hostname(),nonce:"7".repeat(64),pid:45001,v:1 as const,ticket:"0000000000000101"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),ownerPath=path.join(stage,"owner.json"),directoryIdentity=exactIdentity(lstatSync(stage,{bigint:true})),ownerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true})),originalKill=process.kill;
    let generations=0,removeAttempts=0,callbackEntries=0,creatorPublicationHooks=0,emptied=false;
    ...
    let observed;
    try{
      observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:5,faultInjector:(point:string)=>{
        if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;
        if(point==="before-ledger-operation-callback")callbackEntries++;
        if(point==="after-publication-stage-enumeration"&&++generations===2){assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);assert.deepEqual(exactIdentity(lstatSync(ownerPath,{bigint:true})),ownerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);rmSync(ownerPath);emptied=true;assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);}
  ...
  await t.test("cleanup authorization survives root sync until a closed absent generation",()=>withRoot(async root=>{const external=await tempRoot();try{
    const owner={host:hostname(),nonce:"c".repeat(64),pid:45006,v:1 as const,ticket:"0000000000000106"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),ownerPath=path.join(stage,"owner.json"),originalDirectoryIdentity=exactIdentity(lstatSync(stage,{bigint:true})),originalOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true})),prepared=path.join(external,"replacement"),replacementOwnerPath=path.join(prepared,"owner.json"),originalKill=process.kill,order:string[]=[];mkdirSync(prepared);writeFileSync(replacementOwnerPath,bytes);
    let generations=0,removeAttempts=0,rootSyncs=0,callbackEntries=0,creatorPublicationHooks=0,peerRemoved=false,replacementInstalled=false,replacementDirectoryIdentity:ReturnType<typeof exactIdentity>|undefined,replacementOwnerIdentity:ReturnType<typeof exactIdentity>|undefined;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});
    let observed;
    try{
      observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:50,faultInjector:(point:string)=>{
        if(point==="after-publication-stage-enumeration"){generations++;if(peerRemoved)order.push("next-generation");if(generations===2){assert.equal(peerRemoved,false);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),originalDirectoryIdentity);assert.deepEqual(exactIdentity(lstatSync(ownerPath,{bigint:true})),originalOwnerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);rmSync(stage,{recursive:true});peerRemoved=true;order.push("peer-absent");}}
        if(point==="after-publication-stage-cleanup-root-sync"){rootSyncs++;order.push("cleanup-root-sync");if(!replacementInstalled){assert.equal(existsSync(stage),false);renameSync(prepared,stage);replacementInstalled=true;replacementDirectoryIdentity=exactIdentity(lstatSync(stage,{bigint:true}));replacementOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true}));assert.notDeepEqual(replacementDirectoryIdentity,originalDirectoryIdentity);assert.notDeepEqual(replacementOwnerIdentity,originalOwnerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);order.push("replacement-installed");}}
        if(point==="before-publication-stage-remove-attempt"){removeAttempts++;order.push(removeAttempts===1?"initial-remove-attempt":"replacement-remove-attempt");if(removeAttempts===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}
        if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;
        if(point==="before-ledger-operation-callback")callbackEntries++;
      }} as never).observeClock());
    }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(peerRemoved,true);assert.equal(replacementInstalled,true);assert.deepEqual(observed!.result,{ok:false,reason:"corruption"});assert.equal(rootSyncs,1);assert.deepEqual(order.slice(0,5),["initial-remove-attempt","peer-absent","cleanup-root-sync","replacement-installed","next-generation"]);assert.equal(removeAttempts,1,"the retained authorization refuses before a second removal attempt can target the replacement");assert.deepEqual({callbackEntries,creatorPublicationHooks},{callbackEntries:0,creatorPublicationHooks:0});assert.ok(replacementDirectoryIdentity);assert.ok(replacementOwnerIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),replacementDirectoryIdentity);assert.deepEqual(exactIdentity(lstatSync(ownerPath,{bigint:true})),replacementOwnerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);
  }finally{await rm(external,{recursive:true,force:true});}}));

  await t.test("an invalidated absent generation cannot retire cleanup authorization",()=>withRoot(async root=>{const external=await tempRoot();try{
    const original={host:hostname(),nonce:"d".repeat(64),pid:45007,v:1 as const,ticket:"0000000000000201"},other={host:hostname(),nonce:"e".repeat(64),pid:45008,v:1 as const,ticket:"0000000000000202"},originalBytes=publicationOwnerBytes(original),otherBytes=publicationOwnerBytes(other),originalStage=await writePublicationStage(root,original,originalBytes),originalOwnerPath=path.join(originalStage,"owner.json"),otherStage=await writePublicationStage(root,other,otherBytes),otherOwnerPath=path.join(otherStage,"owner.json"),originalDirectoryIdentity=exactIdentity(lstatSync(originalStage,{bigint:true})),originalOwnerIdentity=exactIdentity(lstatSync(originalOwnerPath,{bigint:true})),prepared=path.join(external,"replacement"),originalKill=process.kill;mkdirSync(prepared);writeFileSync(path.join(prepared,"owner.json"),originalBytes);
    let generations=0,removeAttempts=0,rootSyncs=0,callbackEntries=0,creatorPublicationHooks=0,removed=false,synced=false,otherInvalidated=false,replacementInstalled=false,replacementDirectoryIdentity:ReturnType<typeof exactIdentity>|undefined,replacementOwnerIdentity:ReturnType<typeof exactIdentity>|undefined;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===original.pid||pid===other.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});let observed;
    try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{
      if(point==="after-publication-stage-enumeration"){generations++;if(generations===2){assert.deepEqual(exactIdentity(lstatSync(originalStage,{bigint:true})),originalDirectoryIdentity);assert.deepEqual(exactIdentity(lstatSync(originalOwnerPath,{bigint:true})),originalOwnerIdentity);rmSync(originalStage,{recursive:true});removed=true;}if(synced&&!otherInvalidated){assert.equal(existsSync(originalStage),false,"tombstoned name is absent in initial names");}}
      if(point==="before-publication-stage-remove-attempt"&&++removeAttempts===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});
      if(point==="after-publication-stage-cleanup-root-sync"){rootSyncs++;synced=true;}
      if(point==="after-lock-publication-generation-closed"&&synced&&!otherInvalidated){assert.equal(existsSync(originalStage),false,"tombstoned name is absent in closure names");rmSync(otherOwnerPath);otherInvalidated=true;}
      if(point==="before-publication-stage-validation"&&otherInvalidated&&!replacementInstalled)assert.equal(existsSync(originalStage),false,"tombstoned name is absent in final names before exact revalidation invalidates the peer");
      if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;
    }} as never).observeClock(),()=>{if(otherInvalidated&&!replacementInstalled){renameSync(prepared,originalStage);replacementInstalled=true;replacementDirectoryIdentity=exactIdentity(lstatSync(originalStage,{bigint:true}));replacementOwnerIdentity=exactIdentity(lstatSync(originalOwnerPath,{bigint:true}));assert.notDeepEqual(replacementDirectoryIdentity,originalDirectoryIdentity);assert.notDeepEqual(replacementOwnerIdentity,originalOwnerIdentity);}});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(removed,true);assert.equal(synced,true);assert.equal(otherInvalidated,true);assert.equal(replacementInstalled,true);assert.deepEqual(observed!.result,{ok:false,reason:"corruption"});assert.equal(rootSyncs,1);assert.equal(removeAttempts,1);assert.deepEqual({callbackEntries,creatorPublicationHooks},{callbackEntries:0,creatorPublicationHooks:0});assert.ok(replacementDirectoryIdentity);assert.ok(replacementOwnerIdentity);assert.deepEqual(exactIdentity(lstatSync(originalStage,{bigint:true})),replacementDirectoryIdentity);assert.deepEqual(exactIdentity(lstatSync(originalOwnerPath,{bigint:true})),replacementOwnerIdentity);assert.deepEqual(readFileSync(originalOwnerPath),originalBytes);
  }finally{await rm(external,{recursive:true,force:true});}}));

  await t.test("every raw name snapshot rejects a tombstoned-name reappearance",async t=>{for(const boundary of ["initial","closed","final","per-removal"] as const)await t.test(boundary,()=>withRoot(async root=>{const external=await tempRoot();try{
    const original={host:hostname(),nonce:(boundary==="initial"?"1":boundary==="closed"?"2":boundary==="final"?"3":"4").repeat(64),pid:45100+["initial","closed","final","per-removal"].indexOf(boundary),v:1 as const,ticket:"0000000000000301"},candidate={host:hostname(),nonce:"5".repeat(64),pid:45110+["initial","closed","final","per-removal"].indexOf(boundary),v:1 as const,ticket:"0000000000000302"},bytes=publicationOwnerBytes(original),candidateBytes=publicationOwnerBytes(candidate),stage=await writePublicationStage(root,original,bytes),ownerPath=path.join(stage,"owner.json"),candidateStage=await writePublicationStage(root,candidate,candidateBytes),originalDirectoryIdentity=exactIdentity(lstatSync(stage,{bigint:true})),originalOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true})),prepared=path.join(external,"replacement"),originalKill=process.kill,perRemovalOrder:string[]=[];mkdirSync(prepared);writeFileSync(path.join(prepared,"owner.json"),bytes);let generations=0,removeAttempts=0,synced=false,installed=false,delaysAfterInstall=0,callbackEntries=0,creatorPublicationHooks=0,postSyncRootReenumerations=0,postSyncClosures=0,postSyncFinalValidations=0,replacementDirectoryIdentity:ReturnType<typeof exactIdentity>|undefined,replacementOwnerIdentity:ReturnType<typeof exactIdentity>|undefined;
    const install=()=>{if(installed)return;assert.equal(existsSync(stage),false);renameSync(prepared,stage);installed=true;replacementDirectoryIdentity=exactIdentity(lstatSync(stage,{bigint:true}));replacementOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true}));assert.notDeepEqual(replacementDirectoryIdentity,originalDirectoryIdentity);assert.notDeepEqual(replacementOwnerIdentity,originalOwnerIdentity);};Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===original.pid)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===candidate.pid){if(boundary==="per-removal")throw Object.assign(new Error("dead"),{code:"ESRCH"});return true;}return originalKill.call(process,pid,0);}});let observed;
    try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-publication-stage-enumeration"&&++generations===2)rmSync(stage,{recursive:true});if(point==="before-publication-stage-remove-attempt"){removeAttempts++;if(removeAttempts===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}if(point==="after-publication-stage-cleanup-root-sync"){synced=true;if(boundary==="initial")install();}if(synced&&point==="before-publication-stage-root-reenumeration"){postSyncRootReenumerations++;if(boundary==="per-removal"&&postSyncRootReenumerations===1)perRemovalOrder.push("closed-snapshot");if(boundary==="closed")install();if(boundary==="per-removal"&&postSyncClosures>0&&postSyncFinalValidations>0){perRemovalOrder.push("per-removal-install");install();}}if(synced&&point==="after-lock-publication-generation-closed"){postSyncClosures++;if(boundary==="per-removal")perRemovalOrder.push("closure");if(boundary==="final")install();}if(synced&&postSyncClosures>0&&point==="before-publication-stage-validation"){postSyncFinalValidations++;if(boundary==="per-removal"&&postSyncFinalValidations===1)perRemovalOrder.push("final-validation");}if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).recover(),()=>{if(installed)delaysAfterInstall++;});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(synced,true);assert.equal(installed,true);assert.deepEqual(observed!.result,{ok:false,reason:"corruption"});assert.equal(delaysAfterInstall,0,`${boundary} snapshot rejects directly rather than retrying membership`);assert.equal(removeAttempts,1,`${boundary} snapshot refuses before any second removal attempt`);assert.deepEqual({callbackEntries,creatorPublicationHooks},{callbackEntries:0,creatorPublicationHooks:0});if(boundary==="per-removal"){assert.equal(postSyncRootReenumerations,2);assert.deepEqual(perRemovalOrder,["closed-snapshot","closure","final-validation","per-removal-install"]);}assert.ok(replacementDirectoryIdentity);assert.ok(replacementOwnerIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),replacementDirectoryIdentity);assert.deepEqual(exactIdentity(lstatSync(ownerPath,{bigint:true})),replacementOwnerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);assert.equal(existsSync(candidateStage),true);
  }finally{await rm(external,{recursive:true,force:true});}}));});

  await t.test("pending cleanup sync cannot be bypassed when a later dead candidate becomes live",()=>withRoot(async root=>{
    const removed={host:hostname(),nonce:"6".repeat(64),pid:45201,v:1 as const,ticket:"0000000000000401"},survivor={host:hostname(),nonce:"7".repeat(64),pid:45202,v:1 as const,ticket:"0000000000000402"},removedStage=await writePublicationStage(root,removed,publicationOwnerBytes(removed)),survivorBytes=publicationOwnerBytes(survivor),survivorStage=await writePublicationStage(root,survivor,survivorBytes),originalKill=process.kill,order:string[]=[];let finalLiveness=0,survivorLive=false,rootSyncs=0,removeAttempts=0,callbackEntries=0,creatorPublicationHooks=0;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===removed.pid)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===survivor.pid){if(survivorLive)return true;throw Object.assign(new Error("dead"),{code:"ESRCH"});}return originalKill.call(process,pid,0);}});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="before-publication-stage-final-liveness"&&++finalLiveness===2)survivorLive=true;if(point==="before-publication-stage-remove-attempt"){removeAttempts++;order.push("remove");}if(point==="after-publication-stage-cleanup-root-sync"){rootSyncs++;order.push("root-sync");}if((publicationCrashPoints as readonly string[]).includes(point))creatorPublicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).recover());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(existsSync(removedStage),false);assert.equal(existsSync(survivorStage),true);assert.deepEqual(await readFile(path.join(survivorStage,"owner.json")),survivorBytes);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.equal(rootSyncs,1,"array return cannot bypass a pending cleanup-root sync");assert.equal(removeAttempts,1);assert.deepEqual(order.slice(0,2),["remove","root-sync"]);assert.deepEqual({callbackEntries,creatorPublicationHooks},{callbackEntries:0,creatorPublicationHooks:0});
  }));
  ...
  await t.test("an active owner retries a dead peer that becomes empty after generation closure",()=>withRoot(async root=>{
    ...
    try{
      observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:5,faultInjector:(point:string)=>{
        if(point==="after-lock-publication-root-sync"&&!injected){injected=true;mkdirSync(stage);writeFileSync(ownerPath,bytes);directoryIdentity=exactIdentity(lstatSync(stage,{bigint:true}));}
        if(point==="after-lock-publication-generation-closed"&&injected&&!emptied){assert.ok(directoryIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);rmSync(ownerPath);emptied=true;assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);}
        if(point==="before-publication-stage-final-validation"&&emptied&&++postProgressValidationFaults===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});
        if(point==="before-ledger-operation-callback")callbackEntries++;
      }} as never).observeClock());
    }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(injected,true);assert.equal(emptied,true);assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.equal(postProgressValidationFaults,1,"the invalidated generation retries into the bounded second generation");assert.deepEqual(observed!.delays,[5]);assert.equal(callbackEntries,0);assert.ok(directoryIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);assert.deepEqual(await readdir(stage),[]);assert.equal(existsSync(path.join(root,"lock")),false);
  }));
});

test("retained dead-stage cleanup authorization rejects identity replacement",async t=>{
  const exactIdentity=(value:Readonly<{dev:bigint;ino:bigint;mode:bigint;nlink:bigint}>)=>({dev:value.dev,ino:value.ino,mode:value.mode,nlink:value.nlink});
  await t.test("whole directory replacement with an empty directory remains corruption",()=>withRoot(async root=>{const external=await tempRoot();try{
    const owner={host:hostname(),nonce:"9".repeat(64),pid:45003,v:1 as const,ticket:"0000000000000103"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),ownerPath=path.join(stage,"owner.json"),originalIdentity=exactIdentity(lstatSync(stage,{bigint:true})),prepared=path.join(external,"replacement"),displaced=path.join(external,"displaced"),originalKill=process.kill;mkdirSync(prepared);let generations=0,removeAttempts=0,callbackEntries=0,replacementIdentity:ReturnType<typeof exactIdentity>|undefined;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:5,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-publication-stage-enumeration"&&++generations===2){renameSync(stage,displaced);renameSync(prepared,stage);replacementIdentity=exactIdentity(lstatSync(stage,{bigint:true}));assert.notDeepEqual(replacementIdentity,originalIdentity);}if(point==="before-publication-stage-remove-attempt"&&++removeAttempts===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}} as never).recover());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"corruption"});assert.deepEqual(observed!.delays,[5]);assert.equal(removeAttempts,1);assert.equal(callbackEntries,0);assert.ok(replacementIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),replacementIdentity);assert.deepEqual(await readdir(stage),[]);assert.deepEqual(readFileSync(path.join(displaced,"owner.json")),bytes);assert.equal(existsSync(ownerPath),false);
  }finally{await rm(external,{recursive:true,force:true});}}));

  await t.test("canonical-same owner bytes with a new file identity remain corruption",()=>withRoot(async root=>{const external=await tempRoot();try{
    const owner={host:hostname(),nonce:"a".repeat(64),pid:45004,v:1 as const,ticket:"0000000000000104"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),ownerPath=path.join(stage,"owner.json"),directoryIdentity=exactIdentity(lstatSync(stage,{bigint:true})),originalOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true})),prepared=path.join(external,"replacement-owner.json"),displaced=path.join(external,"displaced-owner.json"),originalKill=process.kill;writeFileSync(prepared,bytes);let generations=0,removeAttempts=0,callbackEntries=0,replacementOwnerIdentity:ReturnType<typeof exactIdentity>|undefined;
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:5,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-publication-stage-enumeration"&&++generations===2){renameSync(ownerPath,displaced);renameSync(prepared,ownerPath);replacementOwnerIdentity=exactIdentity(lstatSync(ownerPath,{bigint:true}));assert.notDeepEqual(replacementOwnerIdentity,originalOwnerIdentity);}if(point==="before-publication-stage-remove-attempt"&&++removeAttempts===1)throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}} as never).recover());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"corruption"});assert.deepEqual(observed!.delays,[5]);assert.equal(removeAttempts,1);assert.equal(callbackEntries,0);assert.ok(replacementOwnerIdentity);assert.deepEqual(exactIdentity(lstatSync(stage,{bigint:true})),directoryIdentity);assert.deepEqual(exactIdentity(lstatSync(ownerPath,{bigint:true})),replacementOwnerIdentity);assert.deepEqual(readFileSync(ownerPath),bytes);assert.deepEqual(readFileSync(displaced),bytes);
  }finally{await rm(external,{recursive:true,force:true});}}));
});
