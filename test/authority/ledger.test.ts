...
function incompleteCoordinationAck(owner:AdmissionOwner,purpose:"prep-retired"|"creator-withdrawal",markerName:string,originalName:string,state:AdmissionPartialState,marker:string){const bytes=state==="empty"?Buffer.alloc(0):readFileSync(path.join(marker,"owner.json"));return {directoryIdentity:decimalIdentity(marker),kind:purpose==="prep-retired"?"admission-prep-retired":"creator-withdrawal",markerName,originalName,owner:state==="empty"?null:owner,ownerBytesDigest:rawDigest(bytes),ownerBytesLength:String(bytes.length),ownerDigest:authorityDigest(owner),ownerIdentity:state==="empty"?null:decimalIdentity(path.join(marker,"owner.json")),purpose,recoveryAuthority:purpose==="prep-retired"?"dead-owner-or-exact-creator":"dead-owner-or-exact-creator-after-slot-withdrawn",state,v:coordinationAckVersion};}
...
test("atomic admission prep and incomplete-withdrawal ack windows converge by purpose",async t=>{
  for(const purpose of ["prep-retired","creator-withdrawal"] as const)for(const state of ["marker-only","marker-plus-ack","orphan-ack"] as const)await t.test(`${purpose}-${state}`,()=>withRoot(async root=>{const owner={host:hostname(),nonce:(purpose==="prep-retired"?(state==="marker-only"?"8":state==="marker-plus-ack"?"9":"a"):(state==="marker-only"?"b":state==="marker-plus-ack"?"c":"d")).repeat(64),pid:process.pid,v:1 as const},partialState="partial" as const,markerName=purpose==="prep-retired"?admissionPrepRetiredName(owner,partialState):creatorWithdrawalName(owner,partialState),originalName=purpose==="prep-retired"?admissionPrepName(owner):publicationStageName({...owner,ticket:"0000000000000001"}),marker=path.join(root,markerName);await mkdir(marker);await writeFile(path.join(marker,"owner.json"),ownerStateBytes(owner,partialState));const ack=incompleteCoordinationAck(owner,purpose,markerName,originalName,partialState,marker),ackPath=path.join(root,coordinationAckName(ack));if(state!=="marker-only")await writeFile(ackPath,authorityCanonicalBytes(ack));if(state==="orphan-ack")await rm(marker,{recursive:true});if(purpose==="creator-withdrawal"){const slotMarkerName=admissionRetiredName(owner,"withdrawn"),slotMarker=path.join(root,slotMarkerName);await mkdir(slotMarker);await writeFile(path.join(slotMarker,"owner.json"),publicationOwnerBytes(owner));const slotAck=slotCoordinationAck(owner,slotMarkerName,slotMarker,"withdrawn",markerName,ownerStateBytes(owner,partialState));await writeFile(path.join(root,coordinationAckName(slotAck)),authorityCanonicalBytes(slotAck));await rm(slotMarker,{recursive:true});}const result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200}).observeClock();assert.deepEqual(result,{ok:true,status:"advanced",observedAt:new Date(t0).toISOString()});assert.equal(existsSync(marker),false);assert.equal(existsSync(ackPath),false);}));
});

test("atomic admission incomplete-withdrawal evidence rejects mismatch links reparses and broad prefixes",async t=>{
  for(const kind of ["ack-mismatch","hardlink","broad-prefix"] as const)await t.test(kind,()=>withRoot(async root=>{const owner={host:hostname(),nonce:(kind==="ack-mismatch"?"e":kind==="hardlink"?"f":"1").repeat(64),pid:process.pid,v:1 as const},baseName=creatorWithdrawalName(owner,"partial"),markerName=kind==="broad-prefix"?`${baseName}.extra`:baseName,marker=path.join(root,markerName),external=await tempRoot();try{await mkdir(marker);if(kind==="hardlink"){const source=path.join(external,"owner.json");await writeFile(source,ownerStateBytes(owner,"partial"));await link(source,path.join(marker,"owner.json"));}else await writeFile(path.join(marker,"owner.json"),ownerStateBytes(owner,"partial"));if(kind==="ack-mismatch"){const ack={...incompleteCoordinationAck(owner,"creator-withdrawal",markerName,publicationStageName({...owner,ticket:"0000000000000001"}),"partial",marker),ownerBytesLength:"999"};await writeFile(path.join(root,coordinationAckName(ack)),authorityCanonicalBytes(ack));}const before=await snapshotRootArtifacts(root),result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20}).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.deepEqual(await snapshotRootArtifacts(root),before);}finally{await rm(external,{recursive:true,force:true});}}));
