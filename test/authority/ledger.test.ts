...
test("prep-only dead-owner writer retires a complete prep after final revalidation",()=>withRoot(async root=>{
  const tokenPoint="prep-only-creator-token-carried",deadPoint="prep-only-prep-retirement-authority-dead-owner",beforePoint="prep-only-before-transition",retiredPoint="prep-only-prep-retirement-root-synced",cleanupStagePoint="prep-only-cleanup-stage-zero",cleanupFinalPoint="prep-only-cleanup-stage-complete",sentinel={kind:"prep-retirement-root-synced"};
  ...
  assert.deepEqual(events,[tokenPoint,deadPoint,beforePoint,retiredPoint]);
...
test("prep-only cleanup writer resumes one prep-retired stage from zero through prefix to complete",()=>withRoot(async root=>{
  const cleanupPoint="prep-only-prep-retired-cleanup-authority",beforePoint="prep-only-before-transition",zeroPoint="prep-only-cleanup-stage-zero",prefixPoint="prep-only-cleanup-stage-prefix",completePoint="prep-only-cleanup-stage-complete";
  ...
  const runPhase=async(point:string,index:number)=>{const sentinel={kind:"prep-cleanup-stage",index,point},events:string[]=[];let semanticNow=0,callbacks=0,delays=0,thrown:unknown;const runtime={monotonicNow:()=>0,delay:async()=>{delays++;},observeBoundary:(observed:string)=>{events.push(observed);if(observed===point)throw sentinel;}};try{await new RawFsAuthorityLedger(root,{[__testPrepHousekeeperRuntimeOption]:runtime,now:()=>{semanticNow++;return t0;},lockTimeoutMs:20,faultInjector:(boundary:string)=>{if(boundary==="before-ledger-operation-callback")callbacks++;}} as never).recover();}catch(error){thrown=error;}assert.equal(thrown,sentinel,`${point} is a live private writer boundary`);assert.deepEqual(events,[cleanupPoint,beforePoint,point],point);assert.deepEqual({semanticNow,callbacks,delays},{semanticNow:0,callbacks:0,delays:0},point);assert.equal(existsSync(marker),true,point);assert.deepEqual(exactFsIdentity(marker),markerIdentity,point);assert.deepEqual(exactFsIdentity(markerOwner),markerOwnerIdentity,point);assert.deepEqual(await readFile(markerOwner),markerOwnerBytes,point);assert.equal(existsSync(finalAck),false,point);};
