...
test("publication revalidates exact owner bytes before rename and after every published boundary",async t=>{
  for(const point of ["after-lock-publication-owner-sync","after-lock-publication-stage-sync","after-lock-publication-rename","after-lock-publication-root-sync"] as const)for(const mutation of ["replace","truncate"] as const)await t.test(`${point}:${mutation}`,()=>withRoot(async root=>{let changed=false,callbackCount=0,target="",untrusted=Buffer.alloc(0);const preRename=point==="after-lock-publication-owner-sync"||point==="after-lock-publication-stage-sync",ledger=new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(observed:string)=>{if(observed==="clock-before-clock-high-water-write")callbackCount++;if(changed||observed!==point)return;changed=true;const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));target=preRename?path.join(root,stage!,"owner.json"):path.join(root,"lock","owner.json");const original=readFileSync(target);untrusted=mutation==="replace"?Buffer.from("untrusted-owner-bytes"):original.subarray(0,Math.max(1,Math.floor(original.length/2)));writeFileSync(target,untrusted);}} as never),result=await ledger.observeClock();assert.equal(changed,true);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackCount,0,"durable callback never starts after owner mismatch");assert.deepEqual(await readFile(target),untrusted);if(preRename){assert.equal(existsSync(path.join(root,"lock")),false);assert.ok(path.basename(path.dirname(target)).startsWith(".authority-ledger-lock-publication-"));}else assert.equal(existsSync(path.join(root,"lock")),true);}));
});
...
test("publication write-all handles short writes and refuses zero progress",async t=>{
  ...
  await t.test("short writes",()=>withRoot(async root=>{prototype.write=async function(buffer,offset,length,position){return original.call(this,buffer,offset,Math.min(length,2),position);};try{const result=await new RawFsAuthorityLedger(root,{now:()=>t0,faultInjector:(point:string)=>{if(point==="after-lock-publication-root-sync")prototype.write=original;}} as never).observeClock();assert.equal(result.ok,true);assert.equal((await readdir(root)).some(name=>name==="lock"||name.startsWith(".authority-ledger-lock-publication-")),false);}finally{prototype.write=original;}}));
  for(const [name,reported] of [["zero-progress",0],["negative-progress",-1],["oversized-progress",Number.MAX_SAFE_INTEGER]] as const)await t.test(name,()=>withRoot(async root=>{let callbackCount=0;prototype.write=async function(buffer){return {bytesWritten:reported,buffer};};try{const result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="clock-before-clock-high-water-write")callbackCount++;}} as never).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackCount,0);const stages=(await readdir(root)).filter(stage=>stage.startsWith(".authority-ledger-lock-publication-"));assert.equal(stages.length,1);assert.equal((await readFile(path.join(root,stages[0],"owner.json"))).length,0);}finally{prototype.write=original;}}));
});
...
test("lock and publication enumeration races restart the whole bounded snapshot",async t=>{
  await t.test("active lock disappears after metadata",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"1".repeat(64),pid:process.pid,v:1 as const},directory=path.join(root,"lock");await mkdir(directory);await writeFile(path.join(directory,"owner.json"),authorityCanonicalBytes(owner));let fired=false;const result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(!fired&&point==="after-active-lock-metadata"){fired=true;rmSync(directory,{recursive:true});}}} as never).recover();assert.equal(fired,true);assert.equal(result.ok,true);}));
  await t.test("publication stage disappears after enumeration",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"2".repeat(64),pid:await exitedChildPid(),v:1 as const},stage=await writePublicationStage(root,owner,authorityCanonicalBytes(owner));let fired=false;const result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(!fired&&point==="after-publication-stage-enumeration"){fired=true;rmSync(stage,{recursive:true});}}} as never).recover();assert.equal(fired,true);assert.equal(result.ok,true);}));
});

test("persistent publication snapshot sharing violations exhaust the monotonic deadline without mutation",async t=>{
  for(const code of ["EPERM","EACCES","EBUSY"] as const)for(const boundary of ["before-active-lock-content-read","before-publication-stage-validation"] as const)await t.test(`${boundary}:${code}`,()=>withRoot(async root=>{const owner={host:hostname(),nonce:(boundary==="before-active-lock-content-read"?"3":"4").repeat(64),pid:process.pid,v:1 as const},artifact=boundary==="before-active-lock-content-read"?path.join(root,"lock"):await writePublicationStage(root,owner,authorityCanonicalBytes(owner));if(boundary==="before-active-lock-content-read"){await mkdir(artifact);await writeFile(path.join(artifact,"owner.json"),authorityCanonicalBytes(owner));}const before=await readFile(path.join(artifact,"owner.json"));let attempts=0,semanticClockCalls=0;const result=await new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{if(point===boundary){attempts++;throw Object.assign(new Error(code),{code});}}} as never).recover();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.ok(attempts>1,`${code} is retried to the bounded deadline`);assert.equal(semanticClockCalls,0);assert.deepEqual(await readFile(path.join(artifact,"owner.json")),before);}));
});
