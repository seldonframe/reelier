...
test("prep-only housekeeper revalidates one-use authority before mutation",async t=>{
  ...
  await t.test("handoff literals and token authority service names remain unexported",async()=>{const publicModule=authorityModule as Record<string,unknown>,packageJson=JSON.parse(await readFile(path.resolve("package.json"),"utf8")) as {exports:Record<string,unknown>},typeNames=["PrepCreatorToken","PrepRetirementAuthority","PrepRetiredCleanupAuthority"] as const;for(const name of [...typeNames,"__testServicePrepHousekeepingOnce"])assert.equal(name in publicModule,false);for(const literal of privateLiterals){assert.equal((ledgerFaultPoints as readonly string[]).includes(literal),false);assert.equal(containsPublicLiteral(publicModule,literal),false);assert.equal(containsPublicLiteral(packageJson.exports,literal),false);}assert.deepEqual(Object.keys(packageJson.exports).filter(name=>/(?:prep|housekeep|runtime)/i.test(name)),[]);/* The probe must live on the SAME VOLUME as the checkout. `path.relative` between different Windows
     ...
     it passed; Windows CI puts TEMP on C: and the workspace on D:, and it failed there every run.
...
test("atomic admission preparation states are non-authorizing and dead states drain via recover()",async t=>{
  for(const state of ["empty","zero","partial","complete"] as const){
    ...
    // Re-scoped to the shipped contract per the 2026-08-07 grant. This pin was written by d162ad7
    // (2026-08-03, "close atomic admission contract") asserting that observeClock RETIRES the dead
    // preparation itself. 3460808 (2026-08-05) implemented and pinned the opposite two days later —
    // a lock-seeking contender refuses `busy` and `recover()` drains — and that family is green.
    // No permission width reconciles them: granting reaches these four but breaks 17, dominated by
    // the very `warm … drains via recover()` tests that pin the newer rule. So this asserts the
    // shipped sequence, which is strictly stronger than what it replaced. The axis kept here and
    // absent from the warm family is a SYNTHETIC FOREIGN dead owner — a pid that never ran on this
    // host — where `crashChild` leaves a real local corpse.
    // Every ledger call runs inside the liveness mock: pid 47000+i almost certainly does not exist,
    // but "almost certainly" is not a fixture.
    await t.test(`dead-${state}`,()=>withRoot(async root=>{const owner={host:hostname(),nonce:(state==="empty"?"5":state==="zero"?"6":state==="partial"?"7":"8").repeat(64),pid:47000+["empty","zero","partial","complete"].indexOf(state),v:1 as const},prep=await writeAdmissionPrep(root,owner,state),originalKill=process.kill;let callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});let result,preservedPrep=false,recoveredOk=false,residue:readonly string[]=[],nextObserved;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock();preservedPrep=existsSync(prep);recoveredOk=(await new RawFsAuthorityLedger(root,{now:()=>t0+1_000,lockTimeoutMs:2_000} as never).recover()).ok;residue=(await readdir(root)).filter(name=>name.startsWith(".authority-ledger-admission-"));nextObserved=await new RawFsAuthorityLedger(root,{now:()=>t0+2_000,lockTimeoutMs:2_000} as never).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"},`dead-${state}: a lock-seeking contender refuses busy and never retires the dead preparation itself`);assert.equal(callbackEntries,0,`dead-${state}: and enters no callback`);assert.equal(preservedPrep,true,`dead-${state}: the dead preparation is preserved for recover()`);assert.equal(recoveredOk,true,`dead-${state}: recover() succeeds on the dead-owner root`);assert.deepEqual(residue,[],`dead-${state}: recover() drained every admission-family artifact`);assert.deepEqual(nextObserved,{ok:true,status:"advanced",observedAt:new Date(t0+2_000).toISOString()},`dead-${state}: the next default acquisition completes`);}));
  }
});
...
test("atomic admission slot classification gives corruption precedence and recovers only exact dead slots",async t=>{
  ...
  // Re-scoped to the shipped contract per the 2026-08-07 grant, for the same reason as the
  // `dead-${state}` preparations above: d162ad7 pinned observeClock retiring the dead slot itself,
  // 3460808 pinned the opposite two days later and shipped it. The seven sibling subtests here
  // carry this test's actual subject — corruption precedence over hardlink, reparse and replacement
  // — and are untouched; this is the one dead-slot drain case among them.
  await t.test("dead exact slot",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"a".repeat(64),pid:49000,v:1 as const};await writeAdmissionSlot(root,owner);const originalKill=process.kill;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});let result,recoveredOk=false,residue:readonly string[]=[],nextObserved;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200}).observeClock();recoveredOk=(await new RawFsAuthorityLedger(root,{now:()=>t0+1_000,lockTimeoutMs:2_000}).recover()).ok;residue=(await readdir(root)).filter(name=>name.startsWith(".authority-ledger-admission-"));nextObserved=await new RawFsAuthorityLedger(root,{now:()=>t0+2_000,lockTimeoutMs:2_000}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"},"a lock-seeking contender refuses busy and never retires the dead slot itself");assert.equal(recoveredOk,true,"recover() succeeds on the dead-owner slot root");assert.deepEqual(residue,[],"recover() drained every admission-family artifact");assert.deepEqual(nextObserved,{ok:true,status:"advanced",observedAt:new Date(t0+2_000).toISOString()},"the next default acquisition completes");}));
...
// Re-scoped per the 2026-08-07 grant, and ONLY its retirement half. The non-mutation assertions
// below already pass today and are kept verbatim: the zero-callback/zero-clock counts and the
// byte-and-topology identity of decisions, ingress, journal, transactions, claims and tombstones.
// That semantic-subtree check is unique in this suite — nothing else pins it — so retiring this
// test instead of re-scoping it would have silently dropped real coverage. What changed is only the
// expectation that observeClock retires the dead slot to its `abandoned` marker: 3460808 shipped
// the opposite rule, so the slot is now asserted preserved and recover() is asserted to drain it.
test("pre-admission housekeeper leaves one dead slot to recover() and mutates no semantic state",()=>withRoot(async root=>{
  ...
  assert.deepEqual({callbacks,semanticClockReads,prepCreates,publicationCreates},{callbacks:0,semanticClockReads:0,prepCreates:0,publicationCreates:0});assert.equal(existsSync(slot),true,"the dead slot is preserved for recover(), never retired by a lock-seeking contender");assert.equal(existsSync(path.join(root,admissionRetiredName(owner,"abandoned"))),false,"and no abandoned marker is minted, because no retirement was initiated");assert.deepEqual(await snapshotDurableSubtrees(root,semantic),before,"decisions, ingress, journal clock, transactions, claims, and tombstones retain exact bytes/topology");assert.deepEqual(await readJournalEvents(root),journalClockBefore,"semantic clock journal remains exact");
  // The other half of the shipped rule: what a lock-seeking contender declines, recover() completes.
  // Run under the same liveness mock — pid 49301 almost certainly does not exist, but a fixture may
  // not rest on "almost certainly".
  Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?(()=>{throw Object.assign(new Error("dead"),{code:"ESRCH"});})():originalKill.call(process,pid,0)});
  let drainedOk=false,residue:readonly string[]=[];
  try{drainedOk=(await new RawFsAuthorityLedger(root,{now:()=>t0+1_000,lockTimeoutMs:2_000}).recover()).ok;residue=(await readdir(root)).filter(name=>name.startsWith(".authority-ledger-admission-"));}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(drainedOk,true,"recover() succeeds on the preserved dead slot");
  assert.deepEqual(residue,[],"and drains every admission-family artifact the housekeeper left alone");
}));
...
test("rename collision retains one synced creator stage and fixed slot across replacement classes",async t=>{
  for(const mutation of ["byte-mutation","same-byte-identity-replacement","type-replacement"] as const)await t.test(mutation,()=>withRoot(async root=>{
    ...
    assert.deepEqual(result,{ok:false,reason:"corruption"},mutation);assert.deepEqual({stageCreates,collisions,publishedRenames,callbacks},{stageCreates:1,collisions:1,publishedRenames:0,callbacks:0},mutation);assert.equal((readdirSync(root).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).length),1,mutation);assert.equal(Boolean(stagePath)&&existsSync(stagePath),true,mutation);/* `type-replacement` swaps owner.json for a DIRECTORY. On POSIX that new subdirectory adds a `..`
       entry to its parent, so the STAGE's own nlink goes 2 -> 3; NTFS does not track directory links
       that way, which is why this passed on Windows and failed on Linux CI with nlink 3n vs 2n. The
       stage is still the same object — dev, ino and mode are unchanged — so account for the link the
       mutation legitimately adds rather than asserting the platforms agree. Spec :240 records the
       same asymmetry for a freshly created directory. */
    const expectedStageIdentity=mutation==="type-replacement"&&process.platform!=="win32"&&stageIdentity!==undefined?{...stageIdentity,nlink:stageIdentity.nlink+1n}:stageIdentity;
    assert.deepEqual(exactFsIdentity(stagePath),expectedStageIdentity,`${mutation}: same stage identity`);assert.equal(existsSync(slotPath),true,`${mutation}: fixed slot remains`);assert.deepEqual(exactFsIdentity(slotPath),slotIdentity,`${mutation}: same fixed slot identity`);const ownerPath=path.join(stagePath,"owner.json");if(mutation==="byte-mutation")assert.deepEqual(readFileSync(ownerPath),replacement);else if(mutation==="same-byte-identity-replacement")assert.deepEqual(readFileSync(ownerPath),originalOwnerBytes);else assert.equal(lstatSync(ownerPath).isDirectory(),true);
...
// Every dead-owner fixture in this file rests on one assumption: that the platform will actually
// report the returned pid DEAD. `close` only says the child was reaped by us, not that the pid now
// reads as gone — Windows reuses pids aggressively, and a busy runner can hand the number straight
// to something else. When that happens the ledger sees a live owner and refuses `busy`, which is
// indistinguishable from a real refusal at the assertion.
//
// That is not hypothetical. `marker-only` passed 200/200 locally and failed on Windows CI at
// `0ab1a30` with exactly `{ok:false,reason:"busy"}` — the shape a live owner produces. So verify
// instead of assuming, and fail loudly by name rather than as a mystery `busy` downstream.
//
// The residual race is real and unclosable: a pid verified dead here can be reused before the
// ledger probes it. Verifying shrinks the window; it does not remove it.
function pidReportsDead(pid:number):boolean{try{process.kill(pid,0);return false;}catch(error){return (error as NodeJS.ErrnoException).code==="ESRCH";}}
async function spawnAndReapPid():Promise<number>{return new Promise<number>((resolve,reject)=>{const child=spawn(process.execPath,["-e",""]);const pid=child.pid;child.once("error",reject);child.once("close",()=>{assert.ok(pid);resolve(pid);});});}
async function exitedProcessPid():Promise<number>{
  const rejected:number[]=[];
  for(let attempt=0;attempt<16;attempt++){const pid=await spawnAndReapPid();if(pidReportsDead(pid))return pid;rejected.push(pid);}
  throw new Error(`no reaped pid reported dead after 16 attempts (saw ${rejected.join(",")}). This host reuses or retains pids fast enough that dead-owner fixtures cannot be built from exited children.`);
}
...
test("option-gated acquisitions leave a root that every entry point can still use",()=>withRoot(async root=>{
  ...
  // `directorySync` is platform-derived, not constant: the ledger reports "best-effort" only on
  // win32 and "verified" everywhere else. This pin hard-coded the win32 answer, so it passed on the
  // machine it was written on and failed on Linux CI with `verified`. Same expression as :852.
  assert.deepEqual(await byDefault(at+1_000).recover(),{ok:true,reservations:[],highWaterMark:new Date(at).toISOString(),topology:{directorySync:process.platform==="win32"?"best-effort":"verified"}},"recover() works on the reused root and sees the clock the option-on acquisition wrote");
