...
test("provisional invalidation permanently falls through to staged settlement in the same iteration",async t=>{
  ...
  for(const kind of cases)await t.test(kind,()=>withRoot(async root=>{const external=await tempRoot();try{
    ...
    let ownStage="",ownBytes=Buffer.alloc(0),ownIdentity:Readonly<{dev:bigint;ino:bigint}>|undefined,predecessorIdentity:Readonly<{dev:bigint;ino:bigint}>|undefined,selection=0,provisionalRoot=0,provisionalLiveness=0,stagedBoundaries=0,delayCount=0,firstFallbackDelay=-1,removeAttempts=0,publicationRenames=0,callbackEntries=0,mutated=false;
    ...
    const mutateMembership=()=>{mutated=true;if(kind==="membership-add"){mkdirSync(addedStage);writeFileSync(path.join(addedStage,"owner.json"),addedBytes);livePids.add(added.pid);}else if(kind==="membership-remove")rmSync(distantStage,{recursive:true});else if(kind==="malformed-name")mkdirSync(malformedStage);else if(kind==="foreign-provenance"){mkdirSync(foreignStage);writeFileSync(path.join(foreignStage,"owner.json"),foreignBytes);}else if(kind==="duplicate-provenance"){mkdirSync(duplicateStage);writeFileSync(path.join(duplicateStage,"owner.json"),duplicateBytes);}};
    ...
    assert.equal(mutated,true,`${kind} reaches its intended provisional invalidation branch`);assert.deepEqual(observed!.result,expected);assert.equal(selection,1,"provisional mode never re-enters after fallback");assert.equal(stagedBoundaries>=1,true);assert.equal(firstFallbackDelay,0,"fallback occurs in the same iteration before acquisition backoff");assert.deepEqual({publicationRenames,callbackEntries},{publicationRenames:0,callbackEntries:0});
    ...
    assert.equal(removeAttempts,kind==="predecessor-dead"?1:0);if(kind==="own-missing")assert.equal(existsSync(ownStage),false);if(kind==="own-replaced"){assert.equal(existsSync(ownStage),true);assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);assert.notDeepEqual(identity(ownStage),ownIdentity);}else if(existsSync(ownStage)){assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);assert.deepEqual(identity(ownStage),ownIdentity);}if(kind==="predecessor-missing")assert.equal(existsSync(predecessorStage),false);if(kind==="predecessor-replaced"){assert.equal(existsSync(predecessorStage),true);assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.notDeepEqual(identity(predecessorStage),predecessorIdentity);}else if(kind==="predecessor-dead")assert.equal(existsSync(predecessorStage),false);else{assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.deepEqual(identity(predecessorStage),predecessorIdentity);}if(kind==="membership-add")assert.deepEqual(readFileSync(path.join(addedStage,"owner.json")),addedBytes);if(kind==="membership-remove")assert.equal(existsSync(distantStage),false);else assert.deepEqual(readFileSync(path.join(distantStage,"owner.json")),distantBytes);if(kind==="malformed-name")assert.equal(existsSync(malformedStage),true);if(kind==="foreign-provenance")assert.deepEqual(readFileSync(path.join(foreignStage,"owner.json")),foreignBytes);if(kind==="duplicate-provenance")assert.deepEqual(readFileSync(path.join(duplicateStage,"owner.json")),duplicateBytes);
...
test("malformed membership added during provisional close is corruption, never busy",()=>withRoot(async root=>{
  const predecessor={host:hostname(),nonce:"b".repeat(64),pid:41,v:1 as const,ticket:"0000000000000002"},predecessorStage=path.join(root,publicationStageName(predecessor)),malformed=path.join(root,".authority-ledger-lock-publication-malformed"),originalKill=process.kill;let ownStage="",added=false,stagedBoundaries=0,publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>3n,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&ownStage===""){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),publicationOwnerBytes(predecessor));}if(point==="before-lock-publication-provisional-root-reenumeration"&&!added){added=true;mkdirSync(malformed);}if(point==="before-staged-publication-settlement")stagedBoundaries++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
  assert.equal(added,true);assert.ok(stagedBoundaries>=1);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(existsSync(malformed),true);assert.equal(existsSync(predecessorStage),true);assert.deepEqual({publicationRenames,callbackEntries},{publicationRenames:0,callbackEntries:0});
}));
