...
test("pre-publication same-name foreign stage identity churn remains corruption",()=>withRoot(async root=>{const external=await tempRoot(),owner={host:hostname(),nonce:"9".repeat(64),pid:process.pid,v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes);let attempts=0,callbackEntries=0,semanticClockCalls=0,finalIno=lstatSync(stage,{bigint:true}).ino;try{const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>{semanticClockCalls++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point!=="after-lock-publication-generation-closed")return;attempts++;const replacement=path.join(external,`replacement-${attempts}`);mkdirSync(replacement);writeFileSync(path.join(replacement,"owner.json"),bytes);const previousIno=lstatSync(stage,{bigint:true}).ino;renameSync(stage,path.join(external,`displaced-${attempts}`));renameSync(replacement,stage);finalIno=lstatSync(stage,{bigint:true}).ino;assert.notEqual(finalIno,previousIno,"fixture atomically replaces the foreign stage identity while preserving canonical bytes");}} as never).recover());assert.equal(attempts,21);assert.deepEqual(observed.delays,Array(20).fill(5));assert.equal(observed.elapsedMs,100);assert.equal(callbackEntries,0);assert.equal(semanticClockCalls,0);assert.deepEqual(await snapshotRootArtifacts(root),[{name:path.basename(stage),type:"directory"},{name:path.posix.join(path.basename(stage),"owner.json"),type:"file",bytes:bytes.toString("base64")}]);assert.equal(lstatSync(stage,{bigint:true}).ino,finalIno);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);assert.deepEqual(observed.result,{ok:false,reason:"corruption"});}finally{await rm(external,{recursive:true,force:true});}}));
