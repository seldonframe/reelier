...
import { existsSync,linkSync,lstatSync,mkdirSync,readFileSync,readdirSync,renameSync,rmSync,writeFileSync } from "node:fs";
...
test("publication revalidates exact owner bytes before rename and after every published boundary",async t=>{
  for(const point of ["after-lock-publication-owner-sync","after-lock-publication-stage-sync","after-lock-publication-rename","after-lock-publication-root-sync"] as const)for(const mutation of ["replace","truncate"] as const)await t.test(`${point}:${mutation}`,()=>withRoot(async root=>{let changed=false,semanticNowCalls=0,target="",untrusted=Buffer.alloc(0);const preRename=point==="after-lock-publication-owner-sync"||point==="after-lock-publication-stage-sync",ledger=new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(observed:string)=>{if(changed||observed!==point)return;changed=true;const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));target=preRename?path.join(root,stage!,"owner.json"):path.join(root,"lock","owner.json");const original=readFileSync(target);untrusted=mutation==="replace"?Buffer.from("untrusted-owner-bytes"):original.subarray(0,Math.max(1,Math.floor(original.length/2)));writeFileSync(target,untrusted);}} as never),result=await ledger.observeClock();assert.equal(changed,true);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(semanticNowCalls,0,"operation callback never starts after owner mismatch");assert.deepEqual(await readFile(target),untrusted);if(preRename){assert.equal(existsSync(path.join(root,"lock")),false);assert.ok(path.basename(path.dirname(target)).startsWith(".authority-ledger-lock-publication-"));}else assert.equal(existsSync(path.join(root,"lock")),true);}));
});
...
test("creator cleanup freezes publication object identity before removing owned artifacts",async t=>{
  ...
  for(const state of ownerReplacementStates)await t.test(`owner-inode:${state}`,()=>withRoot(async root=>{const external=await tempRoot();let semanticNowCalls=0,replacementBytes:Buffer<ArrayBufferLike>=Buffer.alloc(0),originalIno=-1,replacementIno=-1,stagePath="";try{const point="after-lock-publication-stage-sync",ledger=new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(observed:string)=>{if(observed!==point)return;const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(stage);stagePath=path.join(root,stage);const ownerPath=path.join(stagePath,"owner.json"),originalBytes=readFileSync(ownerPath),replacementPath=path.join(root,"replacement-owner");originalIno=lstatSync(ownerPath).ino;replacementBytes=state==="zero"?Buffer.alloc(0):state==="prefix"?originalBytes.subarray(0,Math.floor(originalBytes.length/2)):state==="complete-same-bytes"?originalBytes:authorityCanonicalBytes({replacement:true});writeFileSync(replacementPath,replacementBytes);replacementIno=lstatSync(replacementPath).ino;assert.notEqual(replacementIno,originalIno,"fixture must replace the owner object, not write it in place");renameSync(replacementPath,ownerPath);assert.equal(lstatSync(ownerPath).ino,replacementIno);throw new Error(`fault:${point}`);}} as never);await assert.rejects(()=>ledger.recover(),/fault:after-lock-publication-stage-sync/);assert.equal(semanticNowCalls,0);assert.equal(existsSync(stagePath),true);assert.equal(lstatSync(path.join(stagePath,"owner.json")).ino,replacementIno);assert.deepEqual(await readFile(path.join(stagePath,"owner.json")),replacementBytes);}finally{await rm(external,{recursive:true,force:true});}}));
  const stageStates=["empty","zero","prefix","complete"] as const;
  for(const state of stageStates)await t.test(`stage-directory:${state}`,()=>withRoot(async root=>{const external=await tempRoot();let semanticNowCalls=0,stagePath="",replacementIno=-1,ownerBytes:Buffer<ArrayBufferLike>|null=null;try{const point="after-lock-publication-stage-sync",ledger=new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(observed:string)=>{if(observed!==point)return;const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(stage);stagePath=path.join(root,stage);const originalIno=lstatSync(stagePath).ino,replacementPath=path.join(root,"replacement-stage"),displacedPath=path.join(external,"original-stage"),completeBytes=readFileSync(path.join(stagePath,"owner.json"));mkdirSync(replacementPath);ownerBytes=state==="empty"?null:state==="zero"?Buffer.alloc(0):state==="prefix"?completeBytes.subarray(0,Math.floor(completeBytes.length/2)):completeBytes;if(ownerBytes!==null)writeFileSync(path.join(replacementPath,"owner.json"),ownerBytes);replacementIno=lstatSync(replacementPath).ino;assert.notEqual(replacementIno,originalIno,"fixture must replace the stage directory object");renameSync(stagePath,displacedPath);renameSync(replacementPath,stagePath);assert.equal(lstatSync(stagePath).ino,replacementIno);throw new Error(`fault:${point}`);}} as never);await assert.rejects(()=>ledger.recover(),/fault:after-lock-publication-stage-sync/);assert.equal(semanticNowCalls,0);assert.equal(existsSync(stagePath),true);assert.equal(lstatSync(stagePath).ino,replacementIno);const entries=await readdir(stagePath);if(ownerBytes===null)assert.deepEqual(entries,[]);else{assert.deepEqual(entries,["owner.json"]);assert.deepEqual(await readFile(path.join(stagePath,"owner.json")),ownerBytes);}}finally{await rm(external,{recursive:true,force:true});}}));
  for(const [state,point] of ownerStates)await t.test(`owner-link-count:${state}`,()=>withRoot(async root=>{const external=await tempRoot();let semanticNowCalls=0,stagePath="",ownerBytes=Buffer.alloc(0);try{const ledger=new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(observed:string)=>{if(observed!==point)return;const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(stage);stagePath=path.join(root,stage);const ownerPath=path.join(stagePath,"owner.json"),externalLink=path.join(external,"owner-hardlink");ownerBytes=readFileSync(ownerPath);linkSync(ownerPath,externalLink);assert.equal(lstatSync(ownerPath).nlink,2);throw new Error(`fault:${point}`);}} as never);await assert.rejects(()=>ledger.recover(),new RegExp(`fault:${point}`));assert.equal(semanticNowCalls,0);assert.equal(existsSync(stagePath),true);assert.equal(lstatSync(path.join(stagePath,"owner.json")).nlink,2);assert.deepEqual(await readFile(path.join(stagePath,"owner.json")),ownerBytes);assert.deepEqual(await readFile(path.join(external,"owner-hardlink")),ownerBytes);}finally{await rm(external,{recursive:true,force:true});}}));
});

test("lock and publication enumeration races restart the whole bounded snapshot",async t=>{
  await t.test("active lock generation changes after metadata",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"1".repeat(64),pid:process.pid,v:1 as const},directory=path.join(root,"lock"),blockerOwner={host:hostname(),nonce:"5".repeat(64),pid:process.pid,v:1 as const},blocker=path.join(root,publicationStageName(blockerOwner)),blockerBytes=Buffer.from("not-canonical-owner");await mkdir(directory);await writeFile(path.join(directory,"owner.json"),authorityCanonicalBytes(owner));let fired=false,semanticNowCalls=0;const result=await new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{if(!fired&&point==="after-active-lock-metadata"){fired=true;rmSync(directory,{recursive:true});mkdirSync(blocker);writeFileSync(path.join(blocker,"owner.json"),blockerBytes);}}} as never).recover();assert.equal(fired,true);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(semanticNowCalls,0);assert.equal(existsSync(blocker),true);assert.deepEqual(await readFile(path.join(blocker,"owner.json")),blockerBytes);}));
  await t.test("publication root generation changes after enumeration",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"2".repeat(64),pid:await exitedChildPid(),v:1 as const},stage=await writePublicationStage(root,owner,authorityCanonicalBytes(owner)),blockerOwner={host:hostname(),nonce:"6".repeat(64),pid:process.pid,v:1 as const},blocker=path.join(root,publicationStageName(blockerOwner)),blockerBytes=Buffer.from("not-canonical-owner");let fired=false,semanticNowCalls=0;const result=await new RawFsAuthorityLedger(root,{now:()=>{semanticNowCalls++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{if(!fired&&point==="after-publication-stage-enumeration"){fired=true;rmSync(stage,{recursive:true});mkdirSync(blocker);writeFileSync(path.join(blocker,"owner.json"),blockerBytes);}}} as never).recover();assert.equal(fired,true);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(semanticNowCalls,0);assert.equal(existsSync(blocker),true);assert.deepEqual(await readFile(path.join(blocker,"owner.json")),blockerBytes);}));
});
...
test("new ingress, clock, and ledger-lock fault-point sets are complete and disjoint", () => {
  ...
  assert.deepEqual(ledgerLockFaultPoints, [...publicationCrashPoints,...publicationSnapshotFaultPoints,...ledgerLockDurabilityPoints]);
