...
test("a safely closed live generation admits one unique mutating contender stage",()=>withRoot(async root=>{const predecessor={host:hostname(),nonce:"8".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=await writePublicationStage(root,predecessor,predecessorBytes),authenticated=authenticateOutcomeRequest({tenant:"tenant_1",requester:"requester_1",definitionAlias:"definition_1",request:{v:"reelier.outcome-request/v1",requestId:"request_1",sourceRefs:{source:"ref_1"},choices:{}}}),originalKill=process.kill;let stageCreates=0,ownStage="",ownBytes=Buffer.alloc(0),publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let result;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:30,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create"){stageCreates++;const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")&&name!==path.basename(predecessorStage));assert.ok(ownName);ownStage=path.join(root,ownName);}if(point==="after-lock-publication-stage-sync"&&ownStage)ownBytes=readFileSync(path.join(ownStage,"owner.json"));if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).bindIngress(authenticated);}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"});assert.equal(stageCreates,1);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);assert.ok(path.basename(predecessorStage)<path.basename(ownStage));assert.deepEqual((await readdir(root)).sort(),[path.basename(predecessorStage),path.basename(ownStage)].sort());assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);}));

test("a mixed closed generation removes only exact dead stages before waiting on live peers",()=>withRoot(async root=>{const deadOwner={host:hostname(),nonce:"9".repeat(64),pid:await exitedChildPid(),v:1 as const},liveOwner={host:hostname(),nonce:"a".repeat(64),pid:1,v:1 as const},deadBytes=authorityCanonicalBytes(deadOwner),liveBytes=authorityCanonicalBytes(liveOwner),deadStage=await writePublicationStage(root,deadOwner,deadBytes),liveStage=await writePublicationStage(root,liveOwner,liveBytes),originalKill=process.kill;let removeAttempts=0,generationClosures=0,publicationHooks=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===deadOwner.pid)throw Object.assign(new Error("dead"),{code:"ESRCH"});if(pid===liveOwner.pid)return true;return originalKill.call(process,pid,0);}});let result;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:30,faultInjector:(point:string)=>{if(point==="before-publication-stage-remove-attempt")removeAttempts++;if(point==="after-lock-publication-generation-closed")generationClosures++;if((publicationCrashPoints as readonly string[]).includes(point))publicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).recover();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"});assert.equal(removeAttempts,1,"one exact dead artifact creates one root-sync obligation");assert.equal(generationClosures,2,"the synced post-removal generation is closed once before waiting");assert.equal(publicationHooks,0);assert.equal(callbackEntries,0);assert.deepEqual((await readdir(root)),[path.basename(liveStage)]);assert.equal(existsSync(deadStage),false);assert.deepEqual(readFileSync(path.join(liveStage,"owner.json")),liveBytes);}));
