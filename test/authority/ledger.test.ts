...
const legacyMutationBoundaries=new Set([
  ...
  "before-publication-stage-remove-attempt","after-publication-stage-cleanup-root-sync","before-creator-withdrawal-seal","after-creator-withdrawal-root-sync",
...
test("hybrid epoch guard classifies K1 before every legacy compatibility mutation",async t=>{
  ...
  assert.deepEqual(Object.keys(publicAuthority).filter(name=>name.startsWith("__test")&&/(?:k1|epoch|root.?generation)/i.test(name)),[],"the migration guard requires no new public classifier export");
  const cases=[
