...
test("publication admission tickets allocate after the visible maximum without changing owner authority",async()=>{
  await withRoot(async root=>{
    ...
    try{
      const ledger=rawLedgerWithAdmissionClock(root,()=>0xffn,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(name=>name!==path.basename(earlierStage)&&name.startsWith(".authority-ledger-lock-publication-"))??"";}});
    ...
    assert.equal(callbackEntries,0);assert.match(created,new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-0000000000000100-${process.pid}-[0-9a-f]{64}\\.tmp$`));assert.ok(path.basename(earlierStage)<created);assert.deepEqual(await readFile(path.join(earlierStage,"owner.json")),earlierBytes);const ownOwner=JSON.parse(await readFile(path.join(root,created,"owner.json"),"utf8"));assert.deepEqual(Object.keys(ownOwner).sort(),["host","nonce","pid","v"]);
  });
});
...
test("same-ticket distinct publishers classify by decimal PID text and never by nonce",async()=>{
  await withRoot(async root=>{
    const first={host:hostname(),nonce:"a".repeat(64),pid:11,v:1 as const,ticket:"0000000000000009"},second={host:hostname(),nonce:"0".repeat(64),pid:2,v:1 as const,ticket:"0000000000000009"},firstStage=await writePublicationStage(root,first,authorityCanonicalBytes({host:first.host,nonce:first.nonce,pid:first.pid,v:first.v})),secondStage=await writePublicationStage(root,second,authorityCanonicalBytes({host:second.host,nonce:second.nonce,pid:second.pid,v:second.v})),originalKill=process.kill,probes:number[]=[];
    Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>{if(pid===first.pid||pid===second.pid){probes.push(pid);return true;}return originalKill.call(process,pid,0);}});
    try{assert.deepEqual(await rawLedgerWithAdmissionClock(root,()=>0n,{now:()=>t0,lockTimeoutMs:20}).recover(),{ok:false,reason:"busy"});}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.deepEqual(probes.slice(0,2),[11,2]);assert.ok(path.basename(firstStage)<path.basename(secondStage));
  });
});
