...
test("provisional invalidation permanently falls through to staged settlement in the same iteration",async t=>{
  ...
  for(const kind of cases)await t.test(kind,()=>withRoot(async root=>{const external=await tempRoot();try{
    ...
    assert.equal(mutated,true,`${kind} reaches its intended provisional invalidation branch`);assert.deepEqual(observed!.result,expected);assert.equal(selection,1,"provisional mode never re-enters after fallback");assert.equal(stagedBoundaries,1);assert.equal(firstFallbackDelay,0,"fallback occurs in the same iteration before acquisition backoff");assert.deepEqual({publicationRenames,callbackEntries},{publicationRenames:0,callbackEntries:0});
