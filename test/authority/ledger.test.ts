...
test("publication admission tickets allocate after the visible maximum without changing owner authority",async()=>{
  await withRoot(async root=>{
    ...
    let admissionSamples=0,callbackEntries=0,publicationRenames=0,created="",createdBytes=Buffer.alloc(0);
    ...
    try{
      const ledger=rawLedgerWithAdmissionClock(root,()=>{admissionSamples++;return 0xffn;},{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(name=>name!==path.basename(earlierStage)&&name.startsWith(".authority-ledger-lock-publication-"))??"";if(point==="after-lock-publication-stage-sync")createdBytes=readFileSync(path.join(root,created,"owner.json"));}});
      assert.deepEqual(await ledger.observeClock(),{ok:false,reason:"busy"});
    }finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}
    assert.equal(admissionSamples,1);assert.equal(callbackEntries,0);assert.equal(publicationRenames,0);assert.match(created,new RegExp(`^\\.authority-ledger-lock-publication-${publicationHostDigest(hostname())}-0000000000000100-${process.pid}-[0-9a-f]{64}\\.tmp$`));assert.ok(path.basename(earlierStage)<created);assert.deepEqual(await readFile(path.join(earlierStage,"owner.json")),earlierBytes);const ownOwner=JSON.parse(createdBytes.toString("utf8"));assert.deepEqual(Object.keys(ownOwner).sort(),["host","nonce","pid","v"]);assert.deepEqual(createdBytes,publicationOwnerBytes(ownOwner));assert.deepEqual(await readFile(path.join(root,created,"owner.json")),createdBytes);assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),[path.basename(earlierStage),created].sort());
  });
});
...
test("publication admission allocates after one visible maximum regardless of publisher suffix order",()=>withRoot(async root=>{
  ...
  const peerBytes=publicationOwnerBytes(peer),peerStage=await writePublicationStage(root,peer,peerBytes),originalKill=process.kill;
  let created="",samples=0,semanticClockReads=0,callbackEntries=0;
  ...
  assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),[path.basename(peerStage),created].sort());
}));
...
test("dead MAX is removed and root-synced before a fresh ticket is allocated",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"7".repeat(64),pid:await exitedChildPid(),v:1 as const,ticket:"ffffffffffffffff"},bytes=publicationOwnerBytes(owner),stage=await writePublicationStage(root,owner,bytes),order:string[]=[];let samples=0,created="";const ledger=rawLedgerWithAdmissionClock(root,()=>{samples++;order.push("sample");return 0n;},{now:()=>t0,faultInjector:(point:string)=>{if(point==="before-publication-stage-remove-attempt"){if(existsSync(stage))order.push("foreign-remove");else if(created&&existsSync(path.join(root,created)))order.push("creator-remove");else order.push("unexpected-remove");}if(point==="after-publication-stage-cleanup-root-sync")order.push("foreign-root-sync");if(point==="after-lock-publication-stage-create"){order.push("create");created=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"))??"";throw new Error("captured-dead-max-successor");}}});await assert.rejects(()=>ledger.observeClock(),/captured-dead-max-successor/);assert.equal(samples,1);assert.deepEqual(order,["foreign-remove","foreign-root-sync","sample","create","creator-remove"]);assert.equal(existsSync(stage),false);assert.match(created,/-0000000000000001-/);assert.equal(existsSync(path.join(root,created)),false);}));
...
test("an invalidated mixed generation removes dead MAX before ticket allocation",()=>withRoot(async root=>{
  ...
  try{
    observed=await withRecordedDelays(()=>rawLedgerWithAdmissionClock(root,()=>{samples++;order.push("sample");return 0n;},{now:()=>{semanticClockReads++;return t0;},lockTimeoutMs:100,faultInjector:(point:string)=>{
      ...
      if(point==="before-publication-stage-remove-attempt"){assert.equal(existsSync(deadStage),true);order.push("dead-remove");}
      if(point==="after-publication-stage-cleanup-root-sync")order.push("dead-root-sync");
      ...
      if(point==="after-lock-publication-stage-sync"&&creatorCreates>0)creatorSynced=true;
  ...
  assert.deepEqual({result:observed!.result,enumerations,order,samples,semanticClockReads,callbackEntries,creatorCreates,stagedBoundariesAfterCreator,publicationRenames,deadMaxPresent:existsSync(deadStage),temporaryPresent:existsSync(temporaryStage),livePresent:existsSync(liveStage),creatorTicketTwo},{result:{ok:false,reason:"busy"},enumerations:4,order:["add-temporary","remove-temporary","dead-remove","dead-root-sync","sample","creator-create"],samples:1,semanticClockReads:0,callbackEntries:0,creatorCreates:1,stagedBoundariesAfterCreator:0,publicationRenames:0,deadMaxPresent:false,temporaryPresent:false,livePresent:true,creatorTicketTwo:true});
  assert.ok(provisionalSelections>=1,"the post-create lower-live cohort remains in denial-only provisional waiting");
  assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(callbackEntries,0);assert.equal(creatorCreates,1);assert.equal(publicationRenames,0);
  assert.equal(existsSync(deadStage),false);assert.equal(existsSync(temporaryStage),false);assert.equal(existsSync(liveStage),true);assert.deepEqual(await readFile(path.join(liveStage,"owner.json")),liveBytes);
  ...
  const creatorPath=path.join(root,creatorStage),creatorBytes=await readFile(path.join(creatorPath,"owner.json")),creatorOwner=JSON.parse(creatorBytes.toString("utf8")) as {host:string;nonce:string;pid:number;v:1};
  ...
  assert.deepEqual((await readdir(root)).filter(name=>name.startsWith(".authority-ledger-lock-publication-")).sort(),[path.basename(liveStage),creatorStage].sort());
}));
