...
test("a collision-retained creator stage is revalidated before every rename retry",async t=>{for(const mutation of ["identity","type","bytes"] as const)await t.test(mutation,()=>withRoot(async root=>{let callbackEntries=0,collisions=0,stagePath="",replacementIno=0n;const blockerOwner={host:hostname(),nonce:"8".repeat(64),pid:process.pid,v:1 as const};const result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200,faultInjector:(point:string)=>{if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="after-lock-publication-stage-sync"&&!stagePath){const stage=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(stage);stagePath=path.join(root,stage);mkdirSync(path.join(root,"lock"));writeFileSync(path.join(root,"lock","owner.json"),authorityCanonicalBytes(blockerOwner));}if(point!=="after-lock-publication-rename-collision")return;collisions++;assert.equal(collisions,1);const ownerBytes=readFileSync(path.join(stagePath,"owner.json"));if(mutation==="identity"){const replacement=path.join(root,"replacement-publication-stage");mkdirSync(replacement);writeFileSync(path.join(replacement,"owner.json"),ownerBytes);rmSync(stagePath,{recursive:true});renameSync(replacement,stagePath);replacementIno=lstatSync(stagePath,{bigint:true}).ino;}else if(mutation==="type"){rmSync(stagePath,{recursive:true});writeFileSync(stagePath,"replacement-stage-file");}else writeFileSync(path.join(stagePath,"owner.json"),"replacement-owner-bytes");rmSync(path.join(root,"lock"),{recursive:true});}} as never).observeClock();assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackEntries,0);assert.equal(collisions,1);assert.equal(existsSync(path.join(root,"lock")),false,"unvalidated replacement never crosses the rename boundary");assert.equal(existsSync(stagePath),true,"replacement remains at its stage path");if(mutation==="identity")assert.equal(lstatSync(stagePath,{bigint:true}).ino,replacementIno);else if(mutation==="type")assert.deepEqual(readFileSync(stagePath),Buffer.from("replacement-stage-file"));else assert.deepEqual(readFileSync(path.join(stagePath,"owner.json")),Buffer.from("replacement-owner-bytes"));}));});

test("a live publication generation is closed before callback entry",()=>withRoot(async root=>{const livePid=55555,owner={host:hostname(),nonce:"7".repeat(64),pid:livePid,v:1 as const},bytes=authorityCanonicalBytes(owner),stage=path.join(root,publicationStageName(owner)),replacement=Buffer.from("mutated-live-owner"),originalKill=process.kill;let callbackEntries=0,mutated=false;Object.defineProperty(process,"kill",{configurable:true,value:(observedPid:number)=>{if(observedPid!==livePid)return originalKill.call(process,observedPid,0);if(!mutated){mutated=true;writeFileSync(path.join(stage,"owner.json"),replacement);}return true;}});let result;try{result=await new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-root-sync"){mkdirSync(stage);writeFileSync(path.join(stage,"owner.json"),bytes);}if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.equal(mutated,true);assert.deepEqual(result,{ok:false,reason:"corruption"});assert.equal(callbackEntries,0);assert.equal(existsSync(stage),true);assert.deepEqual(readFileSync(path.join(stage,"owner.json")),replacement);}));
