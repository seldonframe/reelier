...
test("publication admission uses the exact visible maximum independently of PID order",()=>withRoot(async root=>{const first={host:hostname(),nonce:"3".repeat(64),pid:3,v:1 as const,ticket:"000000000000000a"},second={host:hostname(),nonce:"4".repeat(64),pid:20,v:1 as const,ticket:"000000000000000b"},firstBytes=publicationOwnerBytes(first),secondBytes=publicationOwnerBytes(second),firstStage=await writePublicationStage(root,first,firstBytes),secondStage=await writePublicationStage(root,second,secondBytes),originalKill=process.kill;assert.ok(String(first.pid)>String(second.pid),"ticket order and decimal PID-text order genuinely disagree");assert.ok(path.basename(firstStage)<path.basename(secondStage),"ticket order controls the first wave despite the reversed PID-text order");let created="",samples=0,semanticClockReads=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===first.pid||pid===second.pid?true:originalKill.call(process,pid,0)});let result;try{result=await rawLedgerWithAdmissionClock(root,()=>{samples++;return 1n;},{now:()=>{semanticClockReads++;return t0;},lockTimeoutMs:20,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-create")created=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-")&&name!==path.basename(firstStage)&&name!==path.basename(secondStage))??"";if(point==="before-ledger-operation-callback")callbackEntries++;}}).observeClock();}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(result,{ok:false,reason:"busy"});assert.equal(samples,1);assert.equal(semanticClockReads,0);assert.equal(callbackEntries,0);assert.match(created,/-000000000000000c-/);assert.ok(path.basename(firstStage)<path.basename(secondStage)&&path.basename(secondStage)<created);assert.deepEqual(readFileSync(path.join(firstStage,"owner.json")),firstBytes);assert.deepEqual(readFileSync(path.join(secondStage,"owner.json")),secondBytes);}));
