...
async function withRecordedDelays<T>(operation:()=>Promise<T>):Promise<Readonly<{result:T;delays:number[];elapsedMs:number}>>{const original=setTimeout,delays:number[]=[],started=process.hrtime.bigint(),replacement=((...args:Parameters<typeof setTimeout>)=>{delays.push(Number(args[1]??0));return original(...args);}) as typeof setTimeout;Object.defineProperty(globalThis,"setTimeout",{configurable:true,writable:true,value:replacement});try{const result=await operation();return {result,delays,elapsedMs:Number(process.hrtime.bigint()-started)/1e6};}finally{Object.defineProperty(globalThis,"setTimeout",{configurable:true,writable:true,value:original});}}
...
test("stable non-head predecessor polling uses deterministic capped monotonic backoff",()=>withRoot(async root=>{const predecessor={host:hostname(),nonce:"5".repeat(64),pid:1,v:1 as const},predecessorBytes=authorityCanonicalBytes(predecessor),predecessorStage=path.join(root,publicationStageName(predecessor)),originalKill=process.kill;let ownStage="",ownBytes=Buffer.alloc(0),predecessorPolls=0,publicationRenames=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===predecessor.pid?true:originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200,faultInjector:(point:string)=>{if(point==="after-lock-publication-stage-sync"&&!ownStage){const ownName=readdirSync(root).find(name=>name.startsWith(".authority-ledger-lock-publication-"));assert.ok(ownName);ownStage=path.join(root,ownName);ownBytes=readFileSync(path.join(ownStage,"owner.json"));mkdirSync(predecessorStage);writeFileSync(path.join(predecessorStage,"owner.json"),predecessorBytes);}if(point==="before-lock-publication-predecessor-validation")predecessorPolls++;if(point==="after-lock-publication-rename")publicationRenames++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).observeClock());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.deepEqual(observed!.delays.slice(0,5),[5,10,20,40,50]);assert.ok(observed!.delays.slice(5).every(value=>value===50));assert.ok(observed!.delays.length<=8);assert.equal(predecessorPolls,observed!.delays.length);assert.ok(observed!.elapsedMs<5_000);assert.equal(publicationRenames,0);assert.equal(callbackEntries,0);assert.deepEqual((await readdir(root)).sort(),[path.basename(predecessorStage),path.basename(ownStage)].sort());assert.deepEqual(readFileSync(path.join(predecessorStage,"owner.json")),predecessorBytes);assert.deepEqual(readFileSync(path.join(ownStage,"owner.json")),ownBytes);}));

test("valid live active-lock waiting uses deterministic capped monotonic backoff",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"6".repeat(64),pid:1,v:1 as const},ownerBytes=authorityCanonicalBytes(owner),lock=path.join(root,"lock"),originalKill=process.kill;await mkdir(lock);await writeFile(path.join(lock,"owner.json"),ownerBytes);let snapshots=0,publicationHooks=0,callbackEntries=0;Object.defineProperty(process,"kill",{configurable:true,value:(pid:number)=>pid===owner.pid?true:originalKill.call(process,pid,0)});let observed;try{observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:200,faultInjector:(point:string)=>{if(point==="after-active-lock-metadata")snapshots++;if((publicationCrashPoints as readonly string[]).includes(point))publicationHooks++;if(point==="before-ledger-operation-callback")callbackEntries++;}} as never).recover());}finally{Object.defineProperty(process,"kill",{configurable:true,value:originalKill});}assert.deepEqual(observed!.result,{ok:false,reason:"busy"});assert.deepEqual(observed!.delays.slice(0,5),[5,10,20,40,50]);assert.ok(observed!.delays.slice(5).every(value=>value===50));assert.ok(observed!.delays.length<=8);assert.equal(snapshots,observed!.delays.length+1);assert.ok(observed!.elapsedMs<5_000);assert.equal(publicationHooks,0);assert.equal(callbackEntries,0);assert.deepEqual(await readdir(root),["lock"]);assert.deepEqual(await readFile(path.join(lock,"owner.json")),ownerBytes);}));
