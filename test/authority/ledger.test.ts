...
async function withRecordedDelays<T>(operation:()=>Promise<T>,onDelay:(requested:number)=>void=()=>{}):Promise<Readonly<{result:T;delays:number[];elapsedMs:number}>>{const originalTimeout=setTimeout,originalClock=process.hrtime.bigint,delays:number[]=[],started=originalClock();let current=started;const replacement=((...args:Parameters<typeof setTimeout>)=>{const requested=Number(args[1]??0);delays.push(requested);onDelay(requested);current+=BigInt(Math.trunc(requested*1_000_000));return originalTimeout(args[0],0);}) as typeof setTimeout;Object.defineProperty(process.hrtime,"bigint",{configurable:true,writable:true,value:()=>current});Object.defineProperty(globalThis,"setTimeout",{configurable:true,writable:true,value:replacement});try{const result=await operation();return {result,delays,elapsedMs:Number(current-started)/1e6};}finally{Object.defineProperty(globalThis,"setTimeout",{configurable:true,writable:true,value:originalTimeout});Object.defineProperty(process.hrtime,"bigint",{configurable:true,writable:true,value:originalClock});}}
