...
const publicationCleanupFaultPoints=["after-lock-publication-rename-collision","before-publication-stage-root-reenumeration","before-publication-stage-remove"] as const;
...
test("new ingress, clock, and ledger-lock fault-point sets are complete and disjoint", () => {
  ...
  assert.deepEqual(ledgerLockFaultPoints, [...publicationCrashPoints,...publicationSnapshotFaultPoints,...publicationCleanupFaultPoints,...ledgerOperationCallbackFaultPoints,...ledgerLockDurabilityPoints]);
