...
test("all late publication snapshot boundaries use one bounded retry protocol",{timeout:5_000},async t=>{
  ...
  await t.test("persistent-final-validation",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"f".repeat(64),pid:await exitedChildPid(),v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes);let finalValidations=0,generations=0,callbackEntries=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-publication-stage-enumeration")generations++;if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="before-publication-stage-final-validation"){finalValidations++;throw Object.assign(new Error("EBUSY"),{code:"EBUSY"});}}} as never).recover());assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.equal(finalValidations,6);assert.equal(generations,6);assert.equal(callbackEntries,0);assert.deepEqual(observed.delays,[5,10,20,40,25]);assert.equal(observed.elapsedMs,100);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);}));
  await t.test("persistent-root-reenumeration",()=>withRoot(async root=>{const owner={host:hostname(),nonce:"1".repeat(64),pid:await exitedChildPid(),v:1 as const},bytes=authorityCanonicalBytes(owner),stage=await writePublicationStage(root,owner,bytes);let attempts=0,generations=0,callbackEntries=0;const observed=await withRecordedDelays(()=>new RawFsAuthorityLedger(root,{now:()=>t0,lockTimeoutMs:100,faultInjector:(point:string)=>{if(point==="after-publication-stage-enumeration")generations++;if(point==="before-ledger-operation-callback")callbackEntries++;if(point==="before-publication-stage-root-reenumeration"){attempts++;throw Object.assign(new Error("EACCES"),{code:"EACCES"});}}} as never).recover());assert.deepEqual(observed.result,{ok:false,reason:"busy"});assert.equal(attempts,6);assert.equal(generations,6);assert.equal(callbackEntries,0);assert.deepEqual(observed.delays,[5,10,20,40,25]);assert.equal(observed.elapsedMs,100);assert.deepEqual(await readFile(path.join(stage,"owner.json")),bytes);}));
});
