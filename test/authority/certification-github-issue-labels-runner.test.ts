...
async function fixture(mode: "normal" | "source-drift" | "effect-drift" | "provider-503" | "accessor-response" | "cut-after-budget" | "cut-after-dispatched" | "cut-after-send-intent" = "normal", authorityMode: "valid" | "absent" | "substituted" = "valid") {
  ...
  const readinessKey = generateKeyPairSync("ed25519"), jobKey = generateKeyPairSync("ed25519"), delegationKey = generateKeyPairSync("ed25519"), receiptKey = generateKeyPairSync("ed25519"), contractKey = generateKeyPairSync("ed25519"), gateKey = generateKeyPairSync("ed25519");
  ...
  const descriptors = [human, jobSigner, delegationSigner, receiptSigner, contractSigner, gateSigner], events: any[] = [];
  ...
  const hermeticGitHubAuthority = { contractDescriptor: contractSigner, contractPrivateKey: authorityMode === "substituted" ? substituted.privateKey : contractKey.privateKey, gateDescriptor: gateSigner, gatePrivateKey: gateKey.privateKey };
