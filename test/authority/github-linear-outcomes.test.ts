...
test("real Linear comment executor validates exact duplicate, conflict, and ambiguous readback without resend", async () => {
  ...
  for (const [name, model, exactReadback] of [
    ...
    const state = { reservation: { reservationId: `${name}_reservation`, state: "reserved", intent: { requestId: `${name}_request`, effectDigest: authorityDigest(operation.contract), effectCanonicalBase64: encoded } }, effect: compiled.effect, effectDigest: authorityDigest(operation.contract), effectCanonicalBase64: encoded } as any;
  ...
  for (const outcome of ["exact-existing", "conflict"] as const) {
    ...
    const encoded = Buffer.from(JSON.stringify(compiled.effect)).toString("base64");
    const state = { reservation: { reservationId: `comment_${outcome}`, state: "reserved", intent: { requestId: `comment_${outcome}_request`, effectDigest: authorityDigest(comment.contract), effectCanonicalBase64: encoded } }, effect: compiled.effect, effectDigest: authorityDigest(comment.contract), effectCanonicalBase64: encoded } as any;
...
test("a verified merge plus unresolved Linear effect stays pending before Linear-only completion with zero GitHub calls", async () => {
  ...
  const commentState: any = { reservationId: "reservation_1", state: "reserved", intent: { requestId: "comment_request_01", effectDigest: authorityDigest(commentOperation.contract), effectCanonicalBase64: Buffer.from(JSON.stringify(compiled.effect)).toString("base64"), executionContext: { allocationId: "linear-comment-allocation" } } };
  const statusState: any = { reservationId: "reservation_2", state: "reserved", intent: { requestId: "status_request_01", effectDigest: authorityDigest(statusOperation.contract), effectCanonicalBase64: Buffer.from(JSON.stringify(compiledStatus.effect)).toString("base64"), executionContext: { allocationId: "linear-status-allocation" } } };
