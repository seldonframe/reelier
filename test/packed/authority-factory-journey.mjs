...
const graphVerification = trustPin => {
  const evidenceSigner = trustPin.keyDescriptors.find(item => item.role === "authority-cell" && item.purpose === "authority-evidence"); assert.equal(typeof evidenceSigner?.keyId, "string");
  return { trustPin, currentTrustObservation: { v: "reelier.portable-current-trust-observation/v1", observedAt: "2026-08-11T20:00:00.000Z", expiresAt: "2026-08-11T21:00:00.000Z", activeAuthorityEvidenceSignerIds: [evidenceSigner.keyId] }, now: new Date("2026-08-11T20:10:00.000Z"), expectedResponseSemanticsProfile: { v: "reelier.http-response-semantics/v1", profileId: "github.issue-labels.hermetic-v1", acknowledgedStatuses: [200] } };
};
...
try {
  ...
  if (evidenceIndex >= 0) {
    ...
    assert.equal(authority.verifyCertificationTaskReceiptGraph(graph, graphVerification(trustPin)).status, "verified"); assert.equal(digest(path.join(evidence, "graph.json")), metadata.graphDigest); assert.equal(digest(path.join(evidence, "trust-pin.json")), metadata.trustPinDigest); assert.equal(digest(path.join(evidence, "factory-journey-summary.json")), metadata.summaryDigest); assert.equal(digest(tarball).slice(7), metadata.tarballSha256); assert.equal(authority.AUTHORITY_ADAPTER_CONTRACT_V1_DIGEST, metadata.adapterContractDigest);
  ...
  assert.equal(authority.verifyCertificationTaskReceiptGraph(graph, graphVerification(trustPin)).status, "verified"); assert.equal(line.graphDigest, authority.authorityDigest(graph)); assert.equal(line.summaryDigest, authority.authorityDigest(summary)); assert.equal(summary.graphDigest, line.graphDigest);
