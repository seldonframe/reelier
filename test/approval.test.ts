...
test("computeApprovalHash is stable across arg-key order", () => {
  const h1 = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { a: 1, b: 2 }, attest: undefined, expect: undefined });
  const h2 = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { b: 2, a: 1 }, attest: undefined, expect: undefined });
  assert.equal(h1, h2);
  assert.match(h1, /^sha256:[0-9a-f]{64}$/);
});

test("computeApprovalHash differs when the tool changes", () => {
  const h1 = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { a: 1 }, attest: undefined, expect: undefined });
  const h2 = computeApprovalHash({ emit: undefined, actionTool: "crm.update_contact", actionArgs: { a: 1 }, attest: undefined, expect: undefined });
  assert.notEqual(h1, h2);
});

test("computeApprovalHash differs when a placeholder is swapped into the args template", () => {
  const h1 = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { email: "a@example.com" }, attest: undefined, expect: undefined });
  const h2 = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { email: "{{email}}" }, attest: undefined, expect: undefined });
  assert.notEqual(h1, h2);
});
...
test("computeApprovalHash does NOT bind server (offline reelier approve can't know it)", () => {
  ...
  const h = computeApprovalHash({ emit: undefined, actionTool: "crm.create_contact", actionArgs: { a: 1 }, attest: undefined, expect: undefined });
