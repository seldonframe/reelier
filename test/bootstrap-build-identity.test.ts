...
async function packageRoot(files: readonly string[] = ["dist", "contract"]): Promise<string> {
  ...
  await writeFile(join(root, "contract", "schema.json"), "{}\n");
  return root;
}
...
test("installed build digest is stable across creation order and changes with shipped bytes or version", async () => {
  ...
  const second = await packageRoot(["contract", "dist"]);
...
test("installed build digest refuses symlinks and case-colliding package paths", async () => {
  ...
  await symlink(join(linked, "dist", "index.js"), join(linked, "dist", "alias.js"), "file");
