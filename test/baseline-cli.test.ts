...
function runArgs(positional: string[]): ParsedArgs {
...
async function captureRun(dir: string, skillPath: string): Promise<{ code: number; stdout: string }> {
  const logs: string[] = [];
  ...
  console.error = () => {};
  try {
    const code = await cmdRun(runArgs([skillPath]), async () => fakeConnection(), { cwd: dir });
