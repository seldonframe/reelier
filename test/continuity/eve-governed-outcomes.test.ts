...
test("real Eve 0.39 drives both reviewed missions through the remote quartet and durable recovery",{skip},async()=>{
  ...
  try{
    ...
    const driver=await import(pathToFileURL(path.join(eveRoot,"scripts","eve-governed-outcomes.mjs")).href) as {runEveGovernedOutcomesV1(input:unknown):Promise<{processRestarts:number;processIdentities:{pid:number;port:number;startedAt:string}[];sessionIds:string[];actions:{requested:string[];completed:string[];failed:string[]};toolEvents:unknown[];messages:string[]}>};
    ...
    assert.equal(report.processRestarts,report.processIdentities.length-1);assert.equal(report.processRestarts,2);assert.equal(new Set(report.processIdentities.map(item=>item.pid)).size,3);assert.equal(new Set(report.processIdentities.map(item=>`${item.pid}:${item.port}:${item.startedAt}`)).size,3);assert.equal(runtimeReopens,1);assert.equal(new Set(report.sessionIds).size,4);
    const exactInventory=["reelier_agent_status","reelier_agent_status","reelier_outcome_proposal","reelier_outcome_proposal","reelier_outcome_request","reelier_outcome_request","reelier_outcome_status","reelier_outcome_status"].sort();assert.deepEqual([...report.actions.requested].sort(),exactInventory);assert.deepEqual([...report.actions.completed].sort(),exactInventory);assert.deepEqual(report.actions.failed,[]);
    ...
    await runtime.reviewOutcomes(["request_composite","request_linear"]);const evidence=await runtime.inspectEvidence(),ledger=await (await fixture.ledger()).recover();assert.equal(evidence.activationConfirmations,1);assert.equal(evidence.routineApprovals,0);assert.equal(evidence.requests.length,2);assert.deepEqual(evidence.reviews.map(item=>item.requestIds.length),[2]);assert.equal(new Set(evidence.requests.map(item=>item.requestId)).size,2);assert.equal(new Set(evidence.requests.map(item=>item.missionId)).size,2);assert.equal(new Set(evidence.requests.map(item=>item.executionContext.allocationId)).size,2);assert.equal(new Set(evidence.requests.map(item=>item.executionContext.runtimeSessionId)).size,2);assert.equal(ledger.ok&&ledger.reservations.length,7);assert.equal(ledger.ok&&new Set(ledger.reservations.map(item=>item.reservationId)).size,7);
