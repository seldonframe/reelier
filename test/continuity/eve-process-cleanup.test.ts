...
test("Linux Eve cleanup kills a SIGTERM-resistant detached descendant while its launcher is alive", { skip: process.platform === "linux" ? false : "requires POSIX process groups" }, async () => {
  ...
  const { isLiveProcess, stopEveProcess } = await import(moduleUrl) as { isLiveProcess(pid: number): boolean; stopEveProcess(child: ReturnType<typeof spawn>): Promise<void> };
  ...
  try {
    ...
    assert.equal(isLiveProcess(descendantPid), true);
    ...
    assert.equal(await waitForProcessExit(isLiveProcess, descendantPid, 2_000), true, `descendant PID ${descendantPid} survived Eve cleanup`);
  } finally {
...
async function waitForProcessExit(isLiveProcess: (pid: number) => boolean, pid: number, timeoutMs: number): Promise<boolean> {
