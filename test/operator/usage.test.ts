...
test("Operator plans charge for managed authority capacity, not inference or receipts", () => {
  assert.equal(operatorPlanV1("free-local").monthlyPriceUsd, 0);
  assert.equal(operatorPlanV1("free-local").maxConcurrentExecutions, null, "free Local Mission Control must not encode an artificial plan limit");
  assert.equal(operatorPlanV1("managed-personal").monthlyPriceUsd, 49);
  assert.equal(operatorPlanV1("managed-personal").maxConcurrentExecutions, 10);
