...
`preflight`, `seal-readiness`, and `export` require exactly one explicit selection: `--scenario <exact-initialized-id>` or `--all`. Missing, conflicting, duplicate, unknown, or extra arguments refuse; selection never widens by default. The initialization commitment root and generated task, Job Card, grant, Cell, and signer IDs remain unchanged for the workspace's lifetime. A narrower command binds a separate selection digest beneath that root; it never creates a replacement authority root or identifier set. Preflight reads and validates the actual immutable initialization snapshot. It never resolves an `env:` or `file:` reference, invokes a provider, probes a runtime, or makes a network request. Runner and test artifacts use `<scenario>.json` or `<scenario>--<name>.json` under `authority/certification/inputs/runners/` and `inputs/tests/`; a selected run never inventories another scenario's files. Every requested and canonical path component is inspected and actual linked/junction/reparse-pointed workspace parents, input directories, or artifacts refuse. Directory trust binds the pre-walk object identity to requested and canonical `stat`/`lstat` observations after canonicalization, so an ancestor replacement refuses even when it points back to the same directory. Ordinary Windows case differences and 8.3 short-name aliases remain valid path spellings.
