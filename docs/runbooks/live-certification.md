...
Use the closed scenario-scoped v3 operator file. It contains resource identifiers and named secret references, never secret values:
...
`init` validates before publishing anything, writes a complete sibling workspace through atomic rename, and deterministically resumes an identical initialization. Workspace, config, snapshot, and staging paths refuse links, junctions, and reparse points, including a custom workspace whose missing parent would traverse one. A failed contender scopes cleanup to the already verified canonical creation parent, exact workspace-derived stage prefix, and unguessable owner marker; Windows short-name spelling cannot strand its owned stage, while old or foreign name-matching stages are ignored. It generates the task, Job Card, root-grant, Authority Cell, and signer identifiers internally. Operators must not add those identifiers to `certification.local.json`.

`preflight`, `seal-readiness`, and `export` require exactly one explicit selection: `--scenario <exact-initialized-id>` or `--all`. Missing, conflicting, duplicate, unknown, or extra arguments refuse; selection never widens by default. The initialization commitment root and generated task, Job Card, grant, Cell, and signer IDs remain unchanged for the workspace's lifetime. A narrower command binds a separate selection digest beneath that root; it never creates a replacement authority root or identifier set. Preflight reads and validates the actual immutable initialization snapshot. It never resolves an `env:` or `file:` reference, invokes a provider, probes a runtime, or makes a network request. Runner and test artifacts use `<scenario>.json` or `<scenario>--<name>.json` under `authority/certification/inputs/runners/` and `inputs/tests/`; a selected run never inventories another scenario's files. Requested and canonical path components are inspected; links and directory identity changes observed during those checks refuse. Ordinary Windows case differences and 8.3 short-name aliases remain valid path spellings. This is bounded local preparation evidence, not proof of exclusive filesystem control: portable Node lacks handle-relative no-follow traversal, so hostile concurrent same-user mutation after any finite check remains `unchecked`. Managed autonomous dispatch must rely on the isolated Authority Cell and certified topology, never on this local check as a `verified` or `safe` boundary.

Provider runner preparation uses three selected-scenario-only directories: `inputs/runners/`, `inputs/tests/`, and `inputs/plans/`. V2 runner manifests bind the private built-in registry and the exact lifecycle; reviewed scenario plans bind the runner, test, endpoint, registry, desired non-secret choices, policy commitments, cleanup before-state, and controlled-cut case. V1 runner and endpoint manifests remain readable for migration but cannot satisfy dispatch readiness.

Preflight reports phase-specific `preparationReady`; the future human signature is not a preparation dependency. Missing selected runner, test, or plan evidence makes preparation incomplete and `seal-readiness` refuses. A complete preparation candidate is still marked `awaiting-human-signature`, `signatureStatus=absent`, `authorization=absent`, `dispatchable=false`, and `completeness=unchecked`. It does not sign, grant authority, or unlock `run`; interactive human signing belongs to the later Authority Cell signing gate.

`export` writes private `0600` content-addressed files and includes only the sanitized selected resource/cleanup projection, the actual initialization artifact, generated identifiers, redacted reference status, preflight, and candidate. The stable initialization root binds the complete private config and full initialized projection. The separate selection digest binds the exported subset beneath that root; generated identifiers always derive from the initialization root. Export preflights once to form the exact readiness candidate, observes the selected inputs again before publication, refuses drift, and self-verifies the complete bundle before atomic no-overwrite publication. It excludes secret-reference payloads and local authority, evidence, Codex, Fly, and credential paths. Offline `verify` enforces initialization-root and five-ID continuity, verifies that the selection is a subset of the initialized scenarios, recomputes the selected projection/selection digest, semantic preflight readiness, every artifact digest, and every link. Exact mapped artifact links and linked output directories refuse. Publication sets the temporary file mode before linking it into place and never changes permissions through the final pathname. A valid package still reports provider certification, signature verification, completion, and universal completeness as `unchecked`.

The scenario-scoped v3 format is tracked at `authority/certification.example.json`. It selects only the scenarios being certified and accepts only their exact resource, cleanup, metadata, and named secret-reference sections. It is private certification-estate configuration, not a customer onboarding requirement.
...
The older v1 file under `docs/runbooks/` remains only for the pre-existing live adapter commands until their runner slice migrates. It is not accepted by the initialized preflight flow. HubSpot and manually entered task/Job/Cell identifiers are not retained by v3.
...
.\scripts\import-fly-certification-secret.ps1 -Name REELIER_CLOUDFLARE_DNS_TOKEN
.\scripts\import-fly-certification-secret.ps1 -Name REELIER_CLOUDFLARE_BOOTSTRAP_TOKEN
