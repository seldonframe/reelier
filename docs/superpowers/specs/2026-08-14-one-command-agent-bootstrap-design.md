...
`routeId` is a generated opaque identifier, never a source-qualified string for consumers to parse. Built-in harness adapters derive it as `route_` plus a full lowercase SHA-256 digest over a closed stable identity tuple: contract domain, harness ID, stable adapter source ID, non-secret source-instance identity digest, discovery source, route key, and transport. Raw origin paths and route keys are inputs to the commitment but are not serialized into the identifier. Discovery source remains its own route field; user-facing display metadata remains in the collector/view layer rather than being encoded into `routeId` or added to the frozen route record. Adapter version/code digest, observed content digest, file evidence identity, and contract/evidence identity belong to `evidenceDigest`, not route identity, so an evidence or adapter upgrade rechecks the same route instead of silently renaming it. No consumer splits, prefixes, or otherwise interprets `routeId`.
