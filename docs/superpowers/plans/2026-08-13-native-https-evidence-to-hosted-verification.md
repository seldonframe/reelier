...
**Task 9 portable/private boundary amendment (2026-08-13):** The signed Task Receipt Graph remains a private certification artifact because its job-card/source receipts and execution provenance necessarily carry principal and account identity. It must never be described or published as nonidentifying. The portable surface is a distinct sanitized export with a closed schema containing only version tags, canonical digests, opaque digest-derived signer identifiers, timestamps, closed enums, counts, and a digest/count join back to the private graph. Offline verification receives both artifacts, verifies the private graph under externally supplied trust/time/profile anchors, independently recomputes the sanitized export, and verifies the digest/count join without copying identities. A portable signer identifier may not be a login, account label, email, or arbitrary free-form string. Portable secret/nonidentification scanning covers the complete serialized export file, not a selected nested fixture. This amendment is recorded before the final remediation production edit because the prior wording conflated portable publication with the necessarily private graph.
...
- Response semantics are externally anchored. For `hermetic-github-fixture`, verification requires the ratified profile preimage `github.issue-labels.hermetic-v1` with acknowledged statuses exactly `[200]` and its independently recomputed digest (or an equivalently trusted caller-supplied registry entry). Re-signing an artifact with `[201]`, another sorted 2xx set, or an unrelated digest must refuse; the graph may not authenticate its own response profile.
- The private receipt graph and sanitized portable export are distinct artifacts. Portable verification joins them by independently recomputed graph/collection digests and counts; the export never copies job-card, source-receipt, account, login, credential-slot, or principal fields.
