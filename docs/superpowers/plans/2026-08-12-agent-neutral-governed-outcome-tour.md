...
**Dependency:** Begin implementation only after `docs/superpowers/plans/2026-08-12-windows-client-linux-authority-cell.md` Tasks 4–6 are complete, independently reviewed, and green against the exact packed artifact. This plan may reuse those reviewed internals but may not change their authority or proof semantics to improve onboarding.
...
- Report sections are exact: `environment`, `pathA`, `pathB`, `pathC`, `verification`, `metrics`, `nonClaims`.

- [ ] Write a failing test that `reelier authority tour --json` is recognized, accepts only `--json` and a confined `--output`, rejects credentials/endpoints/agents/callbacks, and returns a closed report whose unknown keys and accessors are rejected without invocation.
...
- [ ] Run focused tests, including drift/tamper falsifiers and zero external access assertions.
- [ ] Commit RED and GREEN separately and write the task report.
...
- [ ] Write a failing integration test that initializes a fresh confined fixture, exercises one explicit fixture operator confirmation over an opaque signed authority root, records `liveHumanReview: absent`, uses the reviewed Task 4 root→child execution lineage, applies one label Outcome, reconciles an apply-then-cut without resend, restores exact state, and exports a graph.
...
- [ ] Run focused lifecycle/tour tests and verify every duplicate, conflict, ambiguity, refusal, revocation, and unsupported-case reason actually emitted by the reviewed lifecycle remains honest and non-passing where applicable; unsupported categories remain `absent`.
- [ ] Commit RED and GREEN separately and write the task report.
...
- [ ] Write failing tests that verify the clean tour offline and reject altered contract digest, receipt omission/fork, grant/principal substitution, budget imbalance, unknown terminal reason, false pass for ambiguous/manual/absent evidence, and a secret canary.
...
- [ ] Add failure copy that distinguishes `failed`, `blocked`, `manual`, `unchecked`, and `absent`; never call them safe or successful.
- [ ] Implement concise interactive output with links to Path A/B/C docs and an optional, separate read-only local-detection command suggestion; do not inspect local state automatically and state that native Windows does not host the Path C Cell.
