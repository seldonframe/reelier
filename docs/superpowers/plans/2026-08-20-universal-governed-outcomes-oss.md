...
#### Task 5 post-Task4C replacement amendment

Replace the rejected raw-provider Task 5 proof with the reviewed genuine Task4C signed five-definition runtime. One fresh disposable signed standing activation/grant/Job Card/Cell lineage serves both Outcomes. The Outcomes must carry distinct runtime-derived mission/request/reservation/allocation identities and distinct Eve process/session identities, while activation count one and routine approval count zero are folded from signed activation/gate artifacts. Never fabricate a second lineage to satisfy freshness.

Additional authorized modifications: `src/authority/host/agent-tools.ts`, `src/authority/host/github-linear-mission-runtime.ts`, `src/authority/ingress/agent-tool-contracts.ts`, `src/authority/ingress/http.ts`, `src/authority/ingress/mcp.ts`, `test/authority/agent-tools.test.ts`, `test/authority/ingress.test.ts`, `test/authority/github-linear-mission-runtime.test.ts`, `conformance/continuity-adapter/v1/eve-fixture/agent/agent.ts`, `conformance/continuity-adapter/v1/eve-fixture/agent/lib/cell.ts`, `conformance/continuity-adapter/v1/eve-fixture/scripts/eve-governed-outcomes.mjs`, `conformance/continuity-adapter/v1/eve-fixture/scripts/eve-process.mjs`, `conformance/continuity-adapter/v1/eve-fixture/tests/cell.test.ts`, `test/continuity/eve-binding-static.test.ts`, `test/continuity/eve-governed-outcomes.test.ts`, and `test/continuity/eve-kill-resume.test.ts`. Create `test/continuity/support/genuine-governed-eve.ts`. Delete the unused synthetic `conformance/continuity-adapter/v1/eve-fixture/agent/lib/governed-outcomes.ts`; no production host/package rehearsal export exists.

The HTTP/MCP ingress remains provider-neutral and resolves a supplied current-runtime capability per call, so crash recovery cannot route to a captured runtime. Canonical/backend parsing must reject hostile nested proxies, accessors, symbols, and size excess before observable reads. Real Eve 0.39 must emit native quartet actions and completed outputs for both run/resume prompts. Driver, matrix child, stream, and pre-effect fault waits are bounded. The stale matrix Outcome path is replaced with the same genuine signed fixture; checkpoint and read-only continuity scenarios remain compatible. Mission/effect lifecycle uses Task4C's signed-journal storage and genuine ledger/coordinator publication; the existing file storage remains only the reviewed receipt-publication dependency.

#### Task 5 two-target signed-lineage amendment

Keep one signed standing activation but extend its internal definition inventory from five to seven because the two missions have different reviewed Linear targets. The existing composite Linear pair commits `github-linear` to `REEL-TEST-1`; a new explicit Linear-only comment/status pair commits `linear-only` to `REEL-TEST-2`. Each target commitment is closed over exact workspace, team, project, issue, pre/target status, comment marker, evidence URL/digest, and host binding refs. Each pair has distinct reviewed contract, policy, semantic-identity, and idempotency digests and its own exact comment-to-status predecessor policy. Runtime pair selection follows only the authenticated context-bound opaque mode; the model cannot name or override a target. The external four-tool ABI remains unchanged.

Additional authorized production/build files: `src/authority/packs/github-linear-outcomes.ts`, `src/authority/host/linear-outcome-runner.ts`, `src/packs/linear-outcomes/manifest.ts`, `src/packs/linear-outcomes/compile.ts`, `src/packs/linear-outcomes/source.ts`, `src/packs/linear-outcomes/index.ts`, `scripts/build-packs.mjs`, and `src/packs/conformance.ts`. Additional authorized tests: `test/authority/github-linear-outcomes.test.ts`, `test/authority/linear-outcomes-pack.test.ts`, `test/packs/conformance.test.ts`, and `test/acceleration-preflight.test.ts`. Preserve twelve first-party packs while growing the Linear manifest to four definitions and the exact overall alias inventory from sixteen to eighteen. Do not modify `src/authority/pack.ts` unless a focused RED proves its endpoint/purity allowlist requires change and the scope is amended first. Task4C documents remain historical records of the reviewed five-definition runtime it delivered.
