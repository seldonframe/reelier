...
Every terminal return or throw after creator-stage creation and before successful publication must pass through one bounded exact-creator withdrawal funnel. Withdrawal uses a fresh monotonic cleanup deadline, never depends on unrelated publication membership, and may remove only the literal creator path after its non-following directory and owner identities, types, link counts, bytes, and frozen construction state remain exact. Missing is already withdrawn; authorized same-directory removal progress may finish; absence is followed by a ledger-root sync. A replacement, reparse point, hardlink, wrong type, or identity mismatch is preserved and refines a non-corruption terminal result to corruption. Exact withdrawal never changes refusal into success and never mutates a peer. Before creating a new stage, an exact settled stage with the local host and current PID is a denial-only busy backstop: the caller creates no duplicate-provenance stage.
